.PHONY: install dependency-refresh lint ci-contract-gate main-gate-coverage-audit repository-hygiene-gate test-client-lifecycle-gate maintainability-gate duplicate-implementation-inventory duplicate-implementation-gate private-import-boundary-gate foundation-structure-gate documentation-contract-gate quality-scorecard-gate github-issue-closure-matrix-gate rfc0002-github-issue-execution-ledger-gate rfc0002-github-issue-execution-state-audit rfc0002-github-issue-execution-summary rfc0002-cross-repo-issue-posture rfc0002-issue-posture-live-gate rfc0002-github-issue-learning-pattern-gate rfc0002-github-issue-pr-text-gate monetary-float-guard no-sensitive-content-guard runtime-dependency-closure-gate dependency-vulnerability-posture-gate implementation-proof-closure-manifest-gate blueprint-scope-coverage-gate source-observability-contract-gate api-route-metadata-gate api-problem-details-boundary-gate api-idempotency-boundary-gate api-camel-model-boundary-gate api-signal-model-boundary-gate api-temporal-validation-boundary-gate openapi-problem-details-example-gate caller-context-contract-gate signal-api-contract-gate trusted-tenant-context-gate source-temporal-contract-gate review-queue-snapshot-contract-gate operation-metric-contract-gate service-slo-capacity-contract-gate ai-model-risk-ops-contract-gate ai-model-risk-operations-proof-contract-gate ci-signal-evidence-contract-gate implementation-truth-gate data-mesh-contract-gate mesh-policy-source-contract-proof-gate opportunity-archetype-contract-gate demo-readiness-claim-matrix-gate downstream-realization-contract-gate downstream-route-source-contract-proof-gate advise-intake-runtime-execution-proof-gate manage-intake-runtime-execution-proof-gate outbox-event-contract-gate outbox-consumer-contract-gate outbox-recovery-contract-gate migration-contract-gate migration-execution-gate deployment-migration-contract-gate durable-repository-proof-contract-gate runtime-trust-telemetry-test-execution-contract-gate ai-lineage-store-proof-contract-gate ai-workflow-pack-registration-proof-contract-gate ai-workflow-pack-runtime-execution-proof-contract-gate report-intake-route-source-contract-proof-gate report-materialization-source-contract-proof-gate report-materialization-runtime-execution-proof-contract-gate workbench-read-path-source-contract-proof-gate gateway-workbench-contract-proof-contract-gate gateway-workbench-discovery-contract-proof-contract-gate outbox-broker-source-contract-proof-gate outbox-broker-runtime-execution-proof-gate outbox-consumer-contract-proof-contract-gate outbox-consumer-runtime-execution-proof-gate outbox-platform-mesh-event-source-contract-proof-gate platform-catalog-source-contract-proof-gate source-ingestion-worker-check source-ingestion-scheduled-worker-check source-ingestion-runtime-execution-contract-gate canonical-opportunity-source-proofs canonical-signal-api-proof risk-concentration-live-proof-contract-gate high-volatility-live-proof-contract-gate risk-drawdown-live-proof-contract-gate core-benchmark-assignment-live-proof-contract-gate core-portfolio-state-live-proof-contract-gate bond-maturity-live-proof-contract-gate missing-benchmark-live-proof-contract-gate missing-benchmark-performance-readiness-proof-contract-gate low-income-core-cashflow-live-proof-contract-gate manage-mandate-live-proof-contract-gate mandate-restriction-live-proof-contract-gate mandate-restriction-source-product-proof-contract-gate missing-suitability-live-proof-contract-gate missing-risk-profile-source-product-proof-contract-gate missing-risk-profile-live-proof-contract-gate performance-underperformance-live-proof-contract-gate implementation-proof-readiness-check runtime-trust-telemetry-preview-check runtime-trust-telemetry-snapshot-check migrate migrate-rollback deployment-migrate supported-features-gate endpoint-certification-gate postgres-integration-gate typecheck architecture-boundary-gate architecture-boundary-report quality-baseline openapi-gate test test-unit test-integration test-e2e test-unit-coverage test-integration-coverage test-e2e-coverage test-coverage security-audit check ci ci-release compose-config-gate docker-build container-runtime-smoke release-sbom container-image-scan release-image-identity-contract-gate clean
...
LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT ?= output/report/intake-runtime-execution-proof.json
...
LOTUS_IDEA_REPORT_MATERIALIZATION_RUNTIME_EXECUTION_PROOF_OUTPUT ?= output/report/materialization-runtime-execution-proof.json
LOTUS_REPORT_PYTHON ?= python
LOTUS_IDEA_ADVISE_ROUTE_SOURCE_CONTRACT_PROOF ?=
LOTUS_ADVISE_ROOT ?= ../lotus-advise
LOTUS_IDEA_ADVISE_ROUTE_SOURCE_CONTRACT_PROOF_OUTPUT ?= output/downstream/advise-route-source-contract-proof.json
LOTUS_IDEA_ADVISE_INTAKE_RUNTIME_EXECUTION_PROOF ?=
LOTUS_IDEA_ADVISE_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT ?= output/downstream/advise-intake-runtime-execution-proof.json
LOTUS_ADVISE_PYTHON ?= ../lotus-advise/.venv-codex/Scripts/python.exe
LOTUS_IDEA_MANAGE_ROUTE_SOURCE_CONTRACT_PROOF ?=
LOTUS_MANAGE_ROOT ?= ../lotus-manage
LOTUS_IDEA_MANAGE_ROUTE_SOURCE_CONTRACT_PROOF_OUTPUT ?= output/downstream/manage-route-source-contract-proof.json
...
IMPLEMENTATION_PROOF_OUTPUT ?= $(LOTUS_IDEA_IMPLEMENTATION_PROOF_READINESS_OUTPUT)
...
LOTUS_IDEA_OUTBOX_BROKER_RUNTIME_EXECUTION_PROOF_OUTPUT ?= output/outbox/broker/runtime-execution-proof.json
LOTUS_IDEA_OUTBOX_CONSUMER_RUNTIME_EXECUTION_PROOF ?=
LOTUS_IDEA_OUTBOX_CONSUMER_RUNTIME_EXECUTION_PROOF_OUTPUT ?= output/outbox/consumer-runtime-execution-proof.json
...
LOTUS_IDEA_OPPORTUNITY_ARCHETYPE_EVIDENCE_PACK_OUTPUT ?= output/opportunity/canonical-archetype-evidence-pack.json
CANONICAL_OPPORTUNITY_PORTFOLIO_ID ?=
CANONICAL_OPPORTUNITY_TENANT_ID ?=
CANONICAL_OPPORTUNITY_BOOK_ID ?=
CANONICAL_OPPORTUNITY_CLIENT_ID ?=
CANONICAL_OPPORTUNITY_EVALUATION_ID ?=
...
SERVICE_DEPENDENCY_RECOVERY_PROOF ?= output/observability/service-dependency-recovery-proof.json
...
lint:
	...
	$(MAKE) rfc0002-github-issue-pr-text-gate
	$(MAKE) monetary-float-guard
	$(MAKE) no-sensitive-content-guard
	$(MAKE) runtime-dependency-closure-gate
	$(MAKE) dependency-vulnerability-posture-gate
	$(MAKE) implementation-proof-closure-manifest-gate
	$(MAKE) blueprint-scope-coverage-gate
	...
	$(MAKE) feedback-evaluation-contract-gate
...
rfc0002-github-issue-execution-state-audit:
	$(VENV_PYTHON) scripts/github_issue_execution_state_audit.py

rfc0002-github-issue-execution-summary:
...
rfc0002-issue-posture-live-gate:
	$(VENV_PYTHON) scripts/issue_posture_live_gate.py

rfc0002-github-issue-learning-pattern-gate:
	$(VENV_PYTHON) scripts/github_issue_learning_pattern_gate.py

rfc0002-github-issue-pr-text-gate:
	$(VENV_PYTHON) scripts/github_issue_pr_text_gate.py
...
dependency-vulnerability-posture-gate:
	$(VENV_PYTHON) scripts/dependency_vulnerability_posture_gate.py

implementation-proof-closure-manifest-gate:
	$(VENV_PYTHON) scripts/implementation_proof_closure_manifest_gate.py

blueprint-scope-coverage-gate:
	$(VENV_PYTHON) scripts/blueprint_scope_coverage_gate.py
...
feedback-evaluation-contract-gate:
	$(VENV_PYTHON) scripts/feedback_evaluation_contract_gate.py
...
service-dependency-recovery-proof-gate:
	$(VENV_PYTHON) scripts/service_dependency_recovery_proof_gate.py --artifact "$(SERVICE_DEPENDENCY_RECOVERY_PROOF)"

postgres-capacity-threshold-proof-gate:
	$(VENV_PYTHON) scripts/postgres_capacity_threshold_proof_gate.py --artifact "$(POSTGRES_CAPACITY_PROOF_OUTPUT)"
...
incident-response-contract-gate:
	$(VENV_PYTHON) scripts/incident_response_contract_gate.py
...
mesh-policy-source-contract-proof-gate:
	$(VENV_PYTHON) scripts/data_mesh/mesh_policy_source_contract_gate.py

opportunity-archetype-contract-gate:
	$(VENV_PYTHON) scripts/opportunity_archetype_contract_gate.py

opportunity-archetype-evidence-pack-gate:
	$(VENV_PYTHON) scripts/opportunity_archetype_evidence_pack/evidence_pack_contract_gate.py

demo-readiness-claim-matrix-gate:
	$(VENV_PYTHON) scripts/demo_readiness_claim_matrix_gate.py
...
downstream-route-source-contract-proof-gate:
	$(VENV_PYTHON) scripts/downstream_realization/route_source_contract_gate.py

advise-intake-runtime-execution-proof-gate:
	$(VENV_PYTHON) scripts/downstream_realization/advise_intake_runtime_execution_gate.py $(LOTUS_IDEA_ADVISE_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT)
...
runtime-trust-telemetry-test-execution-contract-gate:
	$(VENV_PYTHON) scripts/runtime_trust_telemetry/test_execution_contract_gate.py
...
report-intake-runtime-execution-proof-gate:
	$(VENV_PYTHON) scripts/report/intake_runtime_execution_gate.py $(LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT)
...
report-materialization-runtime-execution-proof-gate:
	$(VENV_PYTHON) scripts/report/materialization_runtime_execution_gate.py $(LOTUS_IDEA_REPORT_MATERIALIZATION_RUNTIME_EXECUTION_PROOF_OUTPUT)
...
outbox-broker-runtime-execution-proof-gate:
	$(VENV_PYTHON) scripts/outbox/broker/runtime_execution_gate.py $(LOTUS_IDEA_OUTBOX_BROKER_RUNTIME_EXECUTION_PROOF)

outbox-consumer-contract-proof-contract-gate:
	$(VENV_PYTHON) scripts/outbox/consumer_contract_proof_contract_gate.py

outbox-consumer-runtime-execution-proof-gate:
	$(VENV_PYTHON) scripts/outbox/consumer_runtime_execution_gate.py $(LOTUS_IDEA_OUTBOX_CONSUMER_RUNTIME_EXECUTION_PROOF)
...
mandate-restriction-source-product-proof-contract-gate:
	$(VENV_PYTHON) scripts/advise_source_product_evidence/source_contract_gate.py --capability mandate-restriction
...
missing-risk-profile-source-product-proof-contract-gate:
	$(VENV_PYTHON) scripts/advise_source_product_evidence/source_contract_gate.py --capability missing-risk-profile
...
missing-benchmark-live-proof-contract-gate:
	$(VENV_PYTHON) scripts/core_missing_benchmark_runtime_evidence/runtime_execution_contract_gate.py

missing-benchmark-performance-readiness-proof-contract-gate:
	$(VENV_PYTHON) scripts/performance_benchmark_readiness_runtime_evidence/runtime_execution_contract_gate.py
...
implementation-proof-readiness-check:
	$(VENV_PYTHON) -m scripts.source_ingestion_scheduler.generate_source_contract --manifest docs/examples/source-ingestion/high-cash-worker-manifest.example.json --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --run-forever --output output/source-ingestion/scheduled-worker-source-contract.json
	...
	$(VENV_PYTHON) scripts/runtime_trust_telemetry/generate_test_execution_contract.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --output output/trust-telemetry/test-execution/runtime-trust-telemetry-test-execution.json
	...
	$(VENV_PYTHON) scripts/downstream_realization/generate_advise_route_source_contract.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --advise-root $(LOTUS_ADVISE_ROOT) --output $(LOTUS_IDEA_ADVISE_ROUTE_SOURCE_CONTRACT_PROOF_OUTPUT) --allow-missing-evidence
	$(VENV_PYTHON) scripts/downstream_realization/generate_advise_intake_runtime_execution.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --advise-root $(LOTUS_ADVISE_ROOT) --advise-python $(LOTUS_ADVISE_PYTHON) --allow-destructive-test-database-reset --output $(LOTUS_IDEA_ADVISE_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT)
	$(VENV_PYTHON) scripts/downstream_realization/generate_manage_route_source_contract.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --manage-root $(LOTUS_MANAGE_ROOT) --output $(LOTUS_IDEA_MANAGE_ROUTE_SOURCE_CONTRACT_PROOF_OUTPUT) --allow-missing-evidence
	$(VENV_PYTHON) scripts/downstream_realization/generate_manage_intake_runtime_execution.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --manage-root $(LOTUS_MANAGE_ROOT) --manage-python $(LOTUS_MANAGE_PYTHON) --output $(LOTUS_IDEA_MANAGE_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT)
	...
	$(VENV_PYTHON) scripts/report/generate_intake_runtime_execution.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --report-root $(LOTUS_REPORT_ROOT) --report-python $(LOTUS_REPORT_PYTHON) --output $(LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT)
	...
	$(VENV_PYTHON) scripts/report/generate_materialization_runtime_execution.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --report-root $(LOTUS_REPORT_ROOT) --report-python $(LOTUS_REPORT_PYTHON) --output $(LOTUS_IDEA_REPORT_MATERIALIZATION_RUNTIME_EXECUTION_PROOF_OUTPUT)
	$(VENV_PYTHON) scripts/outbox/generate_consumer_runtime_execution.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --advise-intake-runtime-execution-proof $(LOTUS_IDEA_ADVISE_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT) --manage-intake-runtime-execution-proof $(LOTUS_IDEA_MANAGE_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT) --report-materialization-runtime-execution-proof $(LOTUS_IDEA_REPORT_MATERIALIZATION_RUNTIME_EXECUTION_PROOF_OUTPUT) --output $(LOTUS_IDEA_OUTBOX_CONSUMER_RUNTIME_EXECUTION_PROOF_OUTPUT)
	$(VENV_PYTHON) scripts/data_mesh/generate_mesh_policy_source_contract.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --output $(LOTUS_IDEA_MESH_POLICY_SOURCE_CONTRACT_PROOF_OUTPUT)
	$(VENV_PYTHON) scripts/data_mesh/generate_platform_catalog_source_contract.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --platform-root $(LOTUS_PLATFORM_ROOT) --output $(LOTUS_IDEA_PLATFORM_CATALOG_SOURCE_CONTRACT_PROOF_OUTPUT) --allow-missing-evidence
	$(VENV_PYTHON) scripts/workbench/generate_discovery_contract_proof.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --platform-root $(LOTUS_PLATFORM_ROOT) --platform-catalog-source-contract-proof $(LOTUS_IDEA_PLATFORM_CATALOG_SOURCE_CONTRACT_PROOF_OUTPUT) --workbench-read-path-source-contract-proof output/workbench/read-path-source-contract-proof.json --gateway-workbench-contract-proof $(LOTUS_IDEA_GATEWAY_WORKBENCH_CONTRACT_PROOF_OUTPUT) --output $(LOTUS_IDEA_GATEWAY_WORKBENCH_DISCOVERY_CONTRACT_PROOF_OUTPUT) --allow-missing-evidence
	$(VENV_PYTHON) scripts/advise_source_product_evidence/generate_source_contract.py --capability mandate-restriction --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --advise-root $(LOTUS_ADVISE_ROOT) --output $(LOTUS_IDEA_MANDATE_RESTRICTION_SOURCE_PRODUCT_PROOF_OUTPUT)
	$(VENV_PYTHON) scripts/advise_source_product_evidence/generate_source_contract.py --capability missing-risk-profile --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --advise-root $(LOTUS_ADVISE_ROOT) --output $(LOTUS_IDEA_MISSING_RISK_PROFILE_SOURCE_PRODUCT_PROOF_OUTPUT)
	$(VENV_PYTHON) scripts/opportunity_archetype_evidence_pack/generate_evidence_pack.py --generated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --output $(LOTUS_IDEA_OPPORTUNITY_ARCHETYPE_EVIDENCE_PACK_OUTPUT)
	$(VENV_PYTHON) scripts/generate_implementation_proof_readiness.py --evaluated-at-utc $(IMPLEMENTATION_PROOF_EVALUATED_AT_UTC) --source-ingestion-manifest docs/examples/source-ingestion/high-cash-worker-manifest.example.json --output $(IMPLEMENTATION_PROOF_OUTPUT) $(if $(LOTUS_CORE_BASE_URL),--core-base-url $(LOTUS_CORE_BASE_URL),) $(if $(LOTUS_CORE_QUERY_BASE_URL),--core-query-base-url $(LOTUS_CORE_QUERY_BASE_URL),) $(if $(LOTUS_CORE_QUERY_CONTROL_PLANE_BASE_URL),--core-query-control-plane-base-url $(LOTUS_CORE_QUERY_CONTROL_PLANE_BASE_URL),) $(if $(LOTUS_IDEA_SOURCE_INGESTION_RUNTIME_EXECUTION),--source-ingestion-runtime-execution $(LOTUS_IDEA_SOURCE_INGESTION_RUNTIME_EXECUTION),) $(if $(LOTUS_IDEA_AI_LINEAGE_STORE_PROOF),--ai-lineage-store-proof $(LOTUS_IDEA_AI_LINEAGE_STORE_PROOF),--ai-lineage-store-proof $(LOTUS_IDEA_AI_LINEAGE_STORE_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_AI_MODEL_RISK_OPERATIONS_PROOF),--ai-model-risk-operations-proof $(LOTUS_IDEA_AI_MODEL_RISK_OPERATIONS_PROOF),--ai-model-risk-operations-proof $(LOTUS_IDEA_AI_MODEL_RISK_OPERATIONS_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_OPERATOR_WORKFLOWS_OPERATIONS_PROOF),--operator-workflows-operations-proof $(LOTUS_IDEA_OPERATOR_WORKFLOWS_OPERATIONS_PROOF),--operator-workflows-operations-proof $(LOTUS_IDEA_OPERATOR_WORKFLOWS_OPERATIONS_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_AI_WORKFLOW_PACK_REGISTRATION_PROOF),--ai-workflow-pack-registration-proof $(LOTUS_IDEA_AI_WORKFLOW_PACK_REGISTRATION_PROOF),--ai-workflow-pack-registration-proof $(LOTUS_IDEA_AI_WORKFLOW_PACK_REGISTRATION_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_AI_WORKFLOW_PACK_RUNTIME_EXECUTION_PROOF),--ai-workflow-pack-runtime-execution-proof $(LOTUS_IDEA_AI_WORKFLOW_PACK_RUNTIME_EXECUTION_PROOF),--ai-workflow-pack-runtime-execution-proof $(LOTUS_IDEA_AI_WORKFLOW_PACK_RUNTIME_EXECUTION_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_ADVISE_ROUTE_SOURCE_CONTRACT_PROOF),--advise-proposal-route-source-contract-proof $(LOTUS_IDEA_ADVISE_ROUTE_SOURCE_CONTRACT_PROOF),--advise-proposal-route-source-contract-proof $(LOTUS_IDEA_ADVISE_ROUTE_SOURCE_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_ADVISE_INTAKE_RUNTIME_EXECUTION_PROOF),--advise-intake-runtime-execution-proof $(LOTUS_IDEA_ADVISE_INTAKE_RUNTIME_EXECUTION_PROOF),--advise-intake-runtime-execution-proof $(LOTUS_IDEA_ADVISE_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_MANAGE_ROUTE_SOURCE_CONTRACT_PROOF),--manage-action-route-source-contract-proof $(LOTUS_IDEA_MANAGE_ROUTE_SOURCE_CONTRACT_PROOF),--manage-action-route-source-contract-proof $(LOTUS_IDEA_MANAGE_ROUTE_SOURCE_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_MANAGE_INTAKE_RUNTIME_EXECUTION_PROOF),--manage-intake-runtime-execution-proof $(LOTUS_IDEA_MANAGE_INTAKE_RUNTIME_EXECUTION_PROOF),--manage-intake-runtime-execution-proof $(LOTUS_IDEA_MANAGE_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_REPORT_INTAKE_ROUTE_SOURCE_CONTRACT_PROOF),--report-intake-route-source-contract-proof $(LOTUS_IDEA_REPORT_INTAKE_ROUTE_SOURCE_CONTRACT_PROOF),--report-intake-route-source-contract-proof $(LOTUS_IDEA_REPORT_INTAKE_ROUTE_SOURCE_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF),--report-intake-runtime-execution-proof $(LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF),--report-intake-runtime-execution-proof $(LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_REPORT_MATERIALIZATION_SOURCE_CONTRACT_PROOF),--report-materialization-source-contract-proof $(LOTUS_IDEA_REPORT_MATERIALIZATION_SOURCE_CONTRACT_PROOF),--report-materialization-source-contract-proof $(LOTUS_IDEA_REPORT_MATERIALIZATION_SOURCE_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_REPORT_MATERIALIZATION_RUNTIME_EXECUTION_PROOF),--report-materialization-runtime-execution-proof $(LOTUS_IDEA_REPORT_MATERIALIZATION_RUNTIME_EXECUTION_PROOF),--report-materialization-runtime-execution-proof $(LOTUS_IDEA_REPORT_MATERIALIZATION_RUNTIME_EXECUTION_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_MESH_POLICY_SOURCE_CONTRACT_PROOF),--mesh-policy-source-contract-proof $(LOTUS_IDEA_MESH_POLICY_SOURCE_CONTRACT_PROOF),--mesh-policy-source-contract-proof $(LOTUS_IDEA_MESH_POLICY_SOURCE_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_PLATFORM_CATALOG_SOURCE_CONTRACT_PROOF),--platform-catalog-source-contract-proof $(LOTUS_IDEA_PLATFORM_CATALOG_SOURCE_CONTRACT_PROOF),--platform-catalog-source-contract-proof $(LOTUS_IDEA_PLATFORM_CATALOG_SOURCE_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_OUTBOX_CONSUMER_CONTRACT_PROOF),--outbox-consumer-contract-proof $(LOTUS_IDEA_OUTBOX_CONSUMER_CONTRACT_PROOF),--outbox-consumer-contract-proof $(LOTUS_IDEA_OUTBOX_CONSUMER_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_OUTBOX_BROKER_RUNTIME_EXECUTION_PROOF),--outbox-broker-runtime-execution-proof $(LOTUS_IDEA_OUTBOX_BROKER_RUNTIME_EXECUTION_PROOF),) $(if $(LOTUS_IDEA_OUTBOX_CONSUMER_RUNTIME_EXECUTION_PROOF),--outbox-consumer-runtime-execution-proof $(LOTUS_IDEA_OUTBOX_CONSUMER_RUNTIME_EXECUTION_PROOF),--outbox-consumer-runtime-execution-proof $(LOTUS_IDEA_OUTBOX_CONSUMER_RUNTIME_EXECUTION_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_OUTBOX_PLATFORM_MESH_EVENT_SOURCE_CONTRACT_PROOF),--outbox-platform-mesh-event-source-contract-proof $(LOTUS_IDEA_OUTBOX_PLATFORM_MESH_EVENT_SOURCE_CONTRACT_PROOF),--outbox-platform-mesh-event-source-contract-proof $(LOTUS_IDEA_OUTBOX_PLATFORM_MESH_EVENT_SOURCE_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_GATEWAY_WORKBENCH_CONTRACT_PROOF),--gateway-workbench-contract-proof $(LOTUS_IDEA_GATEWAY_WORKBENCH_CONTRACT_PROOF),--gateway-workbench-contract-proof $(LOTUS_IDEA_GATEWAY_WORKBENCH_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_GATEWAY_WORKBENCH_DISCOVERY_CONTRACT_PROOF),--gateway-workbench-discovery-contract-proof $(LOTUS_IDEA_GATEWAY_WORKBENCH_DISCOVERY_CONTRACT_PROOF),--gateway-workbench-discovery-contract-proof $(LOTUS_IDEA_GATEWAY_WORKBENCH_DISCOVERY_CONTRACT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_GATEWAY_WORKBENCH_RUNTIME_EXECUTION_PROOF),--gateway-workbench-runtime-execution-proof $(LOTUS_IDEA_GATEWAY_WORKBENCH_RUNTIME_EXECUTION_PROOF),) $(if $(LOTUS_IDEA_RISK_CONCENTRATION_LIVE_PROOF),--risk-concentration-live-proof $(LOTUS_IDEA_RISK_CONCENTRATION_LIVE_PROOF),) $(if $(LOTUS_IDEA_HIGH_VOLATILITY_LIVE_PROOF),--high-volatility-live-proof $(LOTUS_IDEA_HIGH_VOLATILITY_LIVE_PROOF),) $(if $(LOTUS_IDEA_RISK_DRAWDOWN_LIVE_PROOF),--risk-drawdown-live-proof $(LOTUS_IDEA_RISK_DRAWDOWN_LIVE_PROOF),) $(if $(LOTUS_IDEA_PERFORMANCE_UNDERPERFORMANCE_LIVE_PROOF),--performance-underperformance-live-proof $(LOTUS_IDEA_PERFORMANCE_UNDERPERFORMANCE_LIVE_PROOF),) $(if $(LOTUS_IDEA_MISSING_BENCHMARK_PERFORMANCE_READINESS_PROOF),--missing-benchmark-performance-readiness-proof $(LOTUS_IDEA_MISSING_BENCHMARK_PERFORMANCE_READINESS_PROOF),) $(if $(LOTUS_IDEA_CORE_BENCHMARK_ASSIGNMENT_LIVE_PROOF),--core-benchmark-assignment-live-proof $(LOTUS_IDEA_CORE_BENCHMARK_ASSIGNMENT_LIVE_PROOF),) $(if $(LOTUS_IDEA_CORE_PORTFOLIO_STATE_LIVE_PROOF),--core-portfolio-state-live-proof $(LOTUS_IDEA_CORE_PORTFOLIO_STATE_LIVE_PROOF),) $(if $(LOTUS_IDEA_BOND_MATURITY_LIVE_PROOF),--bond-maturity-live-proof $(LOTUS_IDEA_BOND_MATURITY_LIVE_PROOF),) $(if $(LOTUS_IDEA_MISSING_BENCHMARK_LIVE_PROOF),--missing-benchmark-live-proof $(LOTUS_IDEA_MISSING_BENCHMARK_LIVE_PROOF),) $(if $(LOTUS_IDEA_LOW_INCOME_CORE_CASHFLOW_LIVE_PROOF),--low-income-core-cashflow-live-proof $(LOTUS_IDEA_LOW_INCOME_CORE_CASHFLOW_LIVE_PROOF),) $(if $(LOTUS_IDEA_MANAGE_MANDATE_LIVE_PROOF),--manage-mandate-live-proof $(LOTUS_IDEA_MANAGE_MANDATE_LIVE_PROOF),) $(if $(LOTUS_IDEA_MANDATE_RESTRICTION_LIVE_PROOF),--mandate-restriction-live-proof $(LOTUS_IDEA_MANDATE_RESTRICTION_LIVE_PROOF),) $(if $(LOTUS_IDEA_MANDATE_RESTRICTION_SOURCE_PRODUCT_PROOF),--mandate-restriction-source-product-proof $(LOTUS_IDEA_MANDATE_RESTRICTION_SOURCE_PRODUCT_PROOF),--mandate-restriction-source-product-proof $(LOTUS_IDEA_MANDATE_RESTRICTION_SOURCE_PRODUCT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_MISSING_SUITABILITY_LIVE_PROOF),--missing-suitability-live-proof $(LOTUS_IDEA_MISSING_SUITABILITY_LIVE_PROOF),) $(if $(LOTUS_IDEA_MISSING_RISK_PROFILE_SOURCE_PRODUCT_PROOF),--missing-risk-profile-source-product-proof $(LOTUS_IDEA_MISSING_RISK_PROFILE_SOURCE_PRODUCT_PROOF),--missing-risk-profile-source-product-proof $(LOTUS_IDEA_MISSING_RISK_PROFILE_SOURCE_PRODUCT_PROOF_OUTPUT)) $(if $(LOTUS_IDEA_MISSING_RISK_PROFILE_LIVE_PROOF),--missing-risk-profile-live-proof $(LOTUS_IDEA_MISSING_RISK_PROFILE_LIVE_PROOF),) --source-ingestion-scheduled-worker-source-contract output/source-ingestion/scheduled-worker-source-contract.json $(if $(LOTUS_IDEA_SOURCE_INGESTION_SCHEDULED_WORKER_DEPLOYMENT_EVIDENCE),--source-ingestion-scheduled-worker-deployment-evidence $(LOTUS_IDEA_SOURCE_INGESTION_SCHEDULED_WORKER_DEPLOYMENT_EVIDENCE),) --durable-repository-proof output/persistence/durable-repository-proof.json --runtime-trust-telemetry-test-execution output/trust-telemetry/test-execution/runtime-trust-telemetry-test-execution.json --workbench-read-path-source-contract-proof output/workbench/read-path-source-contract-proof.json --outbox-broker-source-contract-proof output/outbox/broker/source-contract-proof.json --opportunity-archetype-evidence-pack $(LOTUS_IDEA_OPPORTUNITY_ARCHETYPE_EVIDENCE_PACK_OUTPUT)

runtime-trust-telemetry-preview-check:
	$(VENV_PYTHON) scripts/runtime_trust_telemetry/generate_preview.py --generated-at-utc 2026-06-21T10:10:00Z --source-safe-local-exercise

runtime-trust-telemetry-snapshot-check:
	$(VENV_PYTHON) scripts/runtime_trust_telemetry/generate_snapshot.py --generated-at-utc 2026-06-21T10:10:00Z --source-safe-local-exercise
...
postgres-integration-gate:
	$(VENV_PYTHON) -m pytest tests/integration/test_postgres_runtime_integration.py tests/integration/test_postgres_source_revision_authority_runtime.py tests/integration/test_postgres_review_decision_fence_runtime.py tests/integration/test_postgres_conversion_intent_fence_runtime.py tests/integration/test_postgres_opportunity_effectiveness_runtime.py tests/integration/test_postgres_presentation_receipts_runtime.py tests/integration/test_postgres_candidate_expiry_runtime.py tests/integration/persistence/test_deployment_migrations_runtime.py tests/integration/persistence/test_candidate_persistence_runtime.py tests/integration/persistence/test_feedback_taxonomy_migration_runtime.py tests/integration/persistence/test_exact_review_authority_migration_runtime.py tests/integration/persistence/test_tenant_scoped_downstream_submission_migration_runtime.py tests/integration/persistence/test_tenant_scoped_mutation_idempotency_migration_runtime.py tests/integration/persistence/test_tenant_scoped_mutation_idempotency_runtime.py tests/integration/persistence/test_tenant_scoped_presentation_receipt_migration_runtime.py tests/integration/outbox/test_postgres_recovery_runtime.py tests/integration/test_postgres_review_queue_runtime.py tests/integration/test_postgres_downstream_submission_runtime.py tests/integration/test_postgres_report_materialization_reconciliation.py tests/integration/test_report_lost_response_postgres_chain.py tests/integration/test_postgres_manage_realization_reconciliation.py tests/integration/test_postgres_data_lifecycle_runtime.py tests/integration/source_runtime_evidence/test_postgres_replay.py --junitxml="$(POSTGRES_RUNTIME_PROOF_JUNIT)"
