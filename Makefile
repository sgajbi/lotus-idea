...
DOWNSTREAM_CAPACITY_CANDIDATE_ID ?=
DOWNSTREAM_CAPACITY_TENANT_ID ?=
DOWNSTREAM_CAPACITY_BOOK_ID ?=
DOWNSTREAM_CAPACITY_PORTFOLIO_ID ?=
DOWNSTREAM_CAPACITY_CLIENT_ID ?=
DOWNSTREAM_CAPACITY_ACCEPTED_NOT_BEFORE_UTC ?=
DOWNSTREAM_CAPACITY_RESOURCE_OUTPUT ?= output/observability/downstream-capacity-resource.json
...
service-capacity-workload:
	$(VENV_PYTHON) scripts/run_service_capacity_workload.py --base-url "$(SERVICE_CAPACITY_BASE_URL)" --environment-profile "$(SERVICE_CAPACITY_PROFILE)" $(SERVICE_CAPACITY_SCENARIO_ARGS) --request-count "$(SERVICE_CAPACITY_REQUEST_COUNT)" --concurrency "$(SERVICE_CAPACITY_CONCURRENCY)" --commit-sha "$(BUILD_GIT_COMMIT_SHA)" --branch "$(BUILD_GIT_BRANCH)" --run-id "$(BUILD_CI_RUN_ID)" $(SERVICE_CAPACITY_POSTGRES_THRESHOLD_PROOF_ARG) $(SERVICE_CAPACITY_DEPENDENCY_RECOVERY_PROOF_ARG) $(SERVICE_CAPACITY_LOAD_SOAK_PROOF_ARG) $(SERVICE_CAPACITY_DOWNSTREAM_RESOURCE_ARG) $(SERVICE_CAPACITY_RESOURCE_BASELINE_ARG) $(SERVICE_CAPACITY_COST_ATTRIBUTION_ARG) --output "$(SERVICE_CAPACITY_OUTPUT)"

downstream-capacity-resource:
	$(VENV_PYTHON) scripts/select_downstream_capacity_resource.py --base-url "$(SERVICE_CAPACITY_BASE_URL)" --candidate-id "$(DOWNSTREAM_CAPACITY_CANDIDATE_ID)" --tenant-id "$(DOWNSTREAM_CAPACITY_TENANT_ID)" --book-id "$(DOWNSTREAM_CAPACITY_BOOK_ID)" --portfolio-id "$(DOWNSTREAM_CAPACITY_PORTFOLIO_ID)" --client-id "$(DOWNSTREAM_CAPACITY_CLIENT_ID)" --accepted-not-before-utc "$(DOWNSTREAM_CAPACITY_ACCEPTED_NOT_BEFORE_UTC)" --commit-sha "$(BUILD_GIT_COMMIT_SHA)" --branch "$(BUILD_GIT_BRANCH)" --run-id "$(BUILD_CI_RUN_ID)" --output "$(DOWNSTREAM_CAPACITY_RESOURCE_OUTPUT)"
...
postgres-integration-gate:
	$(VENV_PYTHON) scripts/postgres_integration_gate.py --junitxml="$(POSTGRES_RUNTIME_PROOF_JUNIT)"
