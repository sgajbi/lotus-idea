...
HIGH_VOLATILITY_LIVE_PROOF_ENV = "LOTUS_IDEA_HIGH_VOLATILITY_LIVE_PROOF"
HIGH_VOLATILITY_LIVE_PROOF_SCHEMA_VERSION = "lotus-idea.high-volatility.live-proof.v1"
...
HIGH_VOLATILITY_LIVE_PROOF_EVIDENCE_REFS = (
...
def build_high_volatility_live_proof_payload(
    *,
    ...
    return {
        "schemaVersion": HIGH_VOLATILITY_LIVE_PROOF_SCHEMA_VERSION,
        "repository": "lotus-idea",
        "proofFamily": "high-volatility",
        ...
        "proofClosed": False,
        "aggregateBlockersCleared": list(HIGH_VOLATILITY_LIVE_BLOCKERS_CLEARED),
        "proofBlockers": proof_blockers,
        "remainingCertificationBlockers": list(REMAINING_CERTIFICATION_BLOCKERS),
        "evidenceRefs": list(HIGH_VOLATILITY_LIVE_PROOF_EVIDENCE_REFS),
        "nonProofBoundaries": list(NON_PROOF_BOUNDARIES),
    }


def high_volatility_live_proof_is_valid(payload: Mapping[str, Any]) -> bool:
    return (
        payload.get("schemaVersion") == HIGH_VOLATILITY_LIVE_PROOF_SCHEMA_VERSION
        and payload.get("repository") == "lotus-idea"
        and payload.get("proofFamily") == "high-volatility"
        and payload.get("sourceAuthority") == "lotus-risk"
        and payload.get("sourceProductId") == "lotus-risk:RiskMetricsReport:v1"
        ...
        and "live_risk_volatility_source_proof_missing"
...
def _proof_blockers(
    *,
    ...
    if not live_risk_source_attempted:
        blockers.append("live_risk_volatility_source_proof_missing")
    if _run_status(evaluation_summary) != "completed":
        blockers.append("live_risk_volatility_source_run_blocked")
    ...
    if not bool(evaluation_summary.get("sourceEvidenceCurrent")):
        blockers.append("risk_volatility_source_evidence_not_current")
    if not bool(evaluation_summary.get("riskSupportabilityReady")):
        blockers.append("risk_volatility_supportability_not_ready")
    if not _candidate_generated(_text(evaluation_summary.get("evaluationOutcome"), "")):
        blockers.append("no_high_volatility_candidate_generated")
