...
RISK_CONCENTRATION_LIVE_PROOF_ENV = "LOTUS_IDEA_RISK_CONCENTRATION_LIVE_PROOF"
RISK_CONCENTRATION_LIVE_PROOF_SCHEMA_VERSION = "lotus-idea.risk-concentration.live-proof.v1"

RISK_CONCENTRATION_LIVE_BLOCKERS_CLEARED = ("opportunity_archetype_live_risk_source_proof_missing",)
...
RISK_CONCENTRATION_LIVE_PROOF_EVIDENCE_REFS = (
...
def build_risk_concentration_live_proof_payload(
    *,
    ...
    return {
        "schemaVersion": RISK_CONCENTRATION_LIVE_PROOF_SCHEMA_VERSION,
        "repository": "lotus-idea",
        "proofFamily": "risk-concentration",
        ...
        "proofClosed": False,
        "aggregateBlockersCleared": list(RISK_CONCENTRATION_LIVE_BLOCKERS_CLEARED),
        "proofBlockers": proof_blockers,
        "remainingCertificationBlockers": list(REMAINING_CERTIFICATION_BLOCKERS),
        "evidenceRefs": list(RISK_CONCENTRATION_LIVE_PROOF_EVIDENCE_REFS),
        "nonProofBoundaries": list(NON_PROOF_BOUNDARIES),
    }


def risk_concentration_live_proof_is_valid(payload: Mapping[str, Any]) -> bool:
    return (
        payload.get("schemaVersion") == RISK_CONCENTRATION_LIVE_PROOF_SCHEMA_VERSION
        and payload.get("repository") == "lotus-idea"
        and payload.get("proofFamily") == "risk-concentration"
        and payload.get("sourceAuthority") == "lotus-risk"
        and payload.get("sourceProductId") == "lotus-risk:ConcentrationRiskReport:v1"
        ...
        and "live_risk_source_proof_missing" not in _text_sequence(payload.get("proofBlockers"))
...
def _proof_blockers(
    *,
    ...
    if not live_risk_source_attempted:
        blockers.append("live_risk_source_proof_missing")
    if _run_status(evaluation_summary) != "completed":
        blockers.append("live_risk_source_run_blocked")
    ...
    if not bool(evaluation_summary.get("sourceEvidenceCurrent")):
        blockers.append("risk_source_evidence_not_current")
    if not _candidate_generated(_text(evaluation_summary.get("evaluationOutcome"), "")):
        blockers.append("no_concentration_candidate_generated")
