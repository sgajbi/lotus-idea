    ...
    return tuple(
        {
            "repository": "lotus-idea",
            "ref": ref,
            "sha256": _sha256(repository_root / ref),
        }
...
def _source_authority_is_valid(value: object) -> bool:
    if not isinstance(value, (list, tuple)) or len(value) != len(MESH_POLICY_SOURCE_AUTHORITY_REFS):
        return False
    for item, expected_ref in zip(value, MESH_POLICY_SOURCE_AUTHORITY_REFS, strict=True):
        if not isinstance(item, Mapping) or set(item) != _SOURCE_AUTHORITY_FIELDS:
            return False
        if item.get("repository") != "lotus-idea" or item.get("ref") != expected_ref:
            return False
