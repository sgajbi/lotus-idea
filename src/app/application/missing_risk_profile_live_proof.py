...
MISSING_RISK_PROFILE_LIVE_PROOF_ENV = "LOTUS_IDEA_MISSING_RISK_PROFILE_LIVE_PROOF"
MISSING_RISK_PROFILE_LIVE_PROOF_SCHEMA_VERSION = "lotus-idea.missing-risk-profile.live-proof.v1"
...
MISSING_RISK_PROFILE_LIVE_PROOF_EVIDENCE_REFS = (
...
NON_PROOF_BOUNDARIES = (
    ...
    "no_typed_risk_profile_source_product_certification",
...
def build_missing_risk_profile_live_proof_payload(
    *,
    ...
    return {
        "schemaVersion": MISSING_RISK_PROFILE_LIVE_PROOF_SCHEMA_VERSION,
        "repository": "lotus-idea",
        "proofFamily": "missing-risk-profile-review",
        ...
        "adviseRiskProfileDiagnosticReady": _risk_profile_diagnostic_ready(diagnostic_codes),
        "sourceDiagnosticCodes": diagnostic_codes,
        ...
        "proofClosed": False,
        "aggregateBlockersCleared": list(MISSING_RISK_PROFILE_LIVE_BLOCKERS_CLEARED),
        "proofBlockers": proof_blockers,
        "remainingCertificationBlockers": list(REMAINING_CERTIFICATION_BLOCKERS),
        "evidenceRefs": list(MISSING_RISK_PROFILE_LIVE_PROOF_EVIDENCE_REFS),
        "nonProofBoundaries": list(NON_PROOF_BOUNDARIES),
    }


def missing_risk_profile_live_proof_is_valid(payload: Mapping[str, Any]) -> bool:
    return (
        payload.get("schemaVersion") == MISSING_RISK_PROFILE_LIVE_PROOF_SCHEMA_VERSION
        and payload.get("repository") == "lotus-idea"
        and payload.get("proofFamily") == "missing-risk-profile-review"
        ...
        and payload.get("adviseRiskProfileDiagnosticReady") is True
        ...
        and payload.get("typedRiskProfileSourceProductCertified") is False
...
def _proof_blockers(
    *,
    ...
    if not live_advise_source_attempted:
        blockers.append("advise_risk_profile_live_source_proof_missing")
    if _run_status(evaluation_summary) != "completed":
        blockers.append("advise_risk_profile_source_run_blocked")
    ...
    if not bool(evaluation_summary.get("sourceEvidenceCurrent")):
        blockers.append("advise_risk_profile_source_evidence_not_current")
    if not _risk_profile_diagnostic_ready(diagnostic_codes):
        blockers.append("advise_risk_profile_diagnostic_not_ready")
    if not _candidate_generated(_text(evaluation_summary.get("evaluationOutcome"), "")):
        blockers.append("no_missing_risk_profile_candidate_generated")
...
def _risk_profile_diagnostic_ready(diagnostic_codes: list[str]) -> bool:
    normalized = {
        code.strip().lower()
        for raw_code in diagnostic_codes
        for token in raw_code.replace(";", ",").replace("|", ",").split(",")
        for code in token.split()
        if code.strip()
    }
    return bool(normalized & RISK_PROFILE_DIAGNOSTIC_CODES)
