...
from app.application.report.intake_route_source_contract import REPORT_INTAKE_ROUTE
...
REPORT_INTAKE_RUNTIME_EXECUTION_ENV = "LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF"
REPORT_INTAKE_RUNTIME_EXECUTION_SCHEMA_VERSION = "lotus-idea.report-intake.runtime-execution.v1"
REPORT_INTAKE_RUNTIME_BLOCKERS_SATISFIED = ("lotus_report_live_intake_route_proof_missing",)
...
REPORT_INTAKE_RUNTIME_EVIDENCE_REFS = (
    ...
    "src/app/application/report/intake_runtime_execution.py",
...
_RECEIPT_DIGEST_FIELDS = tuple(field for field in _RECEIPT_FIELDS if field != "receiptDigest")
_RUNTIME_CHECK_FIELDS = frozenset(
    {
        ...
        "renderClaimDeniedObserved",
        "materializationAuthorityRetained",
...
_EXPECTED_PROOF_TYPE = "lotus_report_idea_evidence_pack_intake_runtime_execution"
_EXPECTED_PROOF_SCOPE = "report_intake_route_serving_and_receipt_behavior"
_EXPECTED_SOURCE_REPOSITORY = "lotus-idea"
_EXPECTED_DOWNSTREAM_AUTHORITY = "lotus-report"


def build_report_intake_runtime_execution_payload(
    *,
...
def report_intake_runtime_execution_is_valid(payload: Mapping[str, Any]) -> bool:
    ...
    if not is_timezone_aware_datetime_text(payload.get("generatedAtUtc")):
        return False
    if tuple(payload.get("evidenceRefs") or ()) != REPORT_INTAKE_RUNTIME_EVIDENCE_REFS:
        return False
    ...
    if not (
        ...
        and all(runtime_checks.get(field) is True for field in _RUNTIME_CHECK_FIELDS)
    ):
        return False
    ...
    return isinstance(receipt_evidence, Mapping) and (
        ...
        and _accepted_receipt_is_valid(receipt_evidence.get("acceptedReplay"))
        ...
        and _render_claim_denied_receipt_is_valid(receipt_evidence.get("renderClaimDenied"))
    )


def load_report_intake_runtime_execution_from_env() -> tuple[dict[str, Any] | None, str | None]:
    path_value = os.getenv(REPORT_INTAKE_RUNTIME_EXECUTION_ENV)
    if not path_value:
        return None, None
    path = Path(path_value)
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError(f"{REPORT_INTAKE_RUNTIME_EXECUTION_ENV} must reference a JSON object")
    try:
        artifact_ref = path.resolve().relative_to(Path.cwd().resolve()).as_posix()
    except ValueError:
        artifact_ref = f"{REPORT_INTAKE_RUNTIME_EXECUTION_ENV} artifact"
    return payload, artifact_ref


def source_safe_report_intake_receipt_digest(receipt: Mapping[str, Any]) -> str:
...
def _runtime_checks(
    *,
    ...
    return {
        ...
        "acceptedReplayReceiptObserved": _accepted_receipt_is_valid(
        ...
        "renderClaimDeniedObserved": _render_claim_denied_receipt_is_valid(
            receipt_evidence.get("renderClaimDenied")
        ),
...
def _render_claim_denied_receipt_is_valid(value: object) -> bool:
    return _receipt_matches(
        value,
        status_code=422,
        intake_status=None,
...
def _receipt_matches(
    ...
    *,
    ...
    return (
        ...
        and value.get("routeExistenceProven") is route_existence_proven
        ...
        and _is_sha256_digest(value.get("receiptDigest"))
    )


def _source_authority(report_root: Path) -> tuple[dict[str, str | None], ...]:
    return build_source_authority_records(_source_authority_sources(report_root))


def _source_authority_is_valid(value: object) -> bool:
    return source_authority_records_are_valid(
        value,
        expected_sources=_source_authority_sources(Path("../lotus-report")),
    )


def _source_authority_sources(report_root: Path) -> Sequence[SourceAuthoritySource]:
    return tuple(
        SourceAuthoritySource(
            repository="lotus-report",
            ref=f"../lotus-report/{ref}",
            path=report_root / ref,
        )
...
def _non_proof_claims_are_retained(value: object) -> bool:
    return (
        ...
        and all(value.get(field) is False for field in _NON_PROOF_CLAIM_FIELDS)
    )
