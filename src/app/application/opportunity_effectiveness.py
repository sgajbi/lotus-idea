    ...
    *,
    ...
    try:
        ranking_quality, ranking_stability = build_ranking_quality(
            summary.ranking_presentation_facts
        )
    except RankingQualityDataError as exc:
...
def _in_memory_repository_summary(
    ...
    *,
    ...
    return OpportunityEffectivenessRepositorySummary(
        ...
        ranking_presentation_facts=_build_ranking_presentation_facts(
            snapshot, records=records, evaluated_at_utc=evaluated_at_utc
        ),
    )
...
def _presentation_evidence_hash(
    ...
    try:
        return presentation_evidence_hash(record, receipt)
    except RankingQualityDataError as exc:
        raise OpportunityEffectivenessDataError(str(exc)) from exc


def _build_ranking_presentation_facts(
    snapshot: IdeaRepositorySnapshot,
    *,
    records: tuple[CandidatePersistenceRecord, ...],
    evaluated_at_utc: datetime,
    ...
    try:
        return build_ranking_presentation_facts(
            snapshot,
            records=records,
            evaluated_at_utc=evaluated_at_utc,
        )
        ...
        raise OpportunityEffectivenessDataError(str(exc)) from exc
    except RankingQualityBoundExceeded as exc:
        raise OpportunityEffectivenessBoundExceeded(str(exc)) from exc
...
def _snapshot_payload_without_digest(
    ...
    return {
        ...
        "presentation": {
            ...
            "rankingQuality": ranking_quality_payload(
                snapshot.ranking_quality,
                snapshot.ranking_stability,
            ),
        },
        "dimensions": {
