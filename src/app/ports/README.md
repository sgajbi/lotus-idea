...
1. `idea_repository.py` defines the central repository workflow protocols for
   candidate snapshots, candidate persistence, lifecycle mutation, review and
   feedback mutation, conversion mutation, report evidence-pack requests, and
   AI explanation reads. Application use cases must depend on these protocols
   instead of declaring local repository protocols, so the future durable
   repository adapter has one governed contract surface.
