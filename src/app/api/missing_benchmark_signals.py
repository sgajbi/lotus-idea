...
_EVALUATE_MISSING_BENCHMARK_POLICY = CapabilityPolicy.for_roles(
...
async def evaluate_missing_benchmark_signal(
    ...
    source_authority = _benchmark_assignment_source_authority(request)
    try:
        require_capability(caller, _EVALUATE_MISSING_BENCHMARK_POLICY)
...
def _benchmark_assignment_source_authority(request: EvaluateMissingBenchmarkSignalRequest) -> str:
    if request.benchmark_assignment_ref is None:
        return "source-owned"
    return request.benchmark_assignment_ref.source_system.value
