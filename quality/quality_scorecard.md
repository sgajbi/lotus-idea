...
| --- | --- | --- | --- | --- |
| Architecture | `Partially implemented` | Layered package skeleton, blocking architecture-boundary gate, blocking maintainability thresholds, and report-only architecture evidence. | Service-specific boundaries not yet implemented. | Replace scaffold placeholders with real module map and ownership truth. |
...
| CI and release evidence | `Partially implemented` | Feature, PR merge, main releasability workflows enforce lint, typecheck, architecture boundaries, maintainability thresholds, documentation contract posture, OpenAPI, supported features, endpoint certification, security, tests, coverage, PostgreSQL runtime proof, Docker, least-privilege workflow permissions, bounded job timeouts, and no soft-failed critical jobs. | Broad quality metrics beyond deterministic size thresholds and documentation surface contracts remain report-only until stable thresholds are justified. | Promote additional deterministic gates after measured baseline. |
| Documentation and operations | `Partially implemented` | README, repo context, wiki, runbooks, standards, RFC index, quality scorecard, and RFC evidence guide are protected by `make documentation-contract-gate`; current-state overclaims are protected by `make implementation-truth-gate`. | Operator docs still need deeper service-specific runtime evidence as live source, Gateway/Workbench, downstream realization, and mesh certification slices land. | Keep docs/wiki/context updated with implementation-backed truth in every RFC slice. |
