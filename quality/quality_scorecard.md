...
| CI and release evidence | `Partially implemented` | Feature, PR merge, main releasability workflows enforce lint, typecheck, architecture boundaries, maintainability thresholds, OpenAPI, supported features, endpoint certification, security, tests, coverage, PostgreSQL runtime proof, Docker, least-privilege workflow permissions, bounded job timeouts, and no soft-failed critical jobs. | Broad quality metrics beyond deterministic size thresholds remain report-only until stable thresholds are justified. | Promote additional deterministic gates after measured baseline. |
