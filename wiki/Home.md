...
| Next proof path | Resolve current mainline-source blockers first: Core PR #948 must merge, the issue-backed Gateway #549 branch must be promoted or cleanly parked with preserved branch evidence, and this Idea PR must merge so all canonical repositories return to clean exact `origin/main`. Then rerun the governed `PB_SG_GLOBAL_BAL_001` canonical proof for Gateway-backed Workbench queue/detail/action behavior and close only issues with issue-specific proof. |
