...
17. `tests/unit/report/test_materialization_runtime_execution.py` and
    ...
    valid current artifact binds source-safe Report materialization receipts,
    but its Render and Archive collaborators are injected test doubles. It clears
    only `report_evidence_pack_live_materialization_proof_missing` and retains
    `rendered_output_creation_missing` plus `archive_record_creation_missing`
    until a rebuilt current-source HTTP chain reaches both owner services under
    `#1240`. Client-publication, production-identity, legal/retention, support,
    final-certification, and supported-feature blockers also remain.
...
maps every non-complete legacy-ledger issue and selected related GitHub product
issues to at least one learning
...
is 242 label-backed RFC-0002 issues across 13 repositories: 204 closed and 38
...
synchronization tracker, #1154 is closed after PR #1157 exact-main, release-image,
wiki, and branch-hygiene proof for governed economic candidate identity, and
#1155 then #1156 are ready for feedback-quality evaluation and
...
artifacts. PR #845 is a historical recursion-stop example, recorded in the
