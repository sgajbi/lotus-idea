...
| Opportunity archetypes | A governed archetype/scenario contract identifies high cash / idle liquidity as the first partially implemented journey, records concentration risk review, underperformance review, allocation drift / mandate review, and high-volatility review as non-promoted bounded foundations, keeps remaining archetypes planned, and is visible as blocked scenario readiness in aggregate proof readiness. Valid source-safe live proof artifacts can clear only the high-cash live Core blocker, concentration live Risk blocker, high-volatility live Risk volatility blocker, and underperformance live Performance blocker where proof contracts exist. Underperformance currently consumes source-reported active return and benchmark context from `lotus-performance:ReturnsSeriesBundle:v1`, but benchmark-assignment source refs remain blocked. Allocation drift currently consumes `lotus-manage:PortfolioActionRegister:v1` action-register supportability posture but blocks the store-wide Manage summary from becoming a portfolio-scoped mandate-review opportunity until portfolio-scoped Manage, mandate performance-health, mandate risk-health, and Core portfolio-state source refs are proven. High volatility currently consumes Risk-owned `RiskMetricsReport:v1` volatility only; drawdown source proof, data-mesh, Workbench, and supported-feature blockers remain. | Use as taxonomy, source-authority framing, and internal foundation proof only; do not present it as full Workbench journey proof, client-demo proof, or a supported feature. |
