...
flowchart LR
    ...
    Open["166 closed and 38 open<br/>25 `status/blocked`, 0 `status/fixed-local`, 1 `status/in-progress`, 1 `status/merged-main`, 2 `status/merged-to-main`, 1 `status/pr-open`, 8 `status/tracker`"]
...
| `status/in-progress` | 1 | Active Idea hardening issue `sgajbi/lotus-idea#1101`. |
...
active for AI workflow evaluator hardening; `lotus-idea#1098` is
