...
| Active synchronization tracker | `sgajbi/lotus-idea#681`; PR-open Slice 19 refactor `sgajbi/lotus-idea#1040`; merged-main QA-pending Slice 19 refactor `sgajbi/lotus-idea#1037` |
...
flowchart LR
    ...
    Open["144 closed and 39 open<br/>25 `status/blocked`, 1 `status/in-progress`, 2 `status/merged-main`, 2 `status/merged-to-main`, 1 `status/pr-open`, 8 `status/tracker`"]
...
| `status/pr-open` | 1 | Pull request is open and awaiting PR checks/merge, currently `sgajbi/lotus-idea#1040`. |
