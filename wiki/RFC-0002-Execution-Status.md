...
```mermaid
flowchart LR
    Issues["GitHub issue posture<br/>188 label-backed RFC-0002 issues"]
    Open["151 closed and 37 open<br/>25 `status/blocked`, 0 `status/fixed-local`, 1 `status/in-progress`, 1 `status/merged-main`, 2 `status/merged-to-main`, 0 `status/pr-open`, 8 `status/tracker`"]
