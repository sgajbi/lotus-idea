...
flowchart LR
    ...
    Open["164 closed and 38 open<br/>25 `status/blocked`, 0 `status/fixed-local`, 1 `status/in-progress`, 1 `status/merged-main`, 2 `status/merged-to-main`, 1 `status/pr-open`, 8 `status/tracker`"]
...
| `status/pr-open` | 1 | Open PR issue: `sgajbi/lotus-idea#1094` through PR #1095. |
...
PR-open through PR #1095 for test-support maintainability; `lotus-idea#1091` is
