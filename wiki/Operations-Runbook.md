...
optional receipt-bound Core portfolio-state runtime evidence through
`LOTUS_IDEA_CORE_PORTFOLIO_STATE_LIVE_PROOF`,
optional receipt-bound Core bond-maturity runtime evidence through
...
contract through `LOTUS_MANAGE_ROOT` and
`LOTUS_IDEA_MANAGE_ROUTE_SOURCE_CONTRACT_PROOF_OUTPUT`, default Report intake route and
...
When Advise, Manage, and Report runtime receipts exist, operators can generate
the bounded domain-consumer runtime proof with
`scripts/outbox/generate_consumer_runtime_execution.py` and validate it with
`make outbox-consumer-runtime-execution-proof-gate`. That artifact may clear
only `downstream_consumer_runtime_proof_missing`; Gateway/Workbench runtime
proof, platform-mesh event publication, supported-feature promotion, and
production certification remain open until their owning evidence exists.
...
client-ready publication, or supported-feature promotion.
Aggregate implementation-proof readiness can also consume a digest-bound
`lotus-advise` route source contract generated by
`scripts/downstream_realization/generate_advise_route_source_contract.py` and a bounded
`lotus-manage` route source contract generated by
`scripts/downstream_realization/generate_manage_route_source_contract.py`, both guarded by
`make downstream-route-source-contract-proof-gate`. Those artifacts bind
sibling contract, route, and service declarations by repository/ref and
SHA-256, add supporting provenance, and clear no blocker.
`advise_live_contract_proof_missing` and `manage_live_contract_proof_missing`
remain until governed runtime serving-and-acceptance receipts exist. The
artifacts do not prove authorization, tenant isolation, request execution,
downstream records, suitability, policy approval, mandate/rebalance authority,
...
`lotus-advise` intake runtime-execution artifact generated by
`scripts/downstream_realization/generate_advise_intake_runtime_execution.py`
and guarded by `make advise-intake-runtime-execution-proof-gate`. The v2
artifact requires one accepted response and one exact replay from an identical
concurrent pair, plus an exact owner-history readback bound through source-safe
source-intent, trusted-scope, and owner-identity digests and the exact evidence
fingerprint, status, and version. It does not certify timeout, restart, correction,
suitability, proposal creation, production identity, or client publication.

Aggregate implementation-proof readiness can also consume a bounded
`lotus-manage` action-intake runtime-execution artifact generated by
`scripts/downstream_realization/generate_manage_intake_runtime_execution.py`
and guarded by `make manage-intake-runtime-execution-proof-gate`. That artifact
is `runtime_execution` evidence over accepted, replayed, rejected,
idempotency-conflict, authorization-denied, and tenant-scoped idempotency
receipts from `POST /api/v1/rebalance/idea-action-intake`. It clears only
`manage_live_contract_proof_missing` when aggregate-current, while preserving
action-register persistence, rebalance/execution, order/OMS, client
publication, production identity, support, and supported-feature blockers.
Aggregate implementation-proof readiness can also consume a bounded
...
clears no blocker. Runtime serving and acceptance evidence is generated by
`scripts/report/generate_intake_runtime_execution.py` and guarded by
`make report-intake-runtime-execution-proof-gate`; it may clear only
`lotus_report_live_intake_route_proof_missing`. It does not create a report
job, render output, archive record, client-ready publication, production
identity, or supported feature.
Aggregate implementation-proof readiness can also consume a bounded
...
use a bounded candidate-and-latest-review projection, including authoritative
persisted snoozes, instead of hydrating unrelated repository state. Active
snoozes are counted and excluded until their exact wake-up boundary. It remains
`not_certified` until
...
local downstream submission denominator, reconciliation workload, and planned Advise/Manage/Report handoff contract
...
live blocker, and the submission/reconciliation workload is local Idea posture only, so
...
| Downstream realization | Readiness diagnostics, durable claim-before-call submission, opaque uncertain-work inspection, audited operator reconciliation, real PostgreSQL concurrency/restart proof, source-safe adapters, and digest-bound route source contracts that preserve all live blockers | Authoritative downstream outcome, route serving or acceptance, automatic uncertain-call retry, suitability, mandate/rebalance authority, execution, order creation, client publication, or supported-feature promotion |
...
| Upstream runtime and source-product proof | `make risk-concentration-live-proof-contract-gate`, `make high-volatility-live-proof-contract-gate`, `make risk-drawdown-live-proof-contract-gate`, `make bond-maturity-live-proof-contract-gate`, `make core-benchmark-assignment-live-proof-contract-gate`, `make core-portfolio-state-live-proof-contract-gate`, `make manage-mandate-live-proof-contract-gate`, `make mandate-restriction-live-proof-contract-gate`, `make missing-suitability-live-proof-contract-gate`, `make missing-risk-profile-live-proof-contract-gate`, `make performance-underperformance-live-proof-contract-gate` | Validate bounded sibling evidence. Risk, Performance, and bond-maturity runtime evidence binds current authority receipts to durable Idea persistence. Other Core and Advise read-only evidence binds pseudonymous request scope to exact producer receipts without inventing source identity or persistence. Advise mandate/restriction, missing-suitability, and missing-risk-profile gates accept truthful candidate and no-opportunity results but fail closed without producer-owned scope/time/hashes/posture. Command existence never promotes support. |
...
| Downstream proof | `make downstream-route-source-contract-proof-gate`, `make report-intake-route-source-contract-proof-gate`, `make report-intake-runtime-execution-proof-gate`, `make report-materialization-source-contract-proof-gate`, `make report-materialization-runtime-execution-proof-gate` | Validate bounded Advise, Manage, and Report contract/runtime artifacts. |
...
a bounded aggregate over candidates and their latest applicable persisted
review decision; process-local repositories derive the same snooze posture from
their immutable review history.
...
Typed Advise source-product artifacts are generated by
`scripts/advise_source_product_evidence/generate_source_contract.py` with
`--capability mandate-restriction` or `--capability missing-risk-profile`.
Both require the sibling Advise declaration and trust-telemetry sources,
preserve blocked producer posture, and clear only their named typed-contract
blocker.
...
endpoint can consume digest-bound Advise, Manage, and Report route source
contracts. All of these contracts add provenance but clear no blocker and do
not prove route serving, acceptance, report-job execution,
