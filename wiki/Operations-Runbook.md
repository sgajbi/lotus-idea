...
| Outbox delivery foundation | Source-safe records, retryable failure status, published status, dead-letter status, HTTP publisher adapter foundation, repo-owned outbox event and downstream consumer contracts, aggregate readiness diagnostic, bounded run-once operator action, bounded outbox broker proof artifact, and bounded downstream consumer runtime proof artifact | Certified external publication, platform mesh event publication proof, downstream delivery, Gateway/Workbench behavior, or supported-feature promotion |
