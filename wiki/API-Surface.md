...
| Bounded source-backed signal evaluation | `POST /api/v1/idea-signals/high-cash/evaluate-from-source` | Fetches Core-owned high-cash evidence through the configured Core source adapter after advisor role, `idea.signal.evaluate`, and portfolio entitlement checks pass. It returns source-redacted candidate or blocked posture and closes the runtime client after each request. | High-cash only; no persistence, source-worker certification, live source certification, data-product certification, Gateway/Workbench support, client publication, or supported-feature promotion. |
