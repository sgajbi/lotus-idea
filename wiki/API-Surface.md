...
Planned-contract readiness includes source-safe claim-before-call submission. Submission requires complete trusted tenant, book, portfolio, and client entitlement scope covering the persisted source candidate; missing or mismatched scope fails closed before claim or dispatch. The claim fingerprint binds that complete scope, and the Advise intake envelope carries the governed candidate `portfolio_id` without exposing client identity. Readiness includes the local downstream submission denominator and unresolved reconciliation workload.
