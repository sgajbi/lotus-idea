...
| Review workflow | `GET /api/v1/review-queues/advisor`, `GET /api/v1/review-queues/advisor/readiness`, `POST /api/v1/idea-candidates/{candidateId}/review-actions`, `POST /api/v1/idea-candidates/{candidateId}/feedback` | Source-safe queue/readiness plus review and feedback capture. Queue reads use candidate-created-at as-of visibility and opaque snapshot-bound continuation paging. Queue `policyVersion` is distinct from candidate `scorePolicyVersion`; unknown score policies fail closed and produce an aggregate readiness blocker. Mutations bind trusted caller entitlements to persisted candidate scope. | Advisor queue/detail have bounded read-only Gateway publication; Workbench proof, data-product certification, and supported review-product promotion remain blocked. |
