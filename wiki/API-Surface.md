...
| Bounded source-backed signal evaluation | `POST /api/v1/idea-signals/high-cash/evaluate-from-source`, `POST /api/v1/idea-signals/low-income/evaluate-from-source`, `POST /api/v1/idea-signals/bond-maturity/evaluate-from-source`, `POST /api/v1/idea-signals/missing-benchmark/evaluate-from-source`, `POST /api/v1/idea-signals/concentration-risk/evaluate-from-source`, `POST /api/v1/idea-signals/high-volatility/evaluate-from-source`, `POST /api/v1/idea-signals/drawdown-review/evaluate-from-source` | Fetches Core-owned high-cash, low-income, bond-maturity, or benchmark-assignment evidence, or Lotus Risk-owned concentration, volatility, or drawdown evidence, through the configured source adapter after advisor role, `idea.signal.evaluate`, and portfolio entitlement checks pass. It returns source-redacted candidate or blocked posture and closes the runtime client after each request. | High-cash, low-income, bond-maturity, missing-benchmark, concentration-risk, high-volatility, and drawdown-review only; no persistence, source-worker certification, live source certification, income-needs assessment, funding advice, treasury instruction, maturity schedule authority, replacement product recommendation, reinvestment advice, planning suitability approval, benchmark assignment, benchmark methodology authority, concentration calculation, volatility/VaR/tracking-error calculation, drawdown calculation, risk methodology approval, trade recommendation, rebalance action, data-product certification, Gateway/Workbench support, client publication, or supported-feature promotion. |
