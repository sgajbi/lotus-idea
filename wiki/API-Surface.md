...
| Bounded source-backed signal evaluation | `POST /api/v1/idea-signals/high-cash/evaluate-from-source`, `POST /api/v1/idea-signals/low-income/evaluate-from-source` | Fetches Core-owned high-cash or low-income evidence through the configured Core source adapter after advisor role, `idea.signal.evaluate`, and portfolio entitlement checks pass. It returns source-redacted candidate or blocked posture and closes the runtime client after each request. | High-cash and low-income only; no persistence, source-worker certification, live source certification, income-needs assessment, funding advice, treasury instruction, planning suitability approval, data-product certification, Gateway/Workbench support, client publication, or supported-feature promotion. |
