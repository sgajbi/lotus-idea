...
| Signal caller context | Caller-supplied signal routes bind standard caller context through `app.api.caller_headers.CallerContextHeaders`. `make signal-api-contract-gate` blocks route-local `X-Caller-*` header clones, duplicate signal permission policy, weak 400/403 examples, and operation-event drift. |
