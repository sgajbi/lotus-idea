...
Planned-contract readiness includes source-safe claim-before-call submission. Submission requires complete trusted tenant, book, portfolio, and client entitlement scope covering the persisted source candidate; missing or mismatched scope fails closed before claim or dispatch. Durable claim identity is `(tenant_id, idempotency_key)`: reuse in one tenant replays or conflicts against the complete request fingerprint, while another tenant may reuse the ordinary caller key under a distinct opaque support reference. The Advise intake envelope carries the governed candidate `portfolio_id` without exposing client identity. Readiness includes the local downstream submission denominator and unresolved reconciliation workload.
