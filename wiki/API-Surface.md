...
Audience-bound business queues, candidate-safe operator exception posture, readiness, review/feedback capture, and immutable visible-render evidence. Candidate-detail, business queue, operator exception-queue, and candidate evidence-replay reads require complete trusted tenant/book/portfolio/client scope before repository access; query filters may narrow that scope but cannot create or widen authority, and a generic operator role grants no estate-wide view. Queue reads use candidate-created-at as-of visibility and audience-bound opaque continuation snapshots. Each queue item includes current Idea-owned material/evidence versions for receipt binding. Workbench review must cite the exact tenant/candidate receipt and exact material/evidence/packet/hash identity; operator review is explicit and cannot carry a fabricated receipt. Malformed evidence digests are `400 invalid_request` before authority or persistence logic runs, while a well-formed non-matching digest remains an authority conflict. The historical `legacy:unknown` revision-vector sentinel remains structurally admissible but grants no current source authority. A presentation receipt's body names its tenant and the trusted caller entitlement set authorizes membership, including plural sets; header order never selects the tenant or durable key. Recording also requires complete trusted tenant/book/portfolio/client scope that authorizes the persisted candidate before candidate lookup, idempotency, or the write; a missing or mismatching dimension is refused with no receipt or effectiveness mutation. The persisted decision retains policy, actor, presentation digest, applicability, and evidence identity. Queue retrieval is never treated as presentation. Feedback remains governed separately by `idea-feedback-taxonomy-v1`. Gateway and Workbench must preserve these contracts unchanged; canonical consumer certification remains open. See [Feedback Evaluation](Feedback-Evaluation) and [Opportunity Effectiveness](Opportunity-Effectiveness).
...
Internal exact-review-gated intent, source-versioned append-only outcome history/current posture, and report evidence-pack request recording. A new conversion requires the exact approving review ID and current material/evidence/packet/hash identity. Idea verifies the immutable grant remains active and unexpired at trusted server acceptance time, then repeats the check under the candidate mutation lock before intent/outbox creation. Conversion outcomes and report evidence-pack requests require complete trusted tenant/book/portfolio/client entitlements matching the persisted candidate before idempotency precheck or mutation. Exact replay of an accepted intent remains replayable after expiry; mutable approved posture and legacy-unverified history are insufficient. All mutations publish named accepted and replayed OpenAPI modes that return the exact persisted action. Report evidence-pack examples preserve no client-publication, rendered-output, or archive-record authority.
...
- `POST /api/v1/downstream-submissions/{supportReference}/report-materialization-reconciliation`
- `GET /api/v1/downstream-submissions/reconciliation`
- `POST /api/v1/downstream-submissions/reconciliation/{supportReference}`

**Current use**

Planned-contract readiness includes source-safe claim-before-call submission. Submission requires complete trusted tenant, book, portfolio, and client entitlement scope covering the persisted source candidate; missing or mismatched scope fails closed before claim or dispatch. A first Advise or Manage attempt also revalidates current candidate evidence, the exact active review grant, target lifecycle, owner authority, applicability, and authoritative source-cut posture. Stale, expired, revoked, target-drifted, or `legacy:unknown` authority returns `409 conversion_intent_authority_conflict` before claim or owner I/O. Caller replay identity is `(tenant_id, idempotency_key)`, while `(tenant_id, resource_type, resource_id, target)` permits exactly one local submission aggregate for governed owner work. A different key for an already claimed resource returns `409 downstream_submission_resource_conflict` without another owner call or support reference. Another tenant may reuse the ordinary caller key. An already admitted attempt remains exactly replayable or read-only reconcilable after later authority expiry; retained evidence is never rewritten to create authority. The Advise intake envelope carries the governed candidate `portfolio_id` without exposing client identity. Readiness includes the local downstream submission denominator and unresolved reconciliation workload.

Both submission routes publish accepted, rejected, accepted-replayed, and rejected-replayed `200` modes, with `reconciliation_required` separately published as `202`; exact submission retries never make another adapter call. Timeouts, transport ambiguity, 5xx responses, malformed responses, lease loss, and local finalization failure become durable reconciliation posture. Operator routes expose opaque inspection and audited accepted/rejected/quarantined resolution.

Advise receipts are persisted for accepted-for-review and rejected-before-work outcomes. Manage receipts bind the durable `management_action_id`, source event version, and owner request fingerprint from the accepted intake; a rejected Manage intake creates no durable action, so it carries the owner reason with no receipt. Both event-history reconciliation routes require `idea.downstream-realization.reconcile`, authorize complete scope before owner I/O, validate the exact typed owner history, and persist only an exact replay or append-only extension. Their appended count is the delta committed by the serialized mutation, so exact or concurrent replays report zero. The Manage history follows the owner machine verbatim: review status is not absorbing (`REQUEST_CHANGES` reopens approved and rejected reviews), so monotonicity is enforced on append-only contiguous event versions. An approved review is never rebalance-execution, order, or client-publication evidence.

Report receipts bind the exact evidence-pack identity and fingerprint to the Report-owned request/job, carry a positive Report lifecycle version, preserve current materialization and Render/Archive creation posture, and retain explicit publication and promotion blockers. Each explicit reconciliation performs the scoped, read-only owner lookup. An unchanged version is a non-mutating replay; a higher version advances the stored receipt transactionally; a lower version or same-version mutation fails closed. Materialization is never resubmitted. A legacy unversioned receipt remains explicit unknown until current owner evidence supplies a version; Idea never invents owner chronology.

For Advise, Manage, and Report recovery, exact owner 404 is published as a
target-specific 409 `owner_acceptance_not_observed` problem and preserves the
uncertain record. Owner outage remains 503. Neither posture authorizes a
repeated owner POST; only a later exact read can advance the same submission.
...
| Candidate identity | `idea-opportunity-identity-v3` derives tenant-scoped economic business identity independently from source content hashes, exact observation date, and transport keys. Equivalent evidence/date refreshes retain the business/material version while incrementing evidence version; material change and recurrent conditions create explicit governed transitions. A one-way v2 backfill preserves existing lifecycle posture, evaluation DTOs reject `duplicateOfCandidateId`, and persistence owns duplicate reconciliation. Review queue/readiness projections preserve distinct business identities that share `sourceSignalIds`; `duplicate` exclusion means explicit adviser suppression, not inferred source-lineage similarity. |
