...
| Bounded source-backed signal evaluation | `POST /api/v1/idea-signals/high-cash/evaluate-from-source`, `POST /api/v1/idea-signals/low-income/evaluate-from-source`, `POST /api/v1/idea-signals/missing-benchmark/evaluate-from-source` | Fetches Core-owned high-cash, low-income, or benchmark-assignment evidence through the configured Core source adapter after advisor role, `idea.signal.evaluate`, and portfolio entitlement checks pass. It returns source-redacted candidate or blocked posture and closes the runtime client after each request. | High-cash, low-income, and missing-benchmark only; no persistence, source-worker certification, live source certification, income-needs assessment, funding advice, treasury instruction, planning suitability approval, benchmark assignment, benchmark methodology authority, data-product certification, Gateway/Workbench support, client publication, or supported-feature promotion. |
