...
| AI explanation governance | `POST /api/v1/idea-candidates/{candidateId}/ai-explanations/evaluate`, `GET /api/v1/ai-explanations/readiness` | Deterministic fallback and model-risk diagnostics. `lotus-idea.ai-metadata-envelope.v1` accepts only purpose-scoped, code-owned operational routing values; unknown fields or values fail before candidate lookup and lineage stores field names only. Local/test may evaluate explicitly unattested governed fixtures; production-like profiles reject workflow output with `ai_execution_provenance_required` until signed Lotus AI provenance exists. Output integrity, API idempotency, and request-id replay/conflict remain enforced. | No provider call, autonomous advice, runtime proof, or client-ready explanation claim. |
