...
| Engineering and agents | 181 label-backed RFC-0002 issues are tracked across 13 repositories: 144 closed and 37 open, with 25 `status/blocked`, 1 `status/in-progress`, 1 `status/merged-main`, 2 `status/merged-to-main`, 8 `status/tracker`, and 0 app-actionable blocked issues. Open blocked work remains external/protected/canonical-proof; `sgajbi/lotus-idea#1033` is QA-closed and `sgajbi/lotus-idea#681` remains in progress. | Fix writable blockers when discovered, keep GitHub state durable, and rerun canonical QA before closure. |
