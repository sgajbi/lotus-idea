...
| Engineering and agents | 199 label-backed RFC-0002 issues are tracked across 13 repositories: 162 closed and 37 open, with 25 `status/blocked`, 0 `status/fixed-local`, 1 `status/in-progress`, 1 `status/merged-main`, 2 `status/merged-to-main`, 0 `status/pr-open`, 8 `status/tracker`, and 0 app-actionable blocked issues. The Idea source ledger tracks 130 RFC-0002 issues: 105 closed and 25 open after `sgajbi/lotus-idea#1084` reached exact main and QA closure evidence; PR #1086 synchronized the current source/docs/wiki posture to main. | Fix writable blockers when discovered, keep GitHub state durable, and rerun canonical QA before closure. |
