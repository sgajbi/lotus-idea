...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        risk_source = LotusRiskVolatilitySourceAdapter(
            DownstreamJsonClient(
                DownstreamClientConfig(
        ...
        print(f"high volatility live proof configuration error: {exc}", file=sys.stderr)
        return 2
...
def _write_blocked_source_proof(*, args: argparse.Namespace, error_code: str) -> int:
    try:
        ...
        print(f"high volatility live proof error: {exc}", file=sys.stderr)
        return 2
    print(f"high volatility live proof blocked: {error_code}", file=sys.stderr)
    return 3


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate source-safe live Lotus Risk volatility proof for lotus-idea."
    )
