...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        evaluation = evaluate_missing_suitability_context_signal_from_advise(
            EvaluateMissingSuitabilityContextFromAdviseCommand(
                evaluation_id=args.evaluation_id,
                as_of_date=as_of_date,
                evaluated_at_utc=evaluated_at_utc,
        ...
        proof_payload = build_missing_suitability_live_proof_payload(
        ...
        print(f"missing suitability live proof configuration error: {exc}", file=sys.stderr)
        return 2
...
def _write_blocked_source_proof(*, args: argparse.Namespace, error_code: str) -> int:
    try:
        ...
        print(f"missing suitability live proof error: {exc}", file=sys.stderr)
        return 2
    print(f"missing suitability live proof blocked: {error_code}", file=sys.stderr)
    return 3


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate source-safe live Lotus Advise missing suitability proof."
    )
    ...
    *,
    ...
    return {
        ...
        "clientReadyPublicationBlocked": (
            str(evidence.client_ready_publication or "").upper() == "BLOCKED"
        ),
        "advisePolicyWorkflowReady": evidence.policy_ref is not None,
