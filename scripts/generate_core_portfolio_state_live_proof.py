...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        proof_payload = build_core_portfolio_state_live_proof_payload(
        ...
        print(f"core portfolio-state live proof configuration error: {exc}", file=sys.stderr)
        return 2
...
def _write_blocked_source_proof(*, args: argparse.Namespace, error_code: str) -> int:
    try:
        ...
        print(f"core portfolio-state live proof error: {exc}", file=sys.stderr)
        return 2
    print(f"core portfolio-state live proof blocked: {error_code}", file=sys.stderr)
    return 3


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate source-safe live Core portfolio-state proof for lotus-idea."
    )
    parser.add_argument(
...
def _evidence_summary(evidence: CorePortfolioStateEvidence) -> dict[str, object]:
    return {
        "runStatus": "completed",
        "sourceAuthority": "lotus-core",
        "sourceProductId": "lotus-core:PortfolioStateSnapshot:v1",
        "portfolioStateRefPresent": evidence.portfolio_state_ref is not None,
        "sourceEvidenceCurrent": core_source_ref_is_current(evidence.portfolio_state_ref),
        "sourceEvidenceAvailable": evidence.source_evidence_available,
        "sourceDiagnosticCodes": (
            [evidence.portfolio_state_diagnostic] if evidence.portfolio_state_diagnostic else []
        ),
    }
...
def _source_error_code(exc: Exception) -> str:
    ...
    return str(code).strip() or "core_portfolio_state_source_unavailable"
