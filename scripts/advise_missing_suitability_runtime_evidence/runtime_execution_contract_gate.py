...
from tests.support.advise_missing_suitability_runtime_evidence import (  # noqa: E402
...
GENERATOR = (
    ROOT
    / "scripts"
    / "advise_missing_suitability_runtime_evidence"
    / "generate_runtime_execution.py"
)
PROHIBITED_PATHS = (
    ROOT / "src" / "app" / "application" / "missing_suitability_live_proof.py",
    ROOT / "scripts" / "generate_missing_suitability_live_proof.py",
    ROOT / "scripts" / "missing_suitability_live_proof_contract_gate.py",
    ROOT / "tests" / "unit" / "test_missing_suitability_live_proof.py",
)
FORBIDDEN_KEYS = {
    "bookId",
    "clientId",
...
def validate_advise_missing_suitability_runtime_execution_contract() -> list[str]:
    errors: list[str] = []
    if not GENERATOR.exists():
        errors.append("capability-owned Advise missing-suitability generator is required")
    for path in PROHIBITED_PATHS:
        if path.exists():
            errors.append(
                f"retired missing-suitability evidence path is prohibited: {path.relative_to(ROOT)}"
            )
    ...
    candidate = build_advise_missing_suitability_runtime_execution(
        generated_at_utc=NOW,
        result=result,
    )
    if not advise_missing_suitability_runtime_execution_is_valid(candidate):
        errors.append("authoritative Advise missing-suitability fixture must validate")
    ...
    no_opportunity = build_advise_missing_suitability_runtime_execution(
        generated_at_utc=NOW,
        result=evaluate_advise_missing_suitability(
            command,
            advise_source=AuthoritativeAdviseMissingSuitabilitySource(context_missing=False),
        ),
    )
    if not advise_missing_suitability_runtime_execution_is_valid(no_opportunity):
    ...
    missing_scope = build_advise_missing_suitability_runtime_execution(
        generated_at_utc=NOW,
        result=replace(
            result,
    ...
    if advise_missing_suitability_runtime_execution_is_valid(missing_scope):
    ...
    if advise_missing_suitability_runtime_execution_is_valid(tampered):
...
def _command() -> EvaluateAdviseMissingSuitability:
    return EvaluateAdviseMissingSuitability(
        tenant_id="tenant-a",
        book_id="book-a",
...
def main() -> int:
    errors = validate_advise_missing_suitability_runtime_execution_contract()
    if errors:
        print("\n".join(errors))
        return 1
    print("Advise missing-suitability runtime execution contract gate passed")
    return 0
