from __future__ import annotations

from datetime import UTC, datetime
import sys
from pathlib import Path

from app.application.risk_concentration_live_proof import (
    RISK_CONCENTRATION_LIVE_BLOCKERS_CLEARED,
    RISK_CONCENTRATION_LIVE_PROOF_SCHEMA_VERSION,
    build_risk_concentration_live_proof_payload,
    risk_concentration_live_proof_is_valid,
)


try:
    from scripts.proof_source_safety import forbidden_content_validator, validate_forbidden_content
except ModuleNotFoundError:
    from proof_source_safety import forbidden_content_validator, validate_forbidden_content  # type: ignore[import-not-found,no-redef]

ROOT = Path(__file__).resolve().parents[1]
LIVE_PROOF_SCRIPT = ROOT / "scripts" / "generate_risk_concentration_live_proof.py"

FORBIDDEN_KEYS = {
    "accountId",
    "candidateId",
    "clientId",
    "correlationId",
    "holdingId",
    "idempotencyKey",
    "portfolioId",
    "requestBody",
    "responseBody",
    "sourcePayload",
    "sourceRoute",
    "traceId",
    "transactionId",
}

FORBIDDEN_TEXT_FRAGMENTS = {
    "PB_SG_GLOBAL_BAL_001",
    "request-body",
    "response-body",
    "signal-ingestion:concentration",
}

REQUIRED_TOP_LEVEL_KEYS = {
    "schemaVersion",
    "repository",
    "proofFamily",
    "sourceAuthority",
    "sourceProductId",
    "generatedAtUtc",
    "liveRiskSourceAttempted",
    "runStatus",
    "evaluationOutcome",
    "candidateGenerated",
    "sourceEvidenceCurrent",
    "sourceDiagnosticCodes",
    "reasonCodes",
    "unsupportedReasons",
    "supportedFeaturePromoted",
    "proofClosed",
    "aggregateBlockersCleared",
    "proofBlockers",
    "remainingCertificationBlockers",
    "evidenceRefs",
    "nonProofBoundaries",
}


_validate_forbidden_content = forbidden_content_validator(
    FORBIDDEN_KEYS,
    FORBIDDEN_TEXT_FRAGMENTS,
)


def validate_risk_concentration_live_proof_contract() -> list[str]:
    errors: list[str] = []
    if not LIVE_PROOF_SCRIPT.exists():
        errors.append("scripts/generate_risk_concentration_live_proof.py is required")
        return errors

    payload = build_risk_concentration_live_proof_payload(
        generated_at_utc=datetime(2026, 6, 21, 10, 10, tzinfo=UTC),
        live_risk_source_attempted=True,
        evaluation_summary={
            "runStatus": "completed",
            "sourceAuthority": "lotus-risk",
            "sourceProductId": "lotus-risk:ConcentrationRiskReport:v1",
            "evaluationOutcome": "candidate_created",
            "sourceEvidenceCurrent": True,
            "sourceDiagnosticCodes": ["risk_issuer_coverage_complete"],
            "reasonCodes": ["concentration_attention"],
            "unsupportedReasons": [],
        },
    )

    if set(payload) != REQUIRED_TOP_LEVEL_KEYS:
        errors.append(
            "risk concentration live proof payload keys must be "
            f"{sorted(REQUIRED_TOP_LEVEL_KEYS)}; got {sorted(payload)}"
        )
    if payload.get("schemaVersion") != RISK_CONCENTRATION_LIVE_PROOF_SCHEMA_VERSION:
        errors.append(f"schemaVersion must be {RISK_CONCENTRATION_LIVE_PROOF_SCHEMA_VERSION}")
    if payload.get("supportedFeaturePromoted") is not False:
        errors.append("risk concentration live proof must not promote supported features")
    if payload.get("proofClosed") is not False:
        errors.append(
            "risk concentration proof must remain open while certification blockers remain"
        )
    if payload.get("aggregateBlockersCleared") != list(RISK_CONCENTRATION_LIVE_BLOCKERS_CLEARED):
        errors.append("risk concentration proof must clear only the live-risk-source blocker")
    for blocker in (
        "opportunity_archetype_data_mesh_not_certified",
        "opportunity_archetype_workbench_product_proof_missing",
        "opportunity_archetype_supported_feature_promotion_missing",
    ):
        if blocker not in payload.get("remainingCertificationBlockers", []):
            errors.append(f"risk concentration proof must retain remaining blocker `{blocker}`")
    if not risk_concentration_live_proof_is_valid(payload):
        errors.append("valid risk concentration proof fixture should validate")

    blocked_payload = build_risk_concentration_live_proof_payload(
        generated_at_utc=datetime(2026, 6, 21, 10, 10, tzinfo=UTC),
        live_risk_source_attempted=True,
        evaluation_summary={
            "runStatus": "blocked",
            "sourceAuthority": "lotus-risk",
            "sourceProductId": "lotus-risk:ConcentrationRiskReport:v1",
            "errorCode": "risk_source_unavailable",
            "sourceEvidenceCurrent": False,
            "evaluationOutcome": "blocked",
            "sourceDiagnosticCodes": ["risk_source_unavailable"],
            "reasonCodes": ["source_partial"],
            "unsupportedReasons": ["source_unavailable"],
        },
    )
    if risk_concentration_live_proof_is_valid(blocked_payload):
        errors.append("blocked risk concentration proof fixture must not validate")

    validate_forbidden_content(payload, errors, FORBIDDEN_KEYS, FORBIDDEN_TEXT_FRAGMENTS)
    validate_forbidden_content(blocked_payload, errors, FORBIDDEN_KEYS, FORBIDDEN_TEXT_FRAGMENTS)
    return errors


def main() -> int:
    errors = validate_risk_concentration_live_proof_contract()
    if errors:
        print("\n".join(errors))
        return 1
    print("Risk concentration live proof contract gate passed")
    return 0


if __name__ == "__main__":
    sys.exit(main())
