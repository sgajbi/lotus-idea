...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with closing(
            ...
            result = evaluate_advise_missing_risk_profile(command, advise_source=source)
        payload = build_advise_missing_risk_profile_runtime_execution(
            generated_at_utc=_parse_instant(args.generated_at_utc, "generated-at-utc"),
            result=result,
        )
        write_json_payload(payload, output=args.output)
        if advise_missing_risk_profile_runtime_execution_is_valid(payload):
            return 0
        blockers = payload["execution"]["qualificationBlockers"]
        print(
            f"Advise missing-risk-profile runtime evidence did not qualify: {','.join(blockers)}",
            file=sys.stderr,
        )
        return 3
    except (DownstreamClientConfigurationError, OSError, ValueError, json.JSONDecodeError) as exc:
        print(
            f"Advise missing-risk-profile runtime evidence configuration error: {exc}",
            file=sys.stderr,
        )
        return 2


def _command(args: argparse.Namespace) -> EvaluateAdviseMissingRiskProfile:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate receipt-bound Advise missing-risk-profile runtime evidence."
    )
