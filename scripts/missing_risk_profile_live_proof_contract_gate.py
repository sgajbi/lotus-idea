...
LIVE_PROOF_SCRIPT = ROOT / "scripts" / "generate_missing_risk_profile_live_proof.py"
...
REQUIRED_TOP_LEVEL_KEYS = {
    ...
    "adviseRiskProfileDiagnosticReady",
...
def validate_missing_risk_profile_live_proof_contract() -> list[str]:
    errors: list[str] = []
    if not LIVE_PROOF_SCRIPT.exists():
        errors.append("scripts/generate_missing_risk_profile_live_proof.py is required")
        return errors
    ...
    if set(payload) != REQUIRED_TOP_LEVEL_KEYS:
        errors.append(
            "missing risk-profile live proof payload keys must be "
            f"{sorted(REQUIRED_TOP_LEVEL_KEYS)}; got {sorted(payload)}"
        )
    if payload.get("schemaVersion") != MISSING_RISK_PROFILE_LIVE_PROOF_SCHEMA_VERSION:
        errors.append(f"schemaVersion must be {MISSING_RISK_PROFILE_LIVE_PROOF_SCHEMA_VERSION}")
    if payload.get("supportedFeaturePromoted") is not False:
        errors.append("missing risk-profile proof must not promote supported features")
    if payload.get("clientPublicationReady") is not False:
        errors.append("missing risk-profile proof must not approve client publication")
    for authority_key in (
        ...
        if payload.get(authority_key) is not False:
            errors.append(f"missing risk-profile proof must keep `{authority_key}` false")
    if payload.get("typedRiskProfileSourceProductCertified") is not False:
        errors.append(
            "missing risk-profile proof must not certify typed risk-profile source product"
        )
    if payload.get("proofClosed") is not False:
        errors.append("missing risk-profile proof must remain open while blockers remain")
    if payload.get("aggregateBlockersCleared") != list(MISSING_RISK_PROFILE_LIVE_BLOCKERS_CLEARED):
        errors.append("missing risk-profile proof must clear only the Advise live-source blocker")
    for blocker in (
        ...
        if blocker not in payload.get("remainingCertificationBlockers", []):
            errors.append(f"missing risk-profile proof must retain remaining blocker `{blocker}`")
    if not missing_risk_profile_live_proof_is_valid(payload):
        errors.append("valid missing risk-profile proof fixture should validate")

    blocked_payload = build_missing_risk_profile_live_proof_payload(
        generated_at_utc=datetime(2026, 6, 28, 10, 10, tzinfo=UTC),
        live_advise_source_attempted=True,
        evaluation_summary={
            "runStatus": "blocked",
    ...
    if missing_risk_profile_live_proof_is_valid(blocked_payload):
        errors.append("blocked missing risk-profile proof fixture must not validate")
...
def main() -> int:
    errors = validate_missing_risk_profile_live_proof_contract()
    if errors:
        print("\n".join(errors))
        return 1
    print("Missing risk-profile live proof contract gate passed")
    return 0
