...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if core_portfolio_state_runtime_execution_is_valid(payload):
            return 0
        print("Core portfolio-state runtime evidence did not qualify", file=sys.stderr)
        return 3
        ...
        print(f"Core portfolio-state runtime evidence configuration error: {exc}", file=sys.stderr)
        return 2
...
def _write_blocked(
    *,
    ...
    command: EvaluateCorePortfolioStateReadiness | None,
    error_code: str,
) -> int:
    try:
        ...
        print(f"Core portfolio-state runtime evidence error: {exc}", file=sys.stderr)
        return 2
    print(f"Core portfolio-state runtime evidence blocked: {error_code}", file=sys.stderr)
    return 3


def _command(args: argparse.Namespace) -> EvaluateCorePortfolioStateReadiness:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate receipt-bound Core portfolio-state runtime evidence."
    )
    parser.add_argument(
...
def _source_error_code(exc: Exception) -> str:
    ...
    return str(getattr(exc, "code", "")).strip() or "core_portfolio_state_source_unavailable"
