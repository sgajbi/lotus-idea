...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if bond_maturity_runtime_execution_is_valid(payload):
            return 0
        print("Bond-maturity runtime evidence did not qualify", file=sys.stderr)
        return 3
        ...
        print(f"Bond-maturity runtime evidence configuration error: {exc}", file=sys.stderr)
        return 2
...
def _write_blocked(
    *,
    ...
    command: EvaluateBondMaturityReadiness | None,
    error_code: str,
) -> int:
    try:
        ...
        print(f"Bond-maturity runtime evidence error: {exc}", file=sys.stderr)
        return 2
    print(f"Bond-maturity runtime evidence blocked: {error_code}", file=sys.stderr)
    return 3
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate receipt-bound Core bond-maturity runtime evidence."
    )
...
def _source_error_code(exc: Exception) -> str:
    ...
    return str(getattr(exc, "code", "")).strip() or "core_maturity_source_unavailable"
