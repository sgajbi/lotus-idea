...
ACTOR_SUBJECT = "lotus-idea-risk-drawdown-runtime-evidence"


def main(argv: list[str] | None = None) -> int:
    args = _parser().parse_args(argv)
    command: EvaluateAndPersistDrawdownReviewFromRiskCommand | None = None
    try:
        generated_at_utc = _parse_instant(args.generated_at_utc, "generated-at-utc")
        command = _command(args)
        repository = get_idea_repository()
        source = LotusRiskDrawdownSourceAdapter(
            DownstreamJsonClient(
                DownstreamClientConfig(
        ...
        if risk_drawdown_runtime_execution_is_valid(payload):
            return 0
        print("risk drawdown runtime evidence did not qualify", file=sys.stderr)
        return 3
        ...
        print(f"risk drawdown runtime evidence configuration error: {exc}", file=sys.stderr)
        return 2
...
def _write_blocked(
    *,
    ...
    command: EvaluateAndPersistDrawdownReviewFromRiskCommand | None,
    error_code: str,
) -> int:
    try:
        ...
        print(f"risk drawdown runtime evidence error: {exc}", file=sys.stderr)
        return 2
    print(f"risk drawdown runtime evidence blocked: {error_code}", file=sys.stderr)
    return 3


def _command(args: argparse.Namespace) -> EvaluateAndPersistDrawdownReviewFromRiskCommand:
    ...
    idempotency_key = str(args.idempotency_key or "").strip() or (
        f"runtime-evidence:risk-drawdown:{portfolio_id}:{as_of_date.isoformat()}:{period_name}"
    )
...
def _policy(args: argparse.Namespace) -> DrawdownReviewSignalPolicy:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate source-safe Lotus Risk drawdown runtime execution evidence."
    )
