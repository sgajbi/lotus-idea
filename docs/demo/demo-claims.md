...
The governed claim matrix lives at
`contracts/demo-readiness/lotus-idea-demo-readiness-claim-matrix.v1.json` and is
enforced by `make demo-readiness-claim-matrix-gate`. It deliberately keeps
`demo_ready`, `client_publication_ready`, `supported_feature_promoted`,
`data_mesh_certified`, `production_certified`, live AI provider certification,
production identity certification, full Workbench journey certification, and
downstream runtime certification false. The matrix is a no-overclaim control for
internal commercial enablement; it is not external client-demo approval.
...
| Opportunity archetype and scenario contract | `Partially implemented` | Governed archetype JSON, typed loader, deterministic policy/source tests, bounded signal APIs, capability-owned proof gates, contract-gated `blocker_issue_refs`, and a source-safe canonical archetype evidence pack that hashes the canonical portfolio reference while binding each archetype to source products, evidence refs, blocker issue refs, and readiness blockers. Advise missing-suitability, mandate/restriction, and missing-risk-profile now use closed v2 runtime-execution contracts over exact request, producer-workflow, and candidate/no-opportunity receipts; source-product proof remains separate. See [implementation proof readiness](../operations/implementation-proof-readiness.md) for the evidence inventory. | The canonical pack is supporting `source_contract` evidence only. Each runtime artifact clears only its namespaced source blocker. Issue-backed blocker traceability and pack composition are not demo evidence. Official source calculations and decisions, data-mesh certification, Workbench proof, client publication, deployment, production, and supported-feature promotion remain blocked. | Complete the remaining authority-specific proof issues, full journey replay, Workbench/downstream proof, mesh certification, and promotion evidence. |
| Demo-readiness claim matrix and commercial proof-pack guardrail | `Partially implemented` | `contracts/demo-readiness/lotus-idea-demo-readiness-claim-matrix.v1.json`, typed loader, `make demo-readiness-claim-matrix-gate`, and focused unit tests. | Internal enablement and no-overclaim control only; the commercial proof pack is not client-safe or RFP-safe for external distribution. Full live journey, Workbench/Gateway proof, downstream owner runtime outcomes, data-mesh certification, production identity/entitlement proof, supported-feature promotion, and client-publication approval remain blocked. | Use the matrix as the source for internal talk-track boundaries while implementing remaining Slice 16/17 proof issues. |
...
| Downstream realization readiness | `Partially implemented` | `GET /api/v1/downstream-realization/readiness`, `POST /api/v1/conversion-intents/{conversionIntentId}/downstream-submissions`, `POST /api/v1/report-evidence-packs/{reportEvidencePackId}/downstream-submissions`, source-safe downstream application orchestration and adapter foundations, local downstream submission denominator, unresolved downstream submission reconciliation workload, endpoint certification ledger, unit/integration tests, planned Advise/Manage/Report contract-readiness records, digest-bound Advise, Manage, and Report route source-contract consumption, Report materialization source-contract consumption, and `downstream_realization_readiness_read` / `downstream_realization_submission` operation events. | Operator diagnostic, local submission/reconciliation workload, submission posture, adapter foundation, and bounded route-declaration provenance only; source-contract evidence clears no blocker and does not prove route serving or acceptance, intake runtime, materialization execution, rendered output, archive creation, suitability or rebalance/execution authority, Gateway/Workbench behavior, client publication, or a supported feature. | Use it to drive downstream realization slices; do not use it as demo evidence until client publication, suitability/rebalance authority, product-surface, data-product, and supported-feature blockers are cleared by live cross-repo proof. |
...
## Current Commercial-Proof Boundary

The Slice 16 claim matrix permits only internal foundation walkthrough and
sales-enablement boundary training. It does not permit external client,
marketing, or RFP distribution. Every commercial statement must remain one of:

1. implementation-backed internal foundation,
2. bounded internal walkthrough,
3. blocked pending external proof, or
4. prohibited claim.

Writable Lotus app/platform repository gaps should be fixed forward rather than
classified as blocked. A blocker is reserved for non-writable or external
dependencies such as source-owner evidence outside the current write scope.
