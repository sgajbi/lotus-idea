...
| Report materialization runtime execution | `report_evidence_pack_live_materialization_proof_missing` only | The proof observes bounded local/dev Report materialization route execution and source-safe receipt outcomes. It does not prove rendered output creation, archive record creation, client-publication authority, retention/legal-hold posture, production identity, Workbench/Gateway behavior, or supported-feature promotion. |
