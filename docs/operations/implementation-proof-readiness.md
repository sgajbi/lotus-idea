...
also `open_blocked`, not QA-pending: it remains open for self-hosted runner
...
reconciled historical Manage #620 closure evidence. PR #772 then synchronized
the platform vulnerability-exception register linkage. The latest exact-main
...
wiki `DiffCount 0`, and local/remote branch cleanup passed. PR #789 classified
...
same non-clearing rule. The Report intake runtime-execution proof is the
separate blocker-clearing artifact for `POST /reports/idea-evidence-packs` and
may clear only `lotus_report_live_intake_route_proof_missing`. Source refs do
not clear runtime execution,
...
| `LOTUS_IDEA_LOW_INCOME_CORE_CASHFLOW_LIVE_PROOF` | Passes validated closed v3 Core cashflow `runtime_execution` evidence into opportunity-archetype readiness. It requires authoritative tenant/book/portfolio/client scope and binds exact movement/projection receipts plus an eligible evaluation to governed successful durable Idea persistence: accepted, exact-replayed, evidence-refreshed, material-version-created, or recurrent-condition-reopened. No-opportunity, non-durable, missing-persistence, incoherent, or tampered evidence clears no blocker. A release handoff that explicitly requires accepted-then-replayed proof must still record those two decisions. A valid artifact clears only `opportunity_archetype_live_core_cashflow_source_proof_missing`; it does not certify client income needs, advice, suitability, data mesh, Workbench, publication, deployment, production, or feature support. Historical v2 artifacts do not qualify. |
...
| `LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT` | Selects the default generated Report intake runtime-execution artifact consumed by aggregate readiness. Defaults to `output/report/intake-runtime-execution-proof.json`. |
| `LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF` | Overrides the default Report intake runtime-execution proof path. A valid aggregate-current artifact clears only `lotus_report_live_intake_route_proof_missing` and preserves materialization, render/archive, client-publication, production identity, support, and supported-feature blockers. |
...
| `LOTUS_IDEA_REPORT_MATERIALIZATION_RUNTIME_EXECUTION_PROOF` | Overrides the default local-ASGI Report materialization proof. The artifact clears only `report_evidence_pack_live_materialization_proof_missing`; injected Render/Archive test doubles cannot clear `rendered_output_creation_missing` or `archive_record_creation_missing`. Current-source owner-service HTTP evidence is required for those blockers. Client-publication, supported-feature promotion, production identity, legal/retention, support, and final certification blockers remain. |
...
`opportunity-archetype-scenarios` capability. The closed v3 application path
invokes the authoritative evaluate-and-persist use case and binds a
pseudonymous tenant/book/portfolio/client request receipt, exact
`lotus-core:PortfolioCashflowProjection:v1` and
`lotus-core:PortfolioCashMovementSummary:v1` receipts, projection arithmetic,
movement counts, policy threshold, source-revision posture, and successful
durable candidate persistence: accepted, exact-replayed, evidence-refreshed,
material-version-created, or recurrent-condition-reopened. Unknown, stale, degraded,
...
Accepted, material-version-created, and recurrent-condition-reopened receipts
must describe a newly generated candidate. Evidence-refreshed and exact-replayed
receipts instead preserve the candidate's persisted forward lifecycle state; the
contract accepts a governed persisted state from `generated` onward, rejects
`detected` and unknown states, and continues to bind the exact candidate and
audit evidence.
A release handoff that specifically requires accepted-then-replayed proof must
still record those two exact decisions; a versioned persistence decision proves
safe canonical materialization but does not substitute for that replay receipt.
...
report-owned intake route modules and tests, and the `lotus-idea` downstream
readiness consumers that keep runtime blockers visible.

Report intake runtime execution is captured by
`scripts/report/generate_intake_runtime_execution.py` and validated by
`make report-intake-runtime-execution-proof-gate`. The proof executes
`POST /reports/idea-evidence-packs` against a local `lotus-report` ASGI app with
an isolated intake ledger and records source-safe accepted, replayed, conflict,
missing-idempotency, client-publication-denied, and render-claim-denied
receipts. Aggregate readiness consumes it from
`LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT` unless
`LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF` is set. It clears only
...
1. repo-native check that generates and consumes the scheduled-worker source
   ...
   Report intake route source contract, Report intake runtime execution,
   Report materialization source contract, outbox broker
...
$env:LOTUS_IDEA_REPORT_INTAKE_RUNTIME_EXECUTION_PROOF_OUTPUT = "output/report/intake-runtime-execution-proof.json"
