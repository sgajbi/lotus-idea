...
| `GET /api/v1/source-ingestion/readiness` | Internal operator diagnostic for high-cash Core source-ingestion run-once worker configuration, optional live-proof artifact validity, and certification blockers. | `idea.source-ingestion.readiness.read` plus operator role | Diagnostic only; a valid proof artifact clears only the live-Core blocker. No scheduled worker deploy proof, data-product certification, platform source-manifest inclusion, Gateway/Workbench support, or supported-feature promotion. |
