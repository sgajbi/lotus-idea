...
| `POST /api/v1/report-evidence-packs/{reportEvidencePackId}/downstream-submissions` | Internal source-safe claim-before-call submission for an existing Report evidence-pack request. OpenAPI publishes accepted, rejected, accepted-replayed, and rejected-replayed `200` modes plus a separate `202 reconciliation_required` mode. Acceptance is returned only after local terminal state commits. | `idea.downstream-realization.submit` plus `Idempotency-Key` | Same-key retries never call downstream again after terminal or uncertain posture. No Report package intake proof, Render output, Archive record, authoritative outcome, Gateway, Workbench, data-product certification, client-ready publication, or supported-feature promotion. |
