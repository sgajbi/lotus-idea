...
| `POST /api/v1/idea-candidates/{candidateId}/ai-explanations/evaluate` | Internal governed AI explanation fallback/verifier evaluation over persisted candidate evidence with source-safe idempotent lineage recording. An attested result may include a separately signed provider-retention confirmation bound to candidate tenant plus verified run/provider/model identity. | `idea.ai-explanation.evaluate`, exact candidate tenant entitlement when scoped, plus `Idempotency-Key` | Local/test output is visibly `unattested_local_test_fixture`; production-like workflow output requires verified Lotus AI provenance. Signed provider-retention receipts are atomic, replay-fenced, and source-safe; provider failure is blocked and not deletion proof. Same-key replay returns persisted lineage after re-verification. No provider call, lifecycle authority, runtime certification, downstream authority, Gateway/Workbench surface, client-ready publication, or supported-feature promotion. |
...
| `GET /api/v1/source-ingestion/readiness` | Internal operator diagnostic for high-cash Core source-ingestion configuration, optional receipt-bound runtime-evidence validity, scheduled-worker deploy-proof validity, and certification blockers. | `idea.source-ingestion.readiness.read` plus operator role | Diagnostic only; valid current v2 runtime evidence affects only the live-Core posture, and a valid scheduled-worker artifact clears only its own blocker. No data-product, mesh, Gateway/Workbench, production, downstream, or supported-feature certification follows. |
