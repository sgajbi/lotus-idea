...
| `POST /api/v1/idea-signals/high-cash/evaluate-and-persist` | High-cash evaluation plus internal candidate persistence, idempotency, replay, and audit posture through the active repository provider. Named success modes distinguish accepted, replayed, duplicate-candidate, blocked and not-eligible behavior; non-candidate modes perform no write and return `persistence=null`. Missing economic scope fails before any durable mutation. | `idea.candidate.persist`, `Idempotency-Key`, and complete `accessScope` for tenant/book/portfolio/client economic identity | `local`/`test` profiles may write to process-local storage with pseudonymous governed scope; `demo`/`staging`/`production` require `LOTUS_IDEA_DATABASE_URL` and fail closed with `durable_repository_not_configured` before in-memory mutation. Evaluation-only diagnostics may remain unscoped, but no durable caller can bypass the repository scope invariant. Real PostgreSQL persistence/replay, source-ingestion replay/conflict recovery, and migration rollback/reapply recovery proof exists for this path and the first review/conversion/report workflow path, while production authentication/identity-provider authority, production storage certification, source-worker certification beyond bounded live proof, mesh certification, Gateway/Workbench proof, and supported-feature promotion remain planned. |
