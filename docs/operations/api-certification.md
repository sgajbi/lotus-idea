...
| `GET /api/v1/implementation-proof/readiness` | Internal aggregate operator diagnostic for RFC-0002 implementation proof blockers. | `idea.implementation-proof.readiness.read` plus operator role | Diagnostic only; no live implementation proof, data-product certification, platform source-manifest inclusion, Gateway/Workbench proof, client-ready publication, or supported-feature promotion. |
