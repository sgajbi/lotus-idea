...
| `GET /api/v1/review-queues/advisor` | Internal deterministic advisor queue over persisted candidates. `evaluatedAtUtc` is the inclusive candidate-created-at boundary; source/evidence dates retain source-authority meaning. | `idea.review.queue.read` plus advisor role | Queue `policyVersion` names the ranking policy; candidate `scorePolicyVersion` names score provenance. Unknown policies fail closed. Page size defaults to 25 and is capped at 100. The opaque `snapshotToken` binds scope, queue policy, accepted score-policy set, and visible state; changed state returns `409 review_queue_snapshot_conflict`. PostgreSQL uses the same policy-filtered bounded projection. Read-only Gateway publication exists; Workbench proof, data-product certification, broader queues, client publication, and supported-feature promotion remain out of scope. |
