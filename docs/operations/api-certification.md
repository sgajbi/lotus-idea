...
All signal operations also share `idea-source-temporal-v2` at the domain
...
with a new content hash preserves the business candidate and material version
while creating a new evidence packet and lineage version. No signal family
currently permits an inferred effective-date window.
...
18. high-cash caller and source-backed evaluation to publish candidate-created, blocked,
    and not-eligible HTTP 200 modes, and high-cash evaluate-and-persist to publish
    accepted, replayed, duplicate-candidate, blocked, and not-eligible modes from
...
19. low-income caller and Core-backed evaluation to publish candidate-created, blocked,
    and not-eligible HTTP 200 modes from application-backed, DTO-serialized
...
22. underperformance caller and Performance-backed evaluation to publish candidate-created,
    blocked, and not-eligible HTTP 200 modes from application-backed,
...
23. concentration-risk caller and Risk-backed evaluation to publish candidate-created, blocked,
    and not-eligible HTTP 200 modes from application-backed, DTO-serialized
    factories. Candidate examples must preserve the governed Risk
    `ConcentrationRiskReport:v1` identity while redacting source routes and content hashes. Idea
    owns opportunity detection and advisor-review posture, not concentration calculation, risk
    methodology, trade recommendation, rebalance, or execution authority.
24. high-volatility caller and Risk-backed evaluation to publish candidate-created, blocked,
    and not-eligible HTTP 200 modes from application-backed, DTO-serialized
    factories. Candidate examples must preserve the governed Risk `RiskMetricsReport:v1`
    identity while redacting source routes and content hashes. Idea owns opportunity detection
    and advisor-review posture, not volatility, VaR, or tracking-error calculation, risk
    methodology, trade recommendation, rebalance, or execution authority.
25. drawdown-review caller and Risk-backed evaluation to publish candidate-created, blocked,
    and not-eligible HTTP 200 modes from application-backed, DTO-serialized
    factories. Candidate examples must preserve the governed Risk
    `DrawdownAnalyticsReport:v1` identity while redacting source routes and content hashes.
    Idea owns opportunity detection and advisor-review posture, not drawdown calculation,
    period selection, risk methodology, trade recommendation, rebalance, or execution
    authority.
26. mandate-restriction caller and Advise-backed evaluation to publish candidate-created,
    blocked, and not-eligible HTTP 200 modes from application-backed,
    DTO-serialized factories. Caller examples must preserve the selected governed Core,
    Manage, or Advise source authority; source-backed examples must retain Advise
    `AdvisoryPolicyEvaluationRecord:v1` identity while redacting source routes and content
    hashes. Idea owns opportunity detection and compliance-review posture, not restriction
    clearance, mandate state, suitability/policy approval, client publication, rebalance,
    order, or execution authority.

Issue `#1154` supersedes evaluator-level duplicate suppression across every
signal family. Evaluation request schemas reject `duplicateOfCandidateId`;
repository persistence owns stable economic duplicate reconciliation and may
return `duplicate_candidate`. Manual review suppression and snooze remain
separate review-posture actions.
...
| --- | --- | --- | --- |
| `POST /api/v1/idea-signals/high-cash/evaluate` | High-cash deterministic evaluation over caller-supplied, source-owned Core evidence. Named OpenAPI success modes cover candidate-created, blocked, and not-eligible posture. | advisor role and `idea.signal.evaluate` capability | No live source ingestion, durable state, Gateway, Workbench, mesh certification, or supported-feature promotion. |
| `POST /api/v1/idea-signals/high-cash/evaluate-from-source` | High-cash deterministic evaluation over Core evidence fetched through the configured source adapter after caller portfolio entitlement passes. The same three named success modes are built through the source-backed application path. | advisor role, `idea.signal.evaluate` capability, portfolio entitlement scope, and exactly one tenant from trusted caller context; body tenant overrides are forbidden | No durable state, source-worker certification, live source certification, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/low-income/evaluate` | Low-income / liquidity-shortfall deterministic evaluation over caller-supplied, source-owned Core cashflow projection and cash movement evidence. Named OpenAPI modes cover candidate-created, blocked, and not-eligible posture. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, client income-needs assessment, funding advice, treasury instruction, planning suitability approval, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/low-income/evaluate-from-source` | Low-income / liquidity-shortfall deterministic evaluation over Core cash movement and cashflow projection evidence fetched through the configured source adapter after caller portfolio entitlement passes. The same three named modes are built through the Core-backed application path. | advisor role, `idea.signal.evaluate` capability, portfolio entitlement scope, and exactly one tenant from trusted caller context; body tenant overrides are forbidden | No durable state, source-worker certification, live source certification, client income-needs assessment, funding advice, treasury instruction, planning suitability approval, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/bond-maturity/evaluate` | Bond-maturity / reinvestment review deterministic evaluation over caller-supplied, source-owned Core `PortfolioMaturitySummary:v1` evidence with upstream `HoldingsAsOf:v1` lineage. Named OpenAPI modes cover candidate-created, blocked, and not-eligible posture. Canonical live Core source proof remains blocked until a valid source-safe runtime artifact is captured and merged. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, raw-position maturity schedule derivation, replacement product recommendation, reinvestment advice, maturity schedule authority, planning suitability approval, orders/execution, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
...
| `POST /api/v1/idea-signals/concentration-risk/evaluate` | Concentration-risk review deterministic evaluation over caller-supplied, source-owned Lotus Risk concentration evidence. OpenAPI publishes candidate-created, blocked, and not-eligible modes from the application path. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, concentration calculation, risk methodology approval, trade recommendation, rebalance action, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/concentration-risk/evaluate-from-source` | Concentration-risk review deterministic evaluation over Lotus Risk `ConcentrationRiskReport:v1` evidence fetched through the configured source adapter after caller portfolio entitlement passes. OpenAPI publishes candidate-created, blocked, and not-eligible modes through the source-backed application path. | advisor role, `idea.signal.evaluate` capability, and portfolio entitlement scope for the requested portfolio | No durable state, source-worker certification, live source certification, concentration calculation, risk methodology approval, trade recommendation, rebalance action, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/high-volatility/evaluate` | High-volatility deterministic evaluation over caller-supplied, source-owned Lotus Risk metrics evidence. OpenAPI publishes candidate-created, blocked, and not-eligible modes from the application path. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, volatility, VaR, or tracking-error calculation, risk methodology approval, trade recommendation, rebalance action, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
...
| `POST /api/v1/idea-signals/allocation-drift/evaluate` | Allocation-drift / mandate-review deterministic evaluation over caller-supplied, source-owned Lotus Manage action-register and mandate-health source-ref posture. OpenAPI publishes candidate-created, blocked, and not-eligible modes from the application path. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, drift calculation, mandate compliance approval, rebalance action, order creation, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/allocation-drift/evaluate-from-source` | Allocation-drift / mandate-review deterministic evaluation over Lotus Manage `PortfolioActionRegister:v1` posture fetched through the configured source adapter after caller portfolio entitlement passes. OpenAPI publishes candidate-created, blocked, and not-eligible modes; candidate lineage retains the supporting Manage, Performance, and Risk products. | advisor role, `idea.signal.evaluate` capability, and portfolio entitlement scope for the requested portfolio | No durable state, source-worker certification, live source certification, drift calculation, mandate compliance approval, rebalance action, order creation, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/mandate-restriction/evaluate` | Mandate/restriction deterministic evaluation over caller-supplied, source-owned Core, Manage, or Advise posture evidence. OpenAPI publishes candidate-created, blocked, and not-eligible modes from the application path while preserving the actual governed source authority. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, restriction clearance, mandate change, suitability approval, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/mandate-restriction/evaluate-from-source` | Mandate/restriction deterministic evaluation over explicit Lotus Advise `AdvisoryPolicyEvaluationRecord:v1` restriction diagnostic posture fetched through the configured source adapter after optional caller access-scope entitlement passes. OpenAPI publishes candidate-created, blocked, and not-eligible modes through the Advise-backed application path. | advisor role, `idea.signal.evaluate` capability, and entitlement scope when request accessScope is supplied | No durable state, source-worker certification, live source certification, restriction clearance, mandate-state change, suitability approval, policy/proposal approval, rebalance/order authority, Gateway, Workbench, mesh certification, client publication, data-product certification, or supported-feature promotion. |
| `POST /api/v1/idea-signals/missing-benchmark/evaluate` | Missing benchmark deterministic evaluation over caller-supplied, source-owned Core benchmark-assignment evidence. OpenAPI publishes candidate-created, blocked, and not-eligible modes from the application path. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, benchmark assignment, benchmark methodology authority, performance calculation, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/missing-benchmark/evaluate-from-source` | Missing benchmark deterministic evaluation over Core benchmark-assignment evidence fetched through the configured source adapter after caller portfolio entitlement passes. OpenAPI publishes candidate-created, blocked, and not-eligible modes through the Core-backed application path. | advisor role, `idea.signal.evaluate` capability, portfolio entitlement scope, and exactly one tenant from trusted caller context; body tenant overrides are forbidden | No durable state, source-worker certification, live source certification, benchmark assignment, benchmark methodology authority, performance calculation, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/missing-risk-profile/evaluate` | Missing risk-profile deterministic evaluation over caller-supplied, source-owned Advise `AdvisoryPolicyEvaluationRecord:v1` posture. OpenAPI publishes candidate-created, blocked, and not-eligible modes from the application path. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, risk-profile approval/creation, risk-capacity determination, suitability or policy approval, typed risk-profile data-product certification, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/missing-risk-profile/evaluate-from-source` | Missing risk-profile deterministic evaluation over Lotus Advise `AdvisoryPolicyEvaluationRecord:v1` diagnostic posture fetched through the configured source adapter after optional caller access-scope entitlement passes. OpenAPI publishes candidate-created, blocked, and not-eligible modes through the Advise-backed application path. | advisor role, `idea.signal.evaluate` capability, and entitlement scope when request accessScope is supplied | No durable state, source-worker certification, live source certification, risk-profile approval/creation, risk-capacity determination, suitability or policy/proposal approval, typed risk-profile data-product certification, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
| `POST /api/v1/idea-signals/missing-suitability/evaluate` | Missing suitability-context deterministic evaluation over caller-supplied, source-owned Advise policy-evaluation evidence. OpenAPI publishes candidate-created, blocked, and not-eligible modes from the application path. | advisor role and `idea.signal.evaluate` capability | No upstream source fetch, suitability approval, policy/proposal/sign-off approval, Gateway, Workbench, mesh certification, client publication, or supported-feature promotion. |
...
| `POST /api/v1/idea-signals/high-cash/evaluate-and-persist` | High-cash evaluation plus internal candidate persistence, idempotency, replay, audit, and evidence-driven expiry through the active repository provider. Named success modes distinguish accepted, replayed, duplicate-candidate, blocked, and not-eligible behavior. Non-candidate modes return `persistence=null`; authoritative not-eligible evidence expires a matching active candidate, while blocked evidence performs no write. Missing economic scope fails before any durable mutation. | `idea.candidate.persist`, `Idempotency-Key`, and complete `accessScope` for tenant/book/portfolio/client economic identity | `local`/`test` profiles may write to process-local storage with pseudonymous governed scope; `demo`/`staging`/`production` require `LOTUS_IDEA_DATABASE_URL` and fail closed with `durable_repository_not_configured` before in-memory mutation. Evaluation-only diagnostics may remain unscoped, but no durable caller can bypass the repository scope invariant. Real PostgreSQL persistence/replay, source-ingestion replay/conflict recovery, concurrent expiry fencing, and migration rollback/reapply recovery proof exists for this path and the first review/conversion/report workflow path, while production authentication/identity-provider authority, production storage certification, source-worker certification beyond bounded live proof, mesh certification, Gateway/Workbench proof, and supported-feature promotion remain planned. |
| `POST /api/v1/idea-candidates/{candidateId}/lifecycle-transitions` | Internal governed pre-review lifecycle transition recording over persisted candidate snapshots. Request input rejects review, conversion, downstream-outcome, and terminal states owned by other commands. | `idea.candidate.lifecycle.transition`, complete trusted tenant/book/portfolio/client scope matching the candidate, plus `Idempotency-Key` | Scope refusal precedes idempotency or mutation and changes no candidate, audit, outbox, or replay state. Accepted and replayed modes return the same exact persisted transition; malformed scope is `400`, missing/mismatched scope is `403`, and incomplete persisted evidence fails closed. No downstream authority, suitability, execution, Gateway, Workbench, or supported-feature promotion. |
| `GET /api/v1/idea-candidates/{candidateId}` | Internal source-safe candidate detail projection over persisted candidate snapshots, redacted evidence, workflow summaries, adviser-safe local downstream-submission posture, audit summary, and fail-closed caller entitlement enforcement. | `idea.candidate.detail.read` or advisor/operator role, plus complete trusted tenant/book/portfolio/client scope | Incomplete scope is refused before repository access. Read-only Gateway publication exists through `lotus-gateway` `GET /api/v1/ideas/candidates/{candidate_id}`, including entitlement-scope forwarding. With the durable PostgreSQL repository provider active, ordinary candidate-detail reads use a repository-side projection over the requested candidate and related detail rows plus one candidate-scoped submission query after scope approval, instead of whole-store snapshot hydration. Submission posture excludes support/idempotency/trace/failure internals and does not assert a source-owned conversion outcome. Bounded read-only Workbench rendering exists for source-safe candidate detail through PR #391. No source-system route disclosure, raw evidence export, full Workbench live proof, data-product certification, downstream authority, client-ready publication, or supported-feature promotion. |
| `POST /api/v1/idea-candidates/{candidateId}/evidence-replay` | Internal operator evidence replay posture over persisted candidate evidence hashes and caller-supplied current source refs. | `idea.candidate.evidence.replay` plus operator role plus complete trusted tenant/book/portfolio/client entitlement scope that authorizes the persisted candidate | Complete scope is required before any candidate read; a missing or mismatching dimension returns product-safe `403 permission_denied` without disclosing the candidate, and a generic operator role grants no estate-wide access. Generated OpenAPI and the endpoint ledger publish DTO-validated matched, hash-mismatch, stale-source, and expired modes. Stale-source and expired are normal product-safe HTTP 200 supportability postures with no current evidence hash. No live Core source export, downstream authority, Gateway/Workbench proof, data-product certification, production recovery certification, or supported-feature promotion. |
| `POST /api/v1/idea-candidates/{candidateId}/ai-explanations/evaluate` | Internal governed AI explanation fallback/verifier evaluation over persisted candidate evidence with source-safe idempotent lineage recording. An attested result may include a separately signed provider-retention confirmation bound to candidate tenant plus verified run/provider/model identity. | `idea.ai-explanation.evaluate`, complete trusted tenant/book/portfolio/client scope matching the candidate, plus `Idempotency-Key` | Scope refusal precedes evaluation, idempotency, and lineage persistence. Generated OpenAPI and the endpoint ledger publish complete named local-fixture, verified-attested, deterministic-fallback, unsupported-claim, forbidden-action, and unsafe-action-content responses from one DTO-validated code factory. Local/test output is visibly `unattested_local_test_fixture`; production-like workflow output requires verified Lotus AI provenance. Blocked responses expose server-owned narrative and empty grounding. Signed provider-retention receipts are atomic, replay-fenced, and source-safe; provider failure is blocked and not deletion proof. Same-key replay returns persisted lineage after re-verification. No provider call, lifecycle authority, runtime certification, downstream authority, Gateway/Workbench surface, client-ready publication, or supported-feature promotion. |
| `POST /api/v1/idea-candidates/{candidateId}/ai-explanations` | Generate a bounded explanation by deriving redacted input from persisted candidate evidence, invoking the registered Lotus AI explanation pack where local/test policy permits, and evaluating the returned content under Idea-owned controls. | `idea.ai-explanation.generate`, complete trusted tenant/book/portfolio/client scope matching the candidate, plus `Idempotency-Key` | Scope refusal precedes Lotus AI execution, idempotency, and lineage persistence. The response distinguishes `EXPLANATION_SERVED` from `EXPLANATION_UNAVAILABLE`, retains exact owner run identity when proved, and reports runtime execution separately from attestation. A stable hashed owner key prevents duplicate provider work; an exact pre/post candidate-evidence fence discards output if business identity, material/evidence version, evidence packet, or evidence hash changes in flight. Runtime failure, owner conflict, invalid/non-accepted output, and production-like attestation requirements retain deterministic fallback evidence and never mutate candidate lifecycle, ranking, review, conversion, or downstream authority. This is not live-provider/runtime certification, Gateway/Workbench proof, client publication, or supported-feature promotion. |
...
| `GET /api/v1/review-queues/advisor` | Internal deterministic advisor queue over persisted candidates routed to `advisor_review_required`. `evaluatedAtUtc` is the inclusive candidate-created-at boundary; source/evidence dates retain source-authority meaning. OpenAPI publishes application-backed `itemsAvailable` and `noItemsAvailable` 200 modes through the real queue projection and response DTO. | `idea.review.queue.read`, advisor role, and complete trusted tenant/book/portfolio/client scope | Scope is required before repository access; query filters may only narrow it. Queue `policyVersion` names the ranking policy; candidate `scorePolicyVersion` names score provenance. Every item publishes the current Idea-owned `materialVersion` and `evidenceVersion` so a visible-render producer can bind a receipt without reconstructing source state. Unknown policies fail closed. Page size defaults to 25 and is capped at 100. The opaque `snapshotToken` binds audience, scope, queue policy, accepted score-policy set, and visible state; changed state returns `409 review_queue_snapshot_conflict`. PostgreSQL uses the same audience-filtered bounded projection. Read-only Gateway publication exists; Workbench proof, data-product certification, client publication, and supported-feature promotion remain out of scope. |
| `GET /api/v1/review-queues/portfolio-manager` | Internal deterministic queue for candidates routed to `pm_review_required`. | `idea.review.queue.portfolio-manager.read`, portfolio-manager role, and complete trusted tenant/book/portfolio/client scope | Scope is required before repository access; query filters may only narrow it. Uses the shared audience-bound queue policy and snapshot contract. This is review work routing only; Manage retains mandate, rebalance, action, and execution authority. |
| `GET /api/v1/review-queues/compliance` | Internal deterministic queue for candidates routed to `compliance_review_required`. | `idea.review.queue.compliance.read`, compliance role, and complete trusted tenant/book/portfolio/client scope | Scope is required before repository access; query filters may only narrow it. Uses the shared audience-bound queue policy and snapshot contract. Queue presence is not compliance approval or suitability clearance. |
| `GET /api/v1/review-queues/operator/exceptions` | Aggregate support-exception posture grouped by business audience. | `idea.review.queue.exceptions.read` plus operator role plus complete trusted tenant/book/portfolio/client entitlement scope | Complete caller scope is required and query scope can only narrow within it; a missing dimension returns `403 permission_denied` rather than widening the aggregate across the estate. Reports invalid-state, unsupported-evidence, unscored, unrankable-policy, and non-reviewable counts without candidate identifiers or business decision authority. It is an internal operational foundation, not a supported product feature. |
...
| `POST /api/v1/idea-candidates/{candidateId}/review-actions` | Internal exact-evidence review decision recording with fail-closed trusted entitlement scope and lifecycle checks. | `idea.review.record`, trusted caller scope, `Idempotency-Key`, expected material/evidence/packet/hash identity, explicit channel, and a persisted presentation receipt for Workbench review | Authorization precedes scoped receipt lookup. Workbench review requires an exact tenant/candidate receipt within the governed window; operator review cannot carry a fabricated receipt. Only `suppress` accepts `suppressionReason`; only `snooze` accepts `snoozedUntilUtc`; irrelevant action fields return `400 invalid_request` before mutation. `reviewId` and exact authority identity are durable across transport keys. Stale identity, missing/foreign receipt, ambiguous persistence, or legacy-unverified authority fails before mutation. PostgreSQL claims identity before candidate/audit/outbox mutation. No suitability, compliance, mandate, execution, client communication, consumer-runtime proof, or supported-feature promotion. |
| `POST /api/v1/idea-candidates/{candidateId}/feedback` | Internal source-provenanced advisor feedback recording with fail-closed trusted entitlement scope and canonical taxonomy checks. | `idea.feedback.record`, advisor role, trusted caller entitlement scope headers, `Idempotency-Key`, `idea-feedback-taxonomy-v1`, and an allowed usefulness outcome/reason pair | The request body cannot assert access or authorization scope. Candidate lookup uses the bounded projection when available; `feedbackId` binds candidate, evidence, source signals, actor, taxonomy version, outcome, reason, and event time independently of the transport key. Equivalent content under a new key replays with the exact persisted event; changed content returns `review_identity_conflict`; invalid taxonomy combinations return stable `feedback_taxonomy_combination_invalid`; missing or ambiguous evidence fails closed; PostgreSQL prevents duplicate feedback/audit/outbox writes under concurrency. Feedback remains separate from review actions and cannot mutate production policy. No online learning, model training, Gateway, Workbench, data-product certification, or supported-feature promotion. |
| `POST /api/v1/idea-candidates/{candidateId}/conversion-intents` | Internal exact-review-gated conversion intent recording for Advise, Manage, or Report targets. | `idea.conversion.intent.record`, `Idempotency-Key`, exact approving `reviewId`, and expected material/evidence/packet/hash identity | Exact replay is resolved first and returns the original accepted result even after later expiry. A new intent requires the exact persisted approval grant to remain active, current, evidence-ready, target-eligible, and strictly before applicability expiry at trusted server time. The repository repeats this check under the candidate mutation lock before intent/outbox creation. Mutable approved posture or ambiguous legacy history is insufficient. Intent only; no downstream business authority or supported-feature promotion. |
| `POST /api/v1/conversion-intents/{conversionIntentId}/downstream-submissions` | Internal source-safe claim-before-call submission for Advise or Manage. OpenAPI publishes accepted, rejected, replayed, uncertain, authority-conflict, and resource-conflict modes. Acceptance is returned only after local terminal state commits. | `idea.downstream-realization.submit` plus `Idempotency-Key` | A first attempt requires retained intent to match current evidence, exact active review authority, target lifecycle, owner authority, applicability, and authoritative source-cut posture. Caller replay identity is tenant-scoped, and each governed resource/target admits only one submission aggregate; a different key returns `409 downstream_submission_resource_conflict` without owner I/O. Already admitted work remains exactly replayable or read-only reconcilable. No authoritative downstream outcome or supported-feature claim. |
| `POST /api/v1/conversion-intents/{conversionIntentId}/outcomes` | Internal source-authorized, source-versioned downstream outcome recording with cross-key resource replay, append-only correction, and deterministic current posture. | `idea.conversion.outcome.record`, complete tenant/book/portfolio/client entitlements matching the persisted candidate, plus `Idempotency-Key` | Candidate scope is checked before idempotency precheck or mutation. Missing, partial, or mismatched scope returns `403`; malformed scope syntax returns `400`. Accepted and replayed success return the same exact identity-bound persisted outcome. Missing or ambiguous persisted evidence fails closed as `503 service_recovery_degraded`; `conversion_outcome_conflict` remains distinct from transport `idempotency_conflict`. Outcome tracking grants no downstream workflow, suitability, execution, reporting, archive, client-publication, or supported-feature authority. |
| `POST /api/v1/conversion-intents/{conversionIntentId}/report-evidence-packs` | Internal report evidence-pack request recording for reviewed report conversion intents. | `idea.report-evidence-pack.request`, complete trusted tenant/book/portfolio/client scope matching the persisted candidate, and `Idempotency-Key` | Scope denial precedes idempotency and mutation. Accepted and replayed success return the same exact persisted request; zero or multiple identity-bound matches fail closed as `503 service_recovery_degraded`. Request only; no `lotus-report` package intake, `lotus-render` output, `lotus-archive` record, client-ready publication, or supported-feature promotion. |
| `POST /api/v1/report-evidence-packs/{reportEvidencePackId}/downstream-submissions` | Internal source-safe claim-before-call submission for an existing Report evidence-pack request. OpenAPI publishes accepted, rejected, replayed, uncertain, idempotency-conflict, and resource-conflict modes. Acceptance is returned only after local terminal state commits. | `idea.downstream-realization.submit` plus `Idempotency-Key` | Caller replay identity is tenant-scoped, and each governed evidence pack admits only one submission aggregate for its target; a different key returns `409 downstream_submission_resource_conflict` without another Report call. No production identity, Report completion, Render output, Archive record, client-ready publication, or supported-feature promotion. |
| `POST /api/v1/downstream-submissions/{supportReference}/report-materialization-reconciliation` | Internal read-only recovery of the exact Report-owned receipt for an uncertain evidence-pack submission. | `idea.downstream-realization.reconcile` plus complete entitlement scope | Uses the existing submission aggregate and stored idempotency key; never retries materialization, and grants no publication or supported-feature authority. |
| `GET /api/v1/downstream-realization/readiness` | Internal operator diagnostic for Advise, Manage, Report, Render, and Archive realization blockers over current `lotus-idea` workflow counts, local downstream submission denominator, unresolved downstream submission reconciliation workload, manifest-backed planned downstream contract readiness, and optional bounded evidence artifacts. | `idea.downstream-realization.readiness.read` plus operator role | Diagnostic only; planned contract records are validated by `make downstream-realization-contract-gate` but are not downstream route-existence proof. Submission and reconciliation workload counts are local Idea posture only. Report intake and materialization source contracts clear no blocker and do not prove report-job execution, rendered output, archive creation, suitability authority, rebalance/action authority, client-ready publication, production certification, or supported-feature promotion. |
...
| `GET /api/v1/data-mesh/trust-telemetry/runtime-preview` | Internal operator diagnostic for source-safe aggregate runtime trust telemetry preview over the active repository provider, including local downstream submission posture counts and issue-backed blocker refs. | `idea.mesh.trust-telemetry.preview.read` plus operator role | Diagnostic only; downstream posture is local Idea state, not downstream acceptance or materialization proof. No candidate detail, source route export, data-product certification, platform source-manifest inclusion, Gateway/Workbench discovery, or supported-feature promotion. |
| `GET /api/v1/data-mesh/trust-telemetry/runtime-snapshot` | Internal operator diagnostic for source-safe contract-shaped runtime trust telemetry snapshot evidence over the active repository provider, including a closed local downstream submission posture block and issue-backed blocker refs. | `idea.mesh.trust-telemetry.snapshot.read` plus operator role | Diagnostic only; downstream posture is local Idea state, not downstream acceptance or materialization proof. No candidate detail, source route export, data-product certification, platform source-manifest inclusion, Gateway/Workbench discovery, client-ready publication, or supported-feature promotion. |
