...
| `GET /api/v1/ai-explanations/readiness` | Internal operator diagnostic for AI explanation guardrail availability, model-risk supportability posture, and certification blockers. | `idea.ai-explanation.readiness.read` plus operator role | Diagnostic only; no `lotus-ai` runtime execution, provider call, prompt exposure, durable AI lineage store, model-risk dashboard certification, Gateway/Workbench proof, data-product certification, client-ready publication, or supported-feature promotion. |
| `GET /api/v1/review-queues/advisor` | Internal deterministic advisor queue projection over persisted candidate snapshots, optionally constrained by tenant/book/portfolio/client scope filters. | `idea.review.queue.read` or advisor role | Read-only Gateway publication exists through `lotus-gateway` `GET /api/v1/ideas/review-queues/advisor`. Scope-aware filtering is implemented, but platform caller-context entitlement proof, durable queue store, Workbench proof, data-product certification, PM/compliance/operator queue surface, client-ready publication, and supported-feature promotion remain out of scope. |
...
| `GET /api/v1/downstream-realization/readiness` | Internal operator diagnostic for Advise, Manage, Report, Render, and Archive realization blockers over current `lotus-idea` workflow counts and planned downstream contract readiness. | `idea.downstream-realization.readiness.read` plus operator role | Diagnostic only; planned contract records are not downstream route-existence proof; no downstream service calls, proposal creation, manage action creation, report materialization, render output, archive record, client-ready publication, or supported-feature promotion. |
| `GET /api/v1/source-ingestion/readiness` | Internal operator diagnostic for high-cash Core source-ingestion run-once worker configuration and certification blockers. | `idea.source-ingestion.readiness.read` plus operator role | Diagnostic only; no live Core proof, scheduled worker deploy proof, data-product certification, platform source-manifest inclusion, Gateway/Workbench support, or supported-feature promotion. |
| `GET /api/v1/outbox-delivery/readiness` | Internal operator diagnostic for aggregate outbox backlog, retry/dead-letter posture, durable repository posture, and broker/downstream certification blockers. | `idea.outbox-delivery.readiness.read` plus operator role | Diagnostic only; no event identifiers, broker publication, downstream delivery, platform mesh event certification, Gateway/Workbench support, or supported-feature promotion. |
...
| `GET /api/v1/implementation-proof/readiness` | Internal aggregate operator diagnostic for RFC-0002 implementation proof blockers, including outbox-delivery readiness. | `idea.implementation-proof.readiness.read` plus operator role | Diagnostic only; no live implementation proof, external broker publication, downstream delivery, data-product certification, platform source-manifest inclusion, Gateway/Workbench proof, client-ready publication, or supported-feature promotion. |
