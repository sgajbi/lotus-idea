...
| `GET /api/v1/outbox-delivery/readiness` | Internal operator diagnostic for aggregate outbox backlog, retry/dead-letter posture, durable repository posture, broker configuration, publisher-adapter presence, and broker/downstream certification blockers. | `idea.outbox-delivery.readiness.read` plus operator role | Diagnostic only; `make outbox-event-contract-gate` and `make outbox-consumer-contract-gate` validate repo-owned event and consumer contracts, but there are no event identifiers, certified live broker runtime, downstream consumer runtime proof, platform mesh event publication proof, Gateway/Workbench support, or supported-feature promotion. |
| `POST /api/v1/outbox-delivery/run-once` | Internal operator action that runs one bounded outbox delivery pass through the active repository and configured publisher adapter. | `idea.outbox-delivery.run` plus operator role | Aggregate run summary only; fails closed without broker configuration; no event identifiers, live broker certification, downstream consumer runtime proof, platform mesh event publication proof, Gateway/Workbench support, or supported-feature promotion. |
...
| `GET /api/v1/implementation-proof/readiness` | Internal aggregate operator diagnostic for RFC-0002 implementation proof blockers, including outbox-delivery readiness, run-once posture, and bounded downstream proof artifact consumption. | `idea.implementation-proof.readiness.read` plus operator role | Diagnostic only; bounded proof artifacts clear only named blockers. No live implementation proof, certified live broker runtime, Advise suitability authority, Manage rebalance/action authority, data-product certification, platform source-manifest inclusion, Gateway/Workbench proof, client-ready publication, or supported-feature promotion. |
