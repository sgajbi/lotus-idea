...
| `POST /api/v1/idea-candidates/{candidateId}/review-actions` | Internal advisor review decision recording with fail-closed trusted entitlement scope and lifecycle/review-posture checks. | `idea.review.record`, advisor role, trusted caller entitlement scope headers, and `Idempotency-Key` | The request body cannot assert access or authorization scope. Candidate lookup uses the bounded projection when available; governance evaluates persisted scope against trusted caller entitlements and `idea-candidate-state-v1`. `reviewId` is a durable resource identity: equivalent content under a new key replays with the exact persisted decision; changed business content returns `review_identity_conflict`. Missing or ambiguous evidence on a successful persistence result fails closed. PostgreSQL claims identity before candidate/audit/outbox mutation. No suitability, compliance, mandate, execution, client communication, Gateway, Workbench, or supported-feature promotion. |
