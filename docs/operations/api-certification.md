...
| `POST /api/v1/conversion-intents/{conversionIntentId}/downstream-submissions` | Internal source-safe claim-before-call submission for Advise or Manage. OpenAPI publishes accepted, rejected, accepted-replayed, and rejected-replayed `200` modes, a separate `202 reconciliation_required` mode, and a `409 conversion_intent_authority_conflict` refusal. Acceptance is returned only after local terminal state commits. | `idea.downstream-realization.submit` plus `Idempotency-Key` | A first attempt requires the retained intent to match current candidate evidence, exact active review authority, target lifecycle, owner authority, applicability, and authoritative source-cut posture; failure occurs before claim or owner I/O. Idempotency is tenant-scoped: same-tenant retries never call downstream again after terminal or uncertain posture, while another tenant may safely reuse the ordinary key under a distinct claim and support reference. An already admitted attempt remains exactly replayable/read-only reconcilable after later authority expiry. A lost response is never blindly resubmitted. No authoritative outcome recording, proposal/action creation proof, suitability, execution, Gateway, Workbench, data-product certification, or supported-feature promotion. |
| `POST /api/v1/conversion-intents/{conversionIntentId}/outcomes` | Internal source-authorized, source-versioned downstream outcome recording with cross-key resource replay, append-only correction, and deterministic current posture. | `idea.conversion.outcome.record` plus `Idempotency-Key` | Accepted and replayed success return the same exact identity-bound persisted outcome. Missing or ambiguous persisted evidence fails closed as `503 service_recovery_degraded`; `conversion_outcome_conflict` remains distinct from transport `idempotency_conflict`. Outcome tracking grants no downstream workflow, suitability, execution, reporting, archive, client-publication, or supported-feature authority. |
| `POST /api/v1/conversion-intents/{conversionIntentId}/report-evidence-packs` | Internal report evidence-pack request recording for reviewed report conversion intents. | `idea.report-evidence-pack.request` plus `Idempotency-Key` | Accepted and replayed success return the same exact persisted request; zero or multiple identity-bound matches fail closed as `503 service_recovery_degraded`. Request only; no `lotus-report` package intake, `lotus-render` output, `lotus-archive` record, client-ready publication, or supported-feature promotion. |
| `POST /api/v1/report-evidence-packs/{reportEvidencePackId}/downstream-submissions` | Internal source-safe claim-before-call submission for an existing Report evidence-pack request. In local/test it can map persisted trusted scope to Report's materialization request with server-fixed `json` output; OpenAPI publishes accepted, rejected, accepted-replayed, and rejected-replayed `200` modes plus a separate `202 reconciliation_required` mode. Acceptance is returned only after local terminal state commits. | `idea.downstream-realization.submit` plus `Idempotency-Key` | Idempotency is tenant-scoped: same-tenant retries never call downstream again after terminal or uncertain posture, while another tenant may safely reuse the ordinary key under a distinct claim and support reference. No production identity, Report job completion, Render output, Archive record, authoritative outcome, Gateway, Workbench, data-product certification, client-ready publication, or supported-feature promotion. |
