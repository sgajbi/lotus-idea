...
25. `src/app/application/downstream_realization_readiness.py` and
    ...
    `idea_downstream_submission`, without materializing candidate, audit,
    outbox, downstream-submission payload, or AI-lineage state.
...
The bounded outbox broker runtime-execution artifact is `runtime_execution`
evidence. It may clear only `external_broker_runtime_proof_missing` when a
configured HTTP broker accepts the source-safe publication canary through the
real `HttpOutboxEventPublisher`; it retains downstream consumer, platform mesh,
Gateway/Workbench, production certification, and supported-feature blockers.
It is generated by `scripts/outbox/broker/generate_runtime_execution.py` and
validated by `make outbox-broker-runtime-execution-proof-gate`.
The bounded outbox consumer runtime-execution artifact is also
`runtime_execution` evidence, but only for the domain-consumer leg. It consumes
valid Advise, Manage, and Report runtime receipts, binds source-safe proof refs
and digests, and may clear only `downstream_consumer_runtime_proof_missing`.
...
blockers. It is generated by
`scripts/outbox/generate_consumer_runtime_execution.py` and validated by
...
`costAttributionVerified`. Platform issue `lotus-platform#599` has merged the
bounded cost-attribution and deployment-promotion source contract, so Idea can
recognize the contract boundary without claiming protected cost certification.
Official provider/platform billing adapters, allocation, decimal reconciliation,
and attestation remain outside Idea and are tracked in `lotus-platform#495`.
The platform producer contract plus Idea's narrow cross-repository verifier are
implemented locally. Idea accepts cost qualification only when the platform
repository, signer, main ref, commit, and artifact digest verify and the
platform resource digest and run id match the already attested Idea resource
proof. Therefore `cost_attribution_evidence_missing` stays open until protected
mainline execution produces that matching evidence.
