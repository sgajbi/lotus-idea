...
| Mandate or restriction review | `lotus-core`; `lotus-manage`; `lotus-advise` | Mandate, restriction, or suitability context blocks or changes actionability. | Partially implemented - deterministic source-ref-preserving policy and bounded caller-supplied API foundation only; live upstream source proof, typed restriction source products, data-mesh certification, Workbench proof, client publication, and supported promotion remain blocked |
