...
| 6 | Persistence, replay, idempotency, and audit | Partially implemented - internal persistence plus high-cash orchestration foundation only | internal records, hashes, replay posture, audit events, idempotent high-cash persist orchestration; database persistence remains planned |
| 7 | Scoring, ranking, suppression, and queue policy | Partially implemented - internal scoring plus repository-snapshot queue projection only | deterministic score inputs/policy, reason codes, stable queue projection, snapshot orchestration, snooze/suppression/dedupe tests; persisted queue/API proof remains planned |
| 8 | Review queues, feedback, and human governance | Partially implemented - internal advisor governance plus persistence orchestration only | advisor review actions, feedback model, entitlement tests, idempotent repository persistence, audit proof; database/API, PM/compliance/operator surfaces remain planned |
...
| 10 | Certified APIs, OpenAPI, and Gateway contract | Partially implemented - certified high-cash evaluate and evaluate-and-persist API foundations only | high-cash evaluate/evaluate-and-persist endpoints, OpenAPI examples, endpoint certification, idempotency proof; Gateway route contract remains planned |
...
| --- | --- | --- | --- | --- |
| Idea candidate lifecycle | `lotus-idea` | Proposed | API, persistence, audit, endpoint certification | lifecycle model, tests, OpenAPI, replay, supported-feature update |
...
| Advisor opportunity queue | `lotus-idea`, `lotus-gateway`, `lotus-workbench` | Proposed | queue API, Gateway route, Workbench panel | browser proof, entitlement proof, unsupported/degraded states |
| Scoring and ranking | `lotus-idea` | Proposed | scoring policy, rank API/projection | golden examples, score versioning, stability, supportability |
...
| Review and feedback | `lotus-idea` | Proposed | review API, audit, feedback data product | role tests, audit events, feedback validation, queue update proof |
