...
| 7 | Scoring, ranking, suppression, and queue policy | Partially implemented - deterministic scoring plus certified advisor queue API foundation and durable repository-side page projection with index-backed access-scope predicates | deterministic score inputs/policy, reason codes, stable queue projection, snapshot orchestration for in-memory/proof paths, repository-side PostgreSQL candidate page projection with tenant/book/portfolio/client expression-index-backed scope filters, snooze/suppression/dedupe tests, advisor queue API proof; Workbench, data-product certification, PM/compliance queues, and supported promotion remain planned |
...
| 19 | Second-last hardening and review | Partially implemented - quality scorecard truth, governed local `ci`/`ci-release` command split, release evidence, CI, API, security, data-access, observability, resilience, and operator-action hardening in progress | code/API/security/data-mesh/docs review and fixes remain before closure |
