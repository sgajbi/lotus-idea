...
| 10 | Certified APIs, OpenAPI, and Gateway contract | Partially implemented - certified internal API foundations including source-safe candidate detail, AI explanation evaluator, and operator diagnostics only | high-cash evaluate/evaluate-and-persist, lifecycle transition, candidate detail, AI explanation, advisor queue, review-action, feedback, conversion intent/outcome, report evidence-pack, data-mesh readiness, and source-ingestion readiness endpoints; Gateway route contract remains planned |
| 11 | Workbench product realization | Planned - source-safe candidate detail API foundation exists, Gateway/Workbench proof remains pending | review queue UI, evidence drawer, browser/accessibility proof |
| 12 | Advise and Manage conversion realization | Partially implemented - internal conversion governance and certified API foundation only | review-gated conversion intents/outcomes, source-authority mapping, certified internal conversion API proof, first PostgreSQL-backed report conversion intent/outcome proof, no downstream authority proof; downstream adapters and contracts remain planned |
| 13 | Report, Render, Archive, and evidence-pack materialization | Partially implemented - internal report evidence-pack request foundation only | certified internal request API, source summaries, retention refs, idempotency/audit proof, first PostgreSQL-backed internal request proof; Report/Render/Archive downstream materialization remains planned |
...
| 15 | Observability, security, entitlements, and operations | Partially implemented - bounded operation events plus source-ingestion readiness diagnostic | operation-event metric/log vocabulary, no-sensitive-label proof, endpoint instrumentation, and source-ingestion configuration/certification blocker diagnostic; broader runbooks, live source proof, and production telemetry remain planned |
...
| Advisor opportunity queue | `lotus-idea`, `lotus-gateway`, `lotus-workbench` | Gated foundation | queue API, Gateway route, Workbench panel | browser proof, entitlement proof, unsupported/degraded states |
