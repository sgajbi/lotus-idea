...
| 3 | Opportunity domain model, vocabulary, and lifecycle | Implemented foundation; `idea-opportunity-identity-v2` and explicit evidence/material/reopen vocabulary are locally complete pending issue `#1154` mainline closure | pure domain model, lifecycle tests, unsupported transition behavior, and issue `#1154` identity invariants |
...
| 5 | Deterministic signal evaluation and candidate generation | Implemented foundation; stable economic business identity independent from source hashes is locally complete pending issue `#1154` mainline closure. Candidate generation remains deterministic and policy-versioned across governed caller-supplied and source-backed APIs for Core, Risk, Performance, Advise, and Manage. | reproducible candidate generation, all-family economic identity, complete reason-path tests, fail-closed missing/stale/unauthorized evidence, source-authority preservation, no duplicated source calculations, adapter/API/OpenAPI proof, and explicit later-slice promotion blockers; issue `#1154` remains open until merge and exact-main closure evidence |
| 6 | Persistence, replay, idempotency, and audit | Implemented foundation; economic-identity reconciliation and migration `016` are locally complete pending issue `#1154` mainline closure. Repository-owned persistence, replay/audit, lifecycle controls, durable downstream recovery, operator reconciliation, outbox delivery/recovery, PostgreSQL adapters, and recovery proof remain in place. | Schema/rollback through migration `016`; complete Idea-owned workflow persistence; equivalent evidence refresh, material version, recurrent reopen, identity-conflict and concurrency fencing; recovery-aware writes; real PostgreSQL restart/concurrency/logical-restore proof; and protected exact-image migration controls. Managed infrastructure, external authority, live production certification, publication, Gateway/Workbench product proof, and supported promotion remain fail-closed under issues `#340`, `#343`-`#345`, `#375`, and `#378`-`#380`. |
