...
| 5 | Deterministic signal evaluation and candidate generation | Implemented on branch, pending mainline closure - deterministic, policy-versioned candidate generation; governed caller-supplied and source-backed APIs across Core, Risk, Performance, Advise, and Manage; trusted Core tenant scope; temporal reconciliation; stable caller-boundary ProblemDetails; and canonical Risk/Performance proof | reproducible candidate generation, complete reason-path tests, fail-closed missing/stale/unauthorized evidence, source-authority preservation, no duplicated source calculations, adapter/API/OpenAPI proof, and explicit later-slice promotion blockers |
