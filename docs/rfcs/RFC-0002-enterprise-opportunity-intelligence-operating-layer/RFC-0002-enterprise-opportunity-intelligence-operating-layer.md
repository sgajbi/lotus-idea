...
| 5 | Deterministic signal evaluation and candidate generation | Partially implemented - high-cash domain policy plus Core source-port and bounded run-once ingestion worker foundation | first signal families, candidate generation, golden scenario tests, Core source-port orchestration, bounded run-once ingestion worker semantics |
| 6 | Persistence, replay, idempotency, and audit | Partially implemented - internal persistence plus schema, rollback, migration execution, PostgreSQL adapter, opt-in API repository wiring, first PostgreSQL runtime workflow proof, source-ingestion replay/conflict recovery proof, bounded run-once ingestion worker foundation, and migration rollback/reapply recovery proof | internal records, hashes, replay posture, audit events, idempotent high-cash persist and lifecycle transition orchestration, schema/rollback contract, migration dry-run and execution CLI, tested PostgreSQL repository adapter, opt-in API provider selection through `LOTUS_IDEA_DATABASE_URL`, real PostgreSQL persistence/replay proof for the high-cash persist API plus the first review/feedback/conversion/report workflow path, internal source-ingestion replay/conflict recovery proof, bounded run-once source-ingestion batch worker foundation, and real PostgreSQL schema rollback/reapply recovery proof; live source adapters, scheduled daemon/deploy workers, downstream proof, and supported promotion remain planned |
...
| 10 | Certified APIs, OpenAPI, and Gateway contract | Partially implemented - certified internal API foundations including AI explanation evaluator only | high-cash evaluate/evaluate-and-persist, lifecycle transition, AI explanation, advisor queue, review-action, feedback, conversion intent/outcome, report evidence-pack, and data-mesh readiness endpoints; Gateway route contract remains planned |
...
| 15 | Observability, security, entitlements, and operations | Partially implemented - bounded certified internal API operation-event foundation including AI explanation only | operation-event metric/log vocabulary, no-sensitive-label proof, endpoint instrumentation; broader runbooks and production telemetry remain planned |
