...
| Bond maturity or reinvestment | `lotus-core`; optional `lotus-manage` context | Fixed-income maturity creates reinvestment or liquidity review need. | Partially implemented - deterministic policy plus Core `HoldingsAsOf:v1` maturity-date source-port/adapter/live-proof foundation; data-mesh certification, Workbench proof, product recommendation, reinvestment advice, and supported promotion remain blocked |
