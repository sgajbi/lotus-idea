...
| 5 | Deterministic signal evaluation and candidate generation | Implemented on `main` - deterministic, policy-versioned candidate generation; governed caller-supplied and source-backed APIs across Core, Risk, Performance, Advise, and Manage; trusted Core tenant scope; temporal reconciliation; stable caller-boundary ProblemDetails; canonical Risk/Performance proof; PRs `#347`/`#348`; green main commit `ad88690` | reproducible candidate generation, complete reason-path tests, fail-closed missing/stale/unauthorized evidence, source-authority preservation, no duplicated source calculations, adapter/API/OpenAPI proof, and explicit later-slice promotion blockers |
| 6 | Persistence, replay, idempotency, and audit | Implemented - repository-owned persistence, replay/audit, lifecycle controls, durable downstream recovery, operator reconciliation, outbox delivery/recovery, deployment migration, PostgreSQL adapters, recovery proof, and bounded broker/consumer/mesh proof foundations | Schema/rollback through migration `015`; complete Idea-owned workflow persistence; identity fencing; recovery-aware writes; real PostgreSQL restart/concurrency/logical-restore proof; and protected exact-image migration controls. Managed infrastructure, external authority, live production certification, publication, Gateway/Workbench product proof, and supported promotion remain fail-closed under issues `#340`, `#343`-`#345`, `#375`, and `#378`-`#380`. |
