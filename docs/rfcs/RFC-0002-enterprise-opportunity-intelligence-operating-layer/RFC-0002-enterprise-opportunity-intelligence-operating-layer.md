...
| Concentration review | `lotus-risk`; `lotus-core` issuer/holding refs | Portfolio has concentration, issuer, sector, country, or product exposure requiring review. | Partially implemented - deterministic policy, fail-closed Lotus Risk concentration source-adapter foundation, source-safe live-proof contract, and bounded caller-supplied API foundation over `ConcentrationRiskReport:v1`; concentration calculation, risk methodology approval, trade recommendation, rebalance action, data-mesh certification, Workbench proof, client publication, and supported promotion remain blocked |
...
| Bond maturity or reinvestment | `lotus-core`; optional `lotus-manage` context | Fixed-income maturity creates reinvestment or liquidity review need. | Partially implemented - deterministic policy plus bounded caller-supplied API foundation and Core `PortfolioMaturitySummary:v1` source-adapter consumption; data-mesh certification, Workbench proof, product recommendation, reinvestment advice, maturity schedule authority, order execution, canonical live Core maturity proof, and supported promotion remain blocked |
...
| High volatility or drawdown | `lotus-risk`; returns from `lotus-performance` | Risk profile or realized risk calls for review. | Partially implemented - deterministic high-volatility and drawdown-review policies plus Lotus Risk `RiskMetricsReport:v1` and `DrawdownAnalyticsReport:v1` source-port/adapter/live-proof foundations; data-mesh certification, Workbench proof, and supported promotion remain blocked |
| Missing benchmark | `lotus-core`; `lotus-performance` dependency | Portfolio lacks benchmark identity required for meaningful analytics. | Partially implemented - deterministic evidence-gap policy and Core benchmark-assignment source-port foundation only; benchmark assignment, methodology, benchmark returns, live readiness proof, data-mesh certification, Workbench proof, and supported promotion remain blocked |
| Missing or stale risk profile | `lotus-advise`; client facts from `lotus-core` | Advice or review is blocked because risk profile is missing, stale, or incompatible. | Partially implemented - deterministic evidence-gap policy, bounded Advise policy-evaluation diagnostic consumption, closed v2 typed Advise source-contract evidence, and receipt-bound runtime-evidence contract exist; fresh live qualification, data-mesh certification, Workbench proof, client publication, deployment/production certification, and supported promotion remain blocked |
| Mandate or restriction review | `lotus-core`; `lotus-manage`; `lotus-advise` | Mandate, restriction, or suitability context blocks or changes actionability. | Partially implemented - deterministic source-ref-preserving policy, bounded caller-supplied API foundation, typed Advise restriction source-product proof, and closed v2 receipt-bound Advise runtime-evidence contract exist; producer scope/time, data-mesh certification, Workbench proof, client publication, restriction-clearance authority, mandate-state authority, deployment/production certification, and supported promotion remain blocked |
...
| `IdeaFeedbackEvent:v2` | versioned usefulness outcome/reason, immutable opportunity-policy context, and source-safe offline quality-evaluation evidence | Implemented internal contract; not certified or promoted |
...
| 2 | Cleanup, structure, and current surface normalization | Partially implemented - runtime composition providers, route metadata, DTO base-model, and idempotency boundaries normalized behind shared API surfaces with blocking enforcement retained | dead-code/doc-sprawl cleanup, module boundaries, vocabulary baseline, runtime composition boundary proof |
| 3 | Opportunity domain model, vocabulary, and lifecycle | Implemented foundation; `idea-opportunity-identity-v3` separates observation evidence from economic materiality and retains explicit evidence/material/reopen vocabulary | pure domain model, lifecycle tests, unsupported transition behavior, identity invariants, and safe one-way policy backfill |
| 4 | Source authority, signal contracts, and data mesh baseline | Implemented - pre-certification mesh baseline enforced without promotion | consumer/producer contracts, source authority tests, producer semantics, consumer freshness/provenance metadata, blocked telemetry posture, optional platform catalog/source-manifest reconciliation |
| 5 | Deterministic signal evaluation and candidate generation | Implemented foundation; stable economic business identity independent from source hashes is merged and exact-main validated through issue `#1154` and PR `#1157`. Candidate generation remains deterministic and policy-versioned across governed caller-supplied and source-backed APIs for Core, Risk, Performance, Advise, and Manage. | reproducible candidate generation, all-family economic identity, complete reason-path tests, fail-closed missing/stale/unauthorized evidence, source-authority preservation, no duplicated source calculations, adapter/API/OpenAPI proof, and explicit later-slice promotion blockers |
| 6 | Persistence, replay, idempotency, and audit | Implemented foundation; economic-identity reconciliation and migration `016` are merged and exact-main validated through issue `#1154` and PR `#1157`. The current RFC implementation extends the governed schema, rollback, deployment manifest, lifecycle inventory, and recovery proof through migration `019` for feedback evaluation, effectiveness projection, and immutable presentation receipts under `#1155` and `#1156`. | Complete Idea-owned workflow persistence; equivalent evidence refresh, material version, recurrent reopen, identity-conflict and concurrency fencing; receipt replay/version/tenant/chronology controls; recovery-aware writes; real PostgreSQL restart/concurrency/logical-restore proof; and protected exact-image migration controls. Managed infrastructure, external authority, live production certification, publication, Gateway/Workbench product proof, and supported promotion remain fail-closed under issues `#340`, `#343`-`#345`, `#375`, and `#378`-`#380`. |
| 7 | Scoring, ranking, suppression, and queue policy | Partially implemented - deterministic scoring plus certified advisor queue API foundation, candidate-created-at as-of visibility, opaque snapshot continuation, and durable repository-side projection with index-backed scope predicates | deterministic score inputs/policy, reason codes, stable queue projection, shared snapshot identity/conflict policy, PostgreSQL before/after fingerprint verification, snooze/suppression/dedupe tests, API and real PostgreSQL proof; Workbench, data-product certification, PM/compliance queues, and supported promotion remain planned |
| 8 | Review queues, feedback, and human governance | Partially implemented - internal review foundations now include a versioned usefulness/reason taxonomy and bounded tenant-scoped offline quality-evaluation projection; supported product proof remains pending | advisor review actions, canonical feedback identity/API/outbox/persistence truth, deterministic migration and rollback, entitlement/replay/conflict tests, privacy-safe reproducible evaluation proof, and no-online-learning control are implemented; Gateway/Workbench, mesh certification, client publication, and supported promotion remain planned |
| 9 | Governed AI explanation and model-risk controls | Repository implementation complete - internal AI governance, governed workflow-pack allowlist, signed Lotus AI run-attestation verification, durable replay protection, certified API foundation, source-safe lineage persistence, v2 source-contract classification for registration and model-risk operations evidence, failure-path projection safety, and not-certified readiness diagnostic | redacted AI request envelopes, fallback/verifier/attested-output API, bounded verified receipt, safe audit, no-authority proof, and `lotus-ai:idea-explanation:v1` request identity mapped to `idea_explanation.pack@v1` proof identity are implemented; live-provider execution, production model-risk approval, runtime operations execution, Gateway/Workbench proof, and promotion remain externally blocked |
| 10 | Certified APIs, OpenAPI, and Gateway contract | Partially implemented - certified internal API foundations include truthful advisor queue as-of/snapshot paging, stable 400/409 errors, AI workflow-pack identity rejection, and bounded read-only Gateway publication for queue/detail | high-cash evaluate/evaluate-and-persist, lifecycle transition, candidate detail/replay, AI explanation, advisor queue, review/feedback, conversion/report, readiness, source-ingestion, and outbox endpoints; first read-only Gateway publication exists for queue/detail |
...
| 12 | Advise and Manage conversion realization | Partially implemented - internal conversion governance, source-versioned outcomes, durable claim-before-call submission and operator reconciliation, certified APIs, source-safe adapters, readiness diagnostics, governed contract gates, digest-bound route source-contract consumption, bounded Advise idea-intake runtime-execution proof consumption, and bounded Manage action-intake runtime-execution proof consumption | Advise/Manage intent handoff now preserves uncertain outcomes without automatic retry and exposes audited source-safe recovery. Runtime route receipts may clear only the named live-route blockers; suitability/rebalance authority, action-register persistence, live downstream execution beyond bounded receipts, source-owned outcome proof, Gateway/Workbench product proof, client publication, and supported product promotion remain planned. |
| 13 | Report, Render, Archive, and evidence-pack materialization | Partially implemented - internal report evidence-pack requests, durable claim-before-call Report handoff, uncertain-outcome reconciliation, source-safe API/adapter foundations, readiness diagnostics, and governed contract gates | Local handoff posture is durable and recoverable without duplicate calls, but Report package creation, Render output, Archive authority, client publication, and supported product proof remain planned. |
| 14 | Data-product promotion, trust telemetry, and platform hardening | Partially implemented - internal not-certified mesh readiness, runtime telemetry preview, API-certified runtime snapshot diagnostic, generated source-safe runtime snapshot evidence, and runtime telemetry test-execution contract plus digest-bound mesh policy source-contract evidence | repo-native declarations, readiness diagnostic, runtime telemetry preview, runtime snapshot endpoint, generated runtime snapshot, runtime telemetry test-execution contract, blocked static trust telemetry, SLO/access/evidence policies; product promotion remains planned |
| 15 | Observability, security, entitlements, and operations | Partially implemented - bounded operation events plus readiness diagnostics, scheduled-worker source-contract and deployment-evidence posture, and source/outbox delivery run-once operator eventing with source-safe run identity | operation-event metric/log vocabulary, no-sensitive-label proof, endpoint instrumentation including candidate evidence replay, source-ingestion run-once, scheduled-worker source-contract and deployment-evidence validity, downstream submission, downstream-realization readiness, source-ingestion readiness, outbox-delivery readiness/run-once with operator-run replay/conflict posture, advisor-queue readiness, AI explanation readiness, data-mesh readiness, and aggregate implementation-proof readiness diagnostics; broader runbooks, certified long-running scheduling execution, and production telemetry remain planned |
| 16 | Demo readiness, archetype scenarios, and commercial proof | Partially implemented - proof-readiness diagnostic consumes the governed archetype/scenario contract as blocked scenario readiness; demo claims remain blocked | demo claim ledger, aggregate proof-readiness diagnostic, and opportunity archetype/scenario contract now identify blockers; live source-backed scenario replay, RFP-safe wording, and demo guide remain planned until live evidence exists |
| 17 | Implementation proof and live validation | Partially implemented - aggregate proof-readiness diagnostic and repo-native artifact generation include bounded live-source ingestion proof, independently classified scheduler source-contract and deployment-evidence inputs, durable repository proof consumption, runtime trust telemetry test-execution evidence consumption, Workbench read-path source-contract proof consumption, Gateway/Workbench contract proof consumption, Gateway/Workbench discovery contract proof consumption, bounded outbox broker source-contract proof consumption, default digest-bound Advise/Manage route source-contract consumption, bounded Advise and Manage intake runtime-execution proof consumption, default Report intake route and materialization source-contract consumption, default digest-bound mesh policy source-contract supporting-evidence consumption, default platform catalog source contract consumption, AI lineage store proof consumption, AI workflow-pack registration/runtime execution proof consumption, and optional closed runtime evidence across Risk, Performance, Core, Manage, and Advise families; full live journey proof remains pending | machine-readable proof blocker index exists through certified operator API and `make implementation-proof-readiness-check`. Scheduler source declarations are non-clearing `source_contract` evidence; only matching observed `deployment` evidence may clear the scheduler deployment blocker, and the default Make proof retains it. Runtime-class proofs clear only their named bounded blockers. Historical flat-v1 Performance artifacts no longer qualify after the closed v2 underperformance and benchmark-readiness contracts; fresh canonical capture and critical result review remain planned. |
| 18 | Documentation, wiki, support, and agent context | Partially implemented - API certification, implementation-proof, live-source, scheduled-worker source-contract and deployment-evidence, durable repository proof, runtime trust telemetry test-execution evidence, Workbench read-path source-contract proof, downstream route source contracts, Advise/Manage intake runtime-execution evidence, closed v3 Manage mandate runtime evidence, receipt-bound Core portfolio-state runtime evidence, outbox broker source-contract proof, mesh policy source-contract evidence, AI lineage store proof, AI workflow-pack proof, and outbox delivery documentation synchronized | README/wiki/context/runbook updates, API certification guide, implementation-proof-readiness guide, downstream-realization-readiness guide, persistence guide, source-ingestion runbook, scheduled-worker source-contract and deployment-evidence boundaries, durable repository proof boundary, runtime telemetry test-execution evidence boundary, Workbench read-path source-contract proof boundary, Advise/Manage/Report route source-contract boundaries, Advise/Manage intake runtime-execution boundaries, Manage mandate runtime-evidence boundary, Core portfolio-state runtime-evidence boundary, outbox broker source-contract proof boundary, mesh policy source-contract boundary, AI lineage store proof boundary, AI workflow-pack registration/runtime execution proof boundary, outbox delivery boundaries, client-demo process, and wiki source updates; wiki check-only remains required before merge |
| 19 | Second-last hardening and review | Partially implemented - quality scorecard, CI/release, API, security, data-access, observability, resilience, operator-action, supported-feature reconciliation, and queue snapshot drift controls are enforced | code/API/security/data-mesh/docs review and remaining issue fixes still precede closure |
...
Issue `#443` hardens Slice 14/17 evidence without advancing runtime mesh
certification. The platform catalog artifact is now a digest-bound, closed-field
v2 `source_contract` under capability-owned `data_mesh/` packages. It can
satisfy only source-manifest and catalog-inclusion blockers; runtime
publication, policy and platform certification, product activation,
Gateway/Workbench discovery, deployment, production certification, and
