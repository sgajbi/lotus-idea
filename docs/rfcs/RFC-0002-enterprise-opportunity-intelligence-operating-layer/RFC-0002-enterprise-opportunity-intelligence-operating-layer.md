...
| 9 | Governed AI explanation and model-risk controls | Partially implemented - internal AI governance, governed workflow-pack allowlist, signed Lotus AI run-attestation verification, durable replay protection, certified API foundation, source-safe lineage persistence, and not-certified readiness diagnostic | redacted AI request envelopes, fallback/verifier/attested-output API, bounded verified receipt, safe audit, no-authority proof, `lotus-ai:idea-explanation:v1` request identity mapped to `idea_explanation.pack@v1` proof identity; mainline cross-repository and live-provider proof remain blocked |
