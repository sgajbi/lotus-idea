...
| 5 | Deterministic signal evaluation and candidate generation | Partially implemented - high-cash domain policy plus Core source-port and manifest-backed run-once ingestion worker foundation | first signal families, candidate generation, golden scenario tests, Core source-port orchestration, bounded run-once ingestion worker semantics, manifest-backed worker check-only contract |
| 6 | Persistence, replay, idempotency, and audit | Partially implemented - internal persistence plus certified evidence replay API, schema, rollback, migration execution, PostgreSQL adapter, opt-in API repository wiring, first PostgreSQL runtime workflow proof, source-ingestion replay/conflict recovery proof, manifest-backed run-once ingestion worker CLI/check, source-safe outbox publisher adapter foundation, certified outbox delivery readiness diagnostic, and migration rollback/reapply recovery proof | internal records, hashes, replay posture, audit events, idempotent high-cash persist, candidate evidence replay, and lifecycle transition orchestration, schema/rollback contract, migration dry-run and execution CLI, tested PostgreSQL repository adapter, opt-in API provider selection through `LOTUS_IDEA_DATABASE_URL`, real PostgreSQL persistence/replay proof for the high-cash persist API plus the first review/feedback/conversion/report workflow path, internal source-ingestion replay/conflict recovery proof, manifest-backed run-once source-ingestion worker CLI/check, internal outbox retry/dead-letter delivery state, HTTP publisher adapter foundation, certified aggregate outbox delivery readiness diagnostic, and real PostgreSQL schema rollback/reapply recovery proof; live source adapters, scheduled daemon/deploy workers, certified live broker runtime, downstream proof, and supported promotion remain planned |
...
| 9 | Governed AI explanation and model-risk controls | Partially implemented - internal AI governance and certified API foundation only | redacted AI request envelopes, fallback/verifier API, safe audit, no-authority proof; `lotus-ai` runtime execution remains planned |
| 10 | Certified APIs, OpenAPI, and Gateway contract | Partially implemented - certified internal API foundations plus bounded read-only Gateway publication for advisor queue and candidate detail | high-cash evaluate/evaluate-and-persist, lifecycle transition, candidate detail, candidate evidence replay, AI explanation, advisor queue, review-action, feedback, conversion intent/outcome, report evidence-pack, downstream-realization readiness, data-mesh readiness, and source-ingestion readiness endpoints; first read-only Gateway publication exists for queue/detail |
| 11 | Workbench product realization | Planned - Gateway read publication foundation exists; Workbench proof remains pending | review queue UI, evidence drawer, browser/accessibility proof |
| 12 | Advise and Manage conversion realization | Partially implemented - internal conversion governance, certified API foundation, source-safe downstream adapter foundations, downstream readiness blocker diagnostic, and governed contract-plan gate | review-gated conversion intents/outcomes, source-authority mapping, certified internal conversion API proof, first PostgreSQL-backed report conversion intent/outcome proof, no downstream authority proof, source-safe Advise/Manage handoff envelopes, manifest-backed downstream readiness blocker reporting, and `make downstream-realization-contract-gate`; live contracts remain planned |
| 13 | Report, Render, Archive, and evidence-pack materialization | Partially implemented - internal report evidence-pack request foundation, source-safe downstream adapter foundation, downstream readiness blocker diagnostic, and governed contract-plan gate | certified internal request API, source summaries, retention refs, idempotency/audit proof, first PostgreSQL-backed internal request proof, source-safe Report evidence-pack request handoff envelope, manifest-backed Report/Render/Archive blocker reporting, and `make downstream-realization-contract-gate`; downstream materialization remains planned |
| 14 | Data-product promotion, trust telemetry, and platform hardening | Partially implemented - internal not-certified mesh readiness and runtime telemetry preview diagnostics | repo-native declarations, readiness diagnostic, runtime telemetry preview, blocked static trust telemetry, SLO/access/evidence policies; product promotion remains planned |
| 15 | Observability, security, entitlements, and operations | Partially implemented - bounded operation events plus readiness diagnostics | operation-event metric/log vocabulary, no-sensitive-label proof, endpoint instrumentation including candidate evidence replay, downstream-realization readiness, source-ingestion readiness, outbox-delivery readiness, advisor-queue readiness, AI explanation readiness, data-mesh readiness, and aggregate implementation-proof readiness diagnostics; broader runbooks, live source proof, and production telemetry remain planned |
...
| 17 | Implementation proof and live validation | Partially implemented - aggregate proof-readiness diagnostic and repo-native artifact generation include outbox-delivery blockers; live proof remains pending | machine-readable proof blocker index exists through certified operator API and `make implementation-proof-readiness-check`, including source-safe outbox delivery proof-family blockers; live canonical evidence and critical result review remain planned |
| 18 | Documentation, wiki, support, and agent context | Partially implemented - API certification, implementation-proof, and outbox delivery documentation synchronized | README/wiki/context/runbook updates, API certification guide, implementation-proof-readiness guide, outbox delivery boundaries, and wiki source updates; wiki check-only remains required before merge |
