...
| Bond maturity or reinvestment | `lotus-core`; optional `lotus-manage` context | Fixed-income maturity creates reinvestment or liquidity review need. | Partially implemented - deterministic policy and Core source-port shape only; governed Core maturity source contract/live proof remains blocked |
