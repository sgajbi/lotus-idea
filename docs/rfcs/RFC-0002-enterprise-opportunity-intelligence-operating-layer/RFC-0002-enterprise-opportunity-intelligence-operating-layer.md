...
| Bond maturity or reinvestment | `lotus-core`; optional `lotus-manage` context | Fixed-income maturity creates reinvestment or liquidity review need. | Partially implemented - deterministic policy plus bounded caller-supplied API foundation and fail-closed Core maturity-summary source-adapter contract pending explicit Core-owned maturity facts (`sgajbi/lotus-core#686`); data-mesh certification, Workbench proof, product recommendation, reinvestment advice, maturity schedule authority, order execution, live Core maturity proof, and supported promotion remain blocked |
