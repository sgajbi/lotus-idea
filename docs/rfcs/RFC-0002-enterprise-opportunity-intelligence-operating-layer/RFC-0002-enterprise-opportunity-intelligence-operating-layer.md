...
| 6 | Persistence, replay, idempotency, and audit | Partially implemented - internal persistence, replay/audit, governed hold/erasure/purge and lifecycle telemetry, durable claim-before-call downstream recovery, source-safe operator reconciliation, outbox retry/dead-letter/re-drive, migration execution, PostgreSQL adapters, real logical backup/restore and resume proof, provider-restore validation, recovery-aware write gating, and bounded broker/consumer/mesh proof foundations | versioned schema/rollback through lifecycle migration `009`; candidate/review/feedback/conversion/report persistence; source ingestion and outbox recovery; opaque support references; lease-fenced finalize; uncertain-outcome quarantine; governed lifecycle API and tombstones; real PostgreSQL restart/concurrency/connection-failure proof; and a weekly attested 17-table restore drill. Physical/WAL provider certification, signed lifecycle authority integration, Report/Archive/AI retention conformance, scheduled purge proof, live source certification, certified external publication, downstream execution/materialization, Gateway/Workbench product proof, and supported promotion remain planned. |
