...
| 6 | Persistence, replay, idempotency, and audit | Partially implemented - internal persistence plus schema, rollback, migration execution, and PostgreSQL adapter foundations only | internal records, hashes, replay posture, audit events, idempotent high-cash persist and lifecycle transition orchestration, schema/rollback contract, migration dry-run and execution CLI, tested PostgreSQL repository adapter foundation; database-backed API state remains planned |
