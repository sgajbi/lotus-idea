...
| 10 | Certified APIs, OpenAPI, and Gateway contract | Partially implemented - certified internal API foundations including source-safe candidate detail, evidence replay, AI explanation evaluator, and operator diagnostics only | high-cash evaluate/evaluate-and-persist, lifecycle transition, candidate detail, candidate evidence replay, AI explanation, advisor queue, review-action, feedback, conversion intent/outcome, report evidence-pack, data-mesh readiness, and source-ingestion readiness endpoints; Gateway route contract remains planned |
...
| 15 | Observability, security, entitlements, and operations | Partially implemented - bounded operation events plus evidence replay and source-ingestion readiness diagnostics | operation-event metric/log vocabulary, no-sensitive-label proof, endpoint instrumentation including candidate evidence replay, and source-ingestion configuration/certification blocker diagnostic; broader runbooks, live source proof, and production telemetry remain planned |
