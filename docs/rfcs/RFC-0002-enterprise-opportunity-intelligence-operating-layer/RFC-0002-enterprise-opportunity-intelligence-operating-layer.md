...
| 6 | Persistence, replay, idempotency, and audit | Partially implemented - internal persistence plus schema, rollback, migration execution, PostgreSQL adapter, and opt-in API repository wiring | internal records, hashes, replay posture, audit events, idempotent high-cash persist and lifecycle transition orchestration, schema/rollback contract, migration dry-run and execution CLI, tested PostgreSQL repository adapter, opt-in API provider selection through `LOTUS_IDEA_DATABASE_URL`; production storage proof and supported promotion remain planned |
