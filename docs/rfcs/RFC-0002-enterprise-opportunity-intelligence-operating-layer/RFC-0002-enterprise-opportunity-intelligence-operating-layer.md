...
| Bond maturity or reinvestment | `lotus-core`; optional `lotus-manage` context | Fixed-income maturity creates reinvestment or liquidity review need. | Partially implemented - deterministic policy plus Core `HoldingsAsOf:v1` maturity-date source-port/adapter/live-proof and bounded caller-supplied API foundation; data-mesh certification, Workbench proof, product recommendation, reinvestment advice, maturity schedule authority, order execution, and supported promotion remain blocked |
