...
| 7 | Scoring, ranking, suppression, and queue policy | Partially implemented - deterministic scoring plus certified advisor queue API foundation and durable repository-side page projection | deterministic score inputs/policy, reason codes, stable queue projection, snapshot orchestration for in-memory/proof paths, repository-side PostgreSQL candidate page projection, snooze/suppression/dedupe tests, advisor queue API proof; Workbench, data-product certification, PM/compliance queues, and supported promotion remain planned |
