...
8. `src/app/api/high_volatility_signals.py` exposes
   ...
   volatility evidence. It requires advisor role and `idea.signal.evaluate`
   capability, rejects non-Risk or wrong-product source refs before candidate
   generation, emits bounded operation events, redacts source route/hash fields
   from candidate responses, and does not fetch Risk sources, calculate
   volatility, approve Risk methodology, recommend trades, create rebalance
   actions, publish client communication, certify data products, prove
...
10. This remains an internal design-module foundation inside the existing
   `lotus-idea` service. It does not add a separately scalable process,
   certify live Advise source support, approve suitability, approve policy,
   approve proposals, approve sign-off, publish client communication, prove
   Gateway/Workbench behavior, promote a data product, or promote a supported
   feature.
...
4. The application layer also consumes Lotus Advise
   ...
   candidates only when `advise_diagnostic` carries explicit restriction or
   mandate diagnostic codes such as `mandate_restriction_review_required`,
   `product_restriction_review_required`, `country_restriction_review_required`,
   `mandate_breach`, or `restriction_breached`.
5. Generic open suitability or policy requirements remain owned by the missing
   suitability context path; they do not create mandate/restriction candidates
   unless Advise emits explicit mandate/restriction diagnostic posture.
...
10. This remains an internal design-module foundation inside the existing
    `lotus-idea` service. It does not add a separately scalable process,
    certify live Advise source support, clear restrictions, change mandate
    state, determine suitability, approve policy, approve proposals, recommend
    trades, create rebalance actions, create orders, publish client
    communication, prove Gateway/Workbench behavior, promote a typed
    restriction data product, or promote a supported feature.
...
11. `idea-source-temporal-v1` now explicitly declares the temporal policy for
    ...
    `source_generated_after_evaluation` reason codes. Caller-supplied DTOs,
    source-backed APIs, and persisted source ingestion share this domain
    policy; no candidate is created or persisted from misaligned evidence.
12. The same scan closed an allocation-drift gap: optional Performance and Risk
...
10. `python -m pytest tests\integration\test_underperformance_signal_api.py tests\integration\test_signal_source_operation_events.py::test_underperformance_source_api_emits_blocked_operation_event tests\unit\test_source_ingestion_state.py tests\unit\test_lotus_performance_sources.py tests\unit\test_service_contract.py -q`
   covers underperformance Lotus Performance-backed evaluation, portfolio
   entitlement denial before runtime construction, missing Performance runtime
   configuration, Performance source unavailability, route-owned runtime
...
11. `python -m pytest tests\integration\test_allocation_drift_signal_api.py tests\integration\test_signal_source_operation_events.py::test_allocation_drift_source_api_emits_blocked_operation_event tests\unit\test_source_ingestion_state.py tests\unit\test_lotus_manage_sources.py tests\unit\test_mandate_health_application.py tests\unit\test_service_contract.py -q`
   ...
   endpoint-ledger registration, and bounded operation-event proof.
12. `python -m pytest tests\integration\test_missing_suitability_signal_api.py tests\integration\test_signal_source_operation_events.py::test_missing_suitability_source_api_emits_blocked_operation_event tests\unit\test_source_ingestion_state.py tests\unit\test_lotus_advise_sources.py tests\unit\test_missing_suitability_application.py tests\unit\test_service_contract.py -q`
   covers missing-suitability Lotus Advise-backed evaluation, request
   access-scope entitlement denial before runtime construction, missing Advise
   runtime configuration, Advise source unavailability, route-owned runtime
...
13. `python -m pytest tests\integration\test_missing_risk_profile_signal_api.py tests\integration\test_signal_source_operation_events.py::test_missing_risk_profile_source_api_emits_blocked_operation_event tests\unit\test_source_ingestion_state.py tests\unit\test_lotus_advise_sources.py tests\unit\test_missing_risk_profile_application.py tests\unit\test_service_contract.py -q`
   ...
   source runtime construction, adapter close delegation, endpoint-ledger
   registration, and bounded operation-event proof.
14. `python -m pytest tests\integration\test_mandate_restriction_signal_api.py tests\integration\test_signal_source_operation_events.py::test_mandate_restriction_source_api_emits_blocked_operation_event tests\unit\test_mandate_restriction_application.py tests\unit\test_source_ingestion_state.py tests\unit\test_lotus_advise_sources.py tests\unit\test_service_contract.py -q`
   covers mandate/restriction Lotus Advise-backed evaluation, explicit
   restriction diagnostic consumption, generic suitability/policy diagnostic
   suppression, request access-scope entitlement denial before runtime
   construction, missing Advise runtime configuration, Advise source
   unavailability, route-owned runtime cleanup, manifest-free Advise source
   runtime construction, adapter close delegation, endpoint-ledger
   registration, and bounded operation-event proof.
15. This closes only the bounded high-cash, low-income, bond-maturity,
   missing-benchmark, concentration-risk, high-volatility, drawdown-review,
   underperformance, allocation-drift, missing-suitability,
   missing-risk-profile, and mandate/restriction source-fetching API
   foundations.
   ...
   compliance approval, mandate-state change, restriction clearance,
   risk-profile approval, suitability approval, policy approval, proposal
