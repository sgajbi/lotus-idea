...
| Bond maturity / reinvestment | `lotus-core:PortfolioStateSnapshot:v1`, `lotus-core:HoldingsAsOf:v1`, future maturity-specific source product if required | `lotus-core` owns instrument/holding maturity facts. | Partially implemented as a bounded caller-supplied maturity-evidence foundation. Live Core source proof is blocked until Core exposes explicit source-owned maturity summary facts (`sgajbi/lotus-core#686`); `lotus-idea` must not derive maturity dates or counts from raw positions and creates no product recommendation, reinvestment advice, suitability approval, risk approval, Workbench proof, data-mesh certification, client publication, or supported-feature promotion. |
