...
| Concentration | `lotus-risk:RiskMetricsReport:v1`; bounded source-adapter foundation added for `lotus-risk:ConcentrationRiskReport:v1` | `lotus-risk` owns concentration methodology, risk metrics, issuer/position calculations, and risk supportability. | Use risk-owned concentration evidence as an opportunity input without recomputing concentration; Lotus Risk source approval for `ConcentrationRiskReport:v1` is in place, while live proof, data-mesh certification, and supported promotion remain blocked until proven. |
...
| Bond maturity / reinvestment | `lotus-core:HoldingsAsOf:v1`, `lotus-core:PortfolioMaturitySummary:v1` | `lotus-core` owns instrument/holding maturity facts and maturity-summary posture. | Partially implemented as a bounded caller-supplied maturity-evidence foundation and Core source-adapter consumer of `PortfolioMaturitySummary:v1`. Live Core source proof remains blocked until canonical runtime proof is captured and merged; `lotus-idea` must not derive maturity dates or counts from raw positions and creates no product recommendation, reinvestment advice, suitability approval, risk approval, Workbench proof, data-mesh certification, client publication, or supported-feature promotion. |
| Low income / liquidity shortfall | `lotus-core:PortfolioCashflowProjection:v1`, `lotus-core:PortfolioCashMovementSummary:v1`, future client-income-needs products if required | `lotus-core` owns cashflow/liquidity evidence; financial-planning advice remains out of scope unless a source owner supplies a governed product. | Partially implemented as a bounded Core cashflow foundation: deterministic projected-cumulative-cashflow thresholding can create only an advisor-review candidate, with no client-income-needs, funding-advice, treasury-instruction, suitability, or planning-objective inference. |
...
| Mandate or restriction review | `lotus-core:PortfolioStateSnapshot:v1`, `lotus-manage:PortfolioActionRegister:v1`, `lotus-advise:AdvisoryPolicyEvaluationRecord:v1` | `lotus-core`, `lotus-manage`, and `lotus-advise` own mandate, product, country, suitability-policy, and actionability restriction posture. | Partially implemented as a bounded policy/API foundation over caller-supplied source refs; do not change mandate state, clear restrictions, approve suitability, create orders, or publish client communication. |
