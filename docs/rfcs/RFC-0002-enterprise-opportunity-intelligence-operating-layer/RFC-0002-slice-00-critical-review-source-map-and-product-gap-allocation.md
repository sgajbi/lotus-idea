...
| Concentration | `lotus-risk:RiskMetricsReport:v1`; bounded source-adapter foundation added for `lotus-risk:ConcentrationRiskReport:v1` | `lotus-risk` owns concentration methodology, risk metrics, issuer/position calculations, and risk supportability. | Use risk-owned concentration evidence as an opportunity input without recomputing concentration; keep live proof, upstream consumer approval, data-mesh certification, and supported promotion blocked until proven. |
