...
| Bond maturity / reinvestment | `lotus-core:PortfolioStateSnapshot:v1`, `lotus-core:HoldingsAsOf:v1`, future maturity-specific source product if required | `lotus-core` owns instrument/holding maturity facts. | Partially implemented as a bounded Core HoldingsAsOf maturity foundation: deterministic maturity-window thresholding can create only an advisor-review candidate from Core source refs, with no product recommendation, reinvestment advice, suitability approval, risk approval, Workbench proof, data-mesh certification, client publication, or supported-feature promotion. |
