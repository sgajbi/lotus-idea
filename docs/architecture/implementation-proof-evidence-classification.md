...
| Risk drawdown runtime execution | Closed v2 `runtime_execution` evidence invokes the authoritative drawdown evaluation-and-persistence use case and binds one exact current `lotus-risk:DrawdownAnalyticsReport:v1` receipt to one accepted or replayed durable Idea persistence receipt. Request, source, evidence, scope, timestamp, provenance, and persistence digests must reconcile. Unknown fields, source substitution, stale or mismatched evidence, non-candidate outcomes, in-memory execution, missing persistence, conflicts, and receipt tampering clear no blocker. A valid current artifact affects only `opportunity_archetype_drawdown_source_proof_missing`; volatility, mesh, Gateway/Workbench, publication, deployment, production, and promotion blockers remain. | #466 fixed locally; full CI, required PostgreSQL execution, merge, exact-main validation, and wiki publication remain. |
| Performance underperformance runtime execution | The current flat v1 summary can self-assert Performance source execution, freshness, and benchmark context and clear a blocker without an authoritative durable persistence receipt. It remains untrusted until replaced by closed, receipt-bound `runtime_execution` evidence while official performance and benchmark authority remain in Lotus Performance. | #469, next separately bounded occurrence after #466 |
