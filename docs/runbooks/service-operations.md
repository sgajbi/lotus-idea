...
| `make manage-mandate-live-proof-contract-gate` | Closed v2 Lotus Manage mandate runtime-evidence contract. It binds a named source-evaluation use case, exact action-register and mandate-health receipts, deterministic candidate/no-opportunity outcome, and fails closed on missing producer trust metadata without Core portfolio-state, mesh, Workbench, publication, rebalance, action, order, or supported-feature promotion. |
| `make mandate-restriction-live-proof-contract-gate` | Closed v2, receipt-bound Lotus Advise mandate/restriction runtime-evidence contract. It validates candidate and no-opportunity outcomes, fails closed without producer-owned scope/time, and grants no restriction, mandate, suitability, policy, publication, data-mesh, Workbench, rebalance, order, deployment, production, or support authority. |
...
| `make core-portfolio-state-live-proof-contract-gate` | Closed v2 Lotus Core portfolio-state runtime-evidence contract. It binds a named read-only use case and complete current `PortfolioStateSnapshot:v1` receipt, fails closed on missing producer trust metadata, and cannot promote Manage, Performance, Risk, mesh, Workbench, publication, deployment, production, or supported-feature posture. |
| `make bond-maturity-live-proof-contract-gate` | Closed v3 Core bond-maturity runtime contract. It binds current `PortfolioMaturitySummary:v1` and upstream `HoldingsAsOf:v1` evidence to authoritative scope and accepted or exact-replayed durable Idea persistence. Empty, non-durable, missing-persistence, incoherent, or tampered execution cannot clear the live-source blocker or promote support. |
...
| `make missing-risk-profile-live-proof-contract-gate` | Closed v2 Lotus Advise missing-risk-profile runtime-execution contract. It binds one source-preserving application result and one Advise fetch to pseudonymous request scope, producer workflow scope/time/hashes/posture, and a deterministic candidate or truthful no-opportunity receipt; it grants no risk-profile, suitability, policy, proposal, sign-off, publication, typed-product, mesh, Workbench, deployment, production, or promotion authority. |
| `make missing-benchmark-live-proof-contract-gate` | Closed v2 Core missing-benchmark runtime-execution contract. It binds one source-preserving application result and one Core fetch to pseudonymous request scope, exact current benchmark-assignment posture, and a deterministic candidate or ready-assignment no-opportunity receipt. It grants no benchmark-assignment, Performance, methodology, mesh, Gateway/Workbench, publication, deployment, production, or promotion authority. |
| `make missing-benchmark-performance-readiness-proof-contract-gate` | Closed v2 Lotus Performance benchmark-readiness runtime-execution contract. It binds one source-preserving use case and one exact `ReturnsSeriesBundle:v1` fetch to pseudonymous scope, provenance hashes, benchmark context, coverage, freshness/quality, producer correlation/trace, and deterministic review-required or no-opportunity receipts. It grants no benchmark-assignment, official performance, methodology, data-mesh, Gateway/Workbench, publication, deployment, production, or promotion authority. |
| `make opportunity-archetype-evidence-pack-gate` | Source-safe RFC-0002 Slice 16 canonical opportunity archetype evidence-pack contract. It hashes the canonical portfolio reference, binds archetypes to source products, evidence refs, blocker issue refs, and readiness blockers, and grants no source calculation, client publication, data-mesh, production, Workbench, or supported-feature authority. |
...
11. For downstream realization blocker checks, call
   ...
   local downstream submission denominator, reconciliation workload,
   planned Advise/Manage/Report handoff contract
