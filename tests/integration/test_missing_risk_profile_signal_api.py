...
class RecordingAdvisePolicyEvaluationSource:
    ...
    def fetch_policy_evaluation_evidence(
        ...
        if self.error is not None:
            raise self.error
        return _advise_risk_profile_evidence(advise_diagnostic=self.advise_diagnostic)
...
def test_missing_risk_profile_signal_from_source_api_returns_review_candidate(
    ...
    assert advise_source.seen_request == AdvisePolicyEvaluationEvidenceRequest(
        ...
        correlation_id="corr-advise-missing-risk-profile-source-api",
        trace_id="trace-advise-missing-risk-profile-source-api",
    )
...
def test_missing_risk_profile_signal_from_source_blocks_when_runtime_not_configured(
    ...
    assert response.json() == {
        ...
        "detail": (
            "Advise source runtime is not configured for missing-risk-profile source evaluation."
        ),
    }
...
def source_evaluation_headers(*, portfolio_ids: str = "PB_SG_GLOBAL_BAL_001") -> dict[str, str]:
    return {
        ...
        "X-Caller-Roles": "advisor",
        "X-Caller-Capabilities": "idea.signal.evaluate",
        "X-Correlation-Id": "corr-advise-missing-risk-profile-source-api",
        "X-Trace-Id": "trace-advise-missing-risk-profile-source-api",
...
def missing_risk_profile_source_payload() -> dict[str, Any]:
    return {
...
def _advise_risk_profile_evidence(
    *,
    advise_diagnostic: str = "risk_profile_missing",
) -> AdvisePolicyEvaluationEvidence:
