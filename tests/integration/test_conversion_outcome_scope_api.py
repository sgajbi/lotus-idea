...
def test_conversion_outcome_api_denies_incomplete_or_mismatched_candidate_scope(
    ...
    intent_id = _seed_outcome_ready_intent(client, suffix="denial")
    repository = get_idea_repository()
    before = repository.snapshot()
    headers = conversion_outcome_headers("outcome-scope-denial")
    ...
    response = client.post(
        ...
        json=conversion_outcome_payload(conversion_outcome_id="outcome-scope-denial"),
        headers=headers,
    )
...
def test_conversion_outcome_api_accepts_matching_multi_value_scope_and_replay() -> None:
    client = managed_test_client(app)
    intent_id = _seed_outcome_ready_intent(client, suffix="multi")
    ...
    payload = conversion_outcome_payload(conversion_outcome_id="outcome-multi-scope")

    accepted = client.post(
        f"/api/v1/conversion-intents/{intent_id}/outcomes", json=payload, headers=headers
    )
...
def test_conversion_outcome_api_rejects_malformed_scope_without_mutation() -> None:
    client = managed_test_client(app)
    intent_id = _seed_outcome_ready_intent(client, suffix="malformed")
    repository = get_idea_repository()
    before = repository.snapshot()
    ...
    response = client.post(
        f"/api/v1/conversion-intents/{intent_id}/outcomes",
        json=conversion_outcome_payload(conversion_outcome_id="outcome-malformed-scope"),
        headers=headers,
    )
...
def _seed_outcome_ready_intent(client: ManagedTestClient, *, suffix: str) -> str:
    reset_idea_repository_for_tests()
    candidate_id = persisted_candidate_id(client, idempotency_key=f"seed-outcome-scope-{suffix}")
    approve_candidate_for_conversion(client, candidate_id)
    intent_id = f"conversion-outcome-scope-{suffix}"
    response = client.post(
        f"/api/v1/idea-candidates/{candidate_id}/conversion-intents",
        json=conversion_intent_payload(conversion_intent_id=intent_id),
        headers=conversion_intent_headers(f"intent-outcome-scope-{suffix}"),
    )
