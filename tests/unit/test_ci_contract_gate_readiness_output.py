from __future__ import annotations

import importlib.util
from pathlib import Path
from types import ModuleType


ROOT = Path(__file__).resolve().parents[2]


def _load_ci_contract_gate() -> ModuleType:
    script_path = ROOT / "scripts" / "ci_contract_gate.py"
    spec = importlib.util.spec_from_file_location("ci_contract_gate", script_path)
    assert spec is not None
    assert spec.loader is not None
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_ci_contract_gate_blocks_missing_implementation_proof_output_wiring() -> None:
    module = _load_ci_contract_gate()
    makefile = (
        (ROOT / "Makefile")
        .read_text(encoding="utf-8")
        .replace(
            "--output $(IMPLEMENTATION_PROOF_OUTPUT) ",
            "",
        )
    )

    errors = module.validate_makefile(makefile)

    assert (
        "Makefile implementation-proof-readiness-check target must "
        "write the aggregate implementation proof readiness output artifact"
    ) in errors


def test_ci_contract_gate_blocks_missing_default_implementation_proof_output_path() -> None:
    module = _load_ci_contract_gate()
    makefile = (
        (ROOT / "Makefile")
        .read_text(encoding="utf-8")
        .replace(
            "LOTUS_IDEA_IMPLEMENTATION_PROOF_READINESS_OUTPUT ?= "
            "output/implementation-proof/readiness-current.json\n",
            "",
        )
    )

    errors = module.validate_makefile(makefile)

    assert (
        "Makefile implementation-proof-readiness-check target must define the default "
        "aggregate implementation proof readiness output artifact"
    ) in errors
