...
from app.application import outbox_consumer_runtime_proof as proof_module
...
def test_outbox_consumer_runtime_proof_rejects_missing_make_target(
    ...
    (tmp_path / "Makefile").write_text(
        "outbox-consumer-contract-gate:\n\tpython scripts/outbox_consumer_contract_gate.py\n",
        encoding="utf-8",
    )
...
def _load_generator_script() -> ModuleType:
    script_path = ROOT / "scripts" / "generate_outbox_consumer_runtime_proof.py"
...
def _load_contract_gate_script() -> ModuleType:
    script_path = ROOT / "scripts" / "outbox_consumer_runtime_proof_contract_gate.py"
    spec = importlib.util.spec_from_file_location(
