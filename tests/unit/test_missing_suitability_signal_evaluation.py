...
def test_missing_suitability_source_correction_preserves_candidate_and_versions_evidence() -> None:
    source_input = suitability_input()
    assert source_input.policy_ref is not None
    corrected_input = replace(
        source_input,
        policy_ref=replace(
            source_input.policy_ref,
            content_hash="sha256:aa4b5c3fa9b136ba22c19cad47ae89f5c2a0b15a3d057e8ce1c65535c9551c9d",
        ),
    )

    original = evaluate_missing_suitability_context_signal(source_input, policy())
    corrected = evaluate_missing_suitability_context_signal(corrected_input, policy())
