...
def test_builds_source_safe_ai_workflow_pack_registration_proof() -> None:
    ...
    assert tuple(proof["evidenceRefs"]) == REQUIRED_AI_WORKFLOW_PACK_EVIDENCE_REFS
    ...
    assert ai_workflow_pack_registration_proof_is_valid(proof) is True
...
def test_rejects_ai_workflow_pack_registration_proof_when_ai_evidence_is_missing(
    tmp_path: Path,
) -> None:
    proof = build_ai_workflow_pack_registration_proof_payload(
        generated_at_utc=datetime(2026, 6, 25, 0, 0, tzinfo=UTC),
...
def test_ai_workflow_pack_registration_proof_cli_allows_missing_evidence(
    tmp_path: Path,
) -> None:
    module = _load_generator_script()
    output_path = tmp_path / "proof" / "missing-ai-proof.json"
...
def test_ai_workflow_pack_registration_proof_contract_gate_scans_tuple_content() -> None:
...
def test_ai_workflow_pack_registration_proof_contract_gate_allows_missing_sibling_evidence(
    tmp_path: Path,
) -> None:
    module = _load_contract_gate_script()

    errors = module.validate_ai_workflow_pack_registration_proof_contract(
        lotus_ai_root=tmp_path / "missing-lotus-ai"
    )
