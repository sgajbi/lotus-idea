...
def test_github_issue_closure_matrix_gate_requires_critical_workflow_e2e_issue(
    tmp_path: Path,
) -> None:
    module = _load_gate()
    matrix = tmp_path / "matrix.md"
    ...
    assert "Missing actionable issue rows: #650" in errors


def test_github_issue_closure_matrix_gate_requires_proof_artifacts_refs_issue(
    tmp_path: Path,
) -> None:
    module = _load_gate()
    matrix = tmp_path / "matrix.md"
    ...
    assert "Missing actionable issue rows: #652" in errors


def test_github_issue_closure_matrix_gate_requires_proof_readiness_capability_issue(
    tmp_path: Path,
) -> None:
    module = _load_gate()
    matrix = tmp_path / "matrix.md"
...
def test_github_issue_closure_matrix_gate_requires_recent_local_issue_rows(
    ...
    for issue_number in (663, 864, 866):
