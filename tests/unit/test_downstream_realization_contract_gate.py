...
def test_downstream_intake_wire_contract_rejects_envelope_and_security_drift(
    ...
    assert "downstream intake wire contract must be version 1.10.0" in errors
...
def test_downstream_realization_contract_gate_blocks_report_intake_wire_contract_drift(
    ...
    target.parent.mkdir(parents=True)
    payload = json.loads(source.read_text(encoding="utf-8"))
    payload["consumer_contracts"][2]["boundary"] = "request_only"
    target.write_text(json.dumps(payload), encoding="utf-8")

    errors = module._validate_downstream_intake_wire_contract(tmp_path)

    assert "report_evidence intake wire contract boundary drifted" in errors


def test_downstream_realization_contract_gate_blocks_report_materialization_payload_drift(
    tmp_path: Path,
) -> None:
    module = _load_downstream_realization_contract_gate()
    source = ROOT / module.DOWNSTREAM_INTAKE_WIRE_CONTRACT_PATH
    ...
    target.parent.mkdir(parents=True)
    payload = json.loads(source.read_text(encoding="utf-8"))
    payload["consumer_contracts"][2]["idea_evidence_pack_fields"] = []
    target.write_text(json.dumps(payload), encoding="utf-8")

    errors = module._validate_downstream_intake_wire_contract(tmp_path)

    assert "report_evidence intake wire contract idea_evidence_pack_fields drifted" in errors
