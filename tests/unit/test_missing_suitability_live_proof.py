...
def test_missing_suitability_live_proof_payload_is_source_safe_and_not_promoted() -> None:
    ...
    assert payload["schemaVersion"] == MISSING_SUITABILITY_LIVE_PROOF_SCHEMA_VERSION
    assert payload["runStatus"] == "completed"
    assert payload["candidateGenerated"] is True
    assert payload["sourceEvidenceCurrent"] is True
    assert payload["clientReadyPublicationBlocked"] is True
    assert payload["advisePolicyWorkflowReady"] is True
    ...
    assert missing_suitability_live_proof_is_valid(payload) is True
...
def test_blocked_missing_suitability_live_proof_does_not_validate() -> None:
    payload = build_missing_suitability_live_proof_payload(
        generated_at_utc=GENERATED_AT,
        live_advise_source_attempted=True,
        evaluation_summary={
    ...
    assert "advise_policy_source_run_blocked" in payload["proofBlockers"]
    assert "source_error_advise_policy_workflow_unavailable" in payload["proofBlockers"]
    assert "advise_policy_source_evidence_not_current" in payload["proofBlockers"]
    assert "client_publication_boundary_not_blocked" in payload["proofBlockers"]
    assert "advise_policy_workflow_not_ready" in payload["proofBlockers"]
    assert "no_missing_suitability_context_candidate_generated" in payload["proofBlockers"]
    assert missing_suitability_live_proof_is_valid(payload) is False


def test_missing_suitability_live_proof_records_missing_live_source_attempt() -> None:
    payload = build_missing_suitability_live_proof_payload(
        generated_at_utc=GENERATED_AT,
    ...
    assert "advise_policy_live_source_proof_missing" in payload["proofBlockers"]
    assert missing_suitability_live_proof_is_valid(payload) is False


def test_missing_suitability_live_proof_rejects_clear_advise_context() -> None:
    payload = build_missing_suitability_live_proof_payload(
        generated_at_utc=GENERATED_AT,
        live_advise_source_attempted=True,
        evaluation_summary={
            ...
            "clientReadyPublicationBlocked": True,
            "advisePolicyWorkflowReady": True,
            "sourceDiagnosticCodes": ["advise_policy_context_available"],
...
def test_missing_suitability_live_proof_cli_writes_source_safe_artifact(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    module = _load_live_proof_script()
    output_path = tmp_path / "missing-suitability-live-proof.json"
...
def test_missing_suitability_live_proof_cli_writes_blocked_artifact(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    module = _load_live_proof_script()
...
def _load_live_proof_script() -> ModuleType:
    script_path = ROOT / "scripts" / "generate_missing_suitability_live_proof.py"
    spec = importlib.util.spec_from_file_location(
        "generate_missing_suitability_live_proof",
        script_path,
    )
