...
def test_github_issue_closure_matrix_gate_requires_scoring_issue(
    tmp_path: Path,
) -> None:
    module = _load_gate()
    matrix = tmp_path / "matrix.md"
    content = module.MATRIX_PATH.read_text(encoding="utf-8")
    content = "\n".join(line for line in content.splitlines() if "[#1178]" not in line)
    matrix.write_text(content, encoding="utf-8")

    errors = module.validate_issue_closure_matrix(matrix)

    assert "Missing actionable issue rows: #1178" in errors
