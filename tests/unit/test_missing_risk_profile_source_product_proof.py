...
def test_missing_risk_profile_source_product_proof_is_source_safe_and_bounded() -> None:
    payload = build_missing_risk_profile_source_product_proof_payload(
        generated_at_utc=GENERATED_AT,
    )

    assert payload["schemaVersion"] == MISSING_RISK_PROFILE_SOURCE_PRODUCT_PROOF_SCHEMA_VERSION
    assert payload["sourceAuthority"] == "lotus-advise"
    assert payload["sourceProductId"] == "lotus-advise:AdvisoryPolicyEvaluationRecord:v1"
    assert payload["typedRiskProfileDiagnosticContractReady"] is True
    assert payload["requiredRiskProfileDiagnostics"] == [
        "risk_profile_missing",
        "risk_profile_stale",
        "risk_profile_expired",
        "risk_profile_review_due",
    ]
    assert payload["riskProfileDiagnosticsOwnedByAdvise"] is True
    assert payload["lotusIdeaDoesNotApproveRiskProfile"] is True
...
def test_missing_risk_profile_source_product_proof_rejects_generic_source_product() -> None:
    payload = build_missing_risk_profile_source_product_proof_payload(
        generated_at_utc=GENERATED_AT,
    ...
    assert "advise_risk_profile_source_product_mismatch" in payload["proofBlockers"]
    assert "advise_risk_profile_required_diagnostics_missing" in payload["proofBlockers"]
    assert missing_risk_profile_source_product_proof_is_valid(payload) is False


def test_missing_risk_profile_source_product_proof_requires_advise_owned_boundary() -> None:
    payload = build_missing_risk_profile_source_product_proof_payload(
        generated_at_utc=GENERATED_AT,
        source_product_summary={
            "riskProfileDiagnosticsOwnedByAdvise": False,
            "lotusIdeaDoesNotApproveRiskProfile": False,
        },
    )

    assert "advise_risk_profile_diagnostics_not_advise_owned" in payload["proofBlockers"]
    assert "lotus_idea_risk_profile_authority_boundary_missing" in payload["proofBlockers"]
    assert missing_risk_profile_source_product_proof_is_valid(payload) is False
...
def test_missing_risk_profile_source_product_generator_writes_valid_artifact(
    tmp_path: Path,
) -> None:
    module = _load_generator_script()
    output_path = tmp_path / "missing-risk-profile-source-product-proof.json"
...
def _load_generator_script() -> ModuleType:
    script_path = ROOT / "scripts" / "generate_missing_risk_profile_source_product_proof.py"
    spec = importlib.util.spec_from_file_location(
        "generate_missing_risk_profile_source_product_proof",
        script_path,
    )
