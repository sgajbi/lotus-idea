...
from app.api.concentration_risk_signals import EvaluateConcentrationRiskSignalResponse
...
def test_concentration_risk_examples_cover_every_domain_outcome_and_authority_boundary() -> None:
    caller_examples = build_concentration_risk_evaluation_response_examples()
    source_examples = build_source_backed_concentration_risk_evaluation_response_examples()
    ...
    assert all(
        EvaluateConcentrationRiskSignalResponse.model_validate(example)
...
def test_concentration_risk_examples_match_ledger_and_generated_openapi() -> None:
    expected_by_path = {
        CONCENTRATION_RISK_EVALUATE_OPERATION_PATH: (
            build_concentration_risk_evaluation_response_examples()
        ),
...
def test_concentration_risk_contract_blocks_missing_openapi_success_mode() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(CONCENTRATION_RISK_EVALUATE_OPERATION_PATH)
    ...
    errors = module.validate_concentration_risk_evaluation_success_contract(
        endpoint,
        openapi_spec,
    )

    assert errors == [
        "('POST', '/api/v1/idea-signals/concentration-risk/evaluate'): OpenAPI 200 "
        "examples must exactly match every named code-owned concentration-risk-evaluation "
        "success mode"
    ]


def test_concentration_risk_contract_blocks_missing_behavior_evidence() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(CONCENTRATION_RISK_EVALUATE_FROM_SOURCE_OPERATION_PATH)
    endpoint["test_evidence"].remove(
        "tests/integration/test_concentration_risk_signal_api.py::"
        "test_concentration_risk_signal_from_source_reports_not_eligible"
    )

    errors = module.validate_source_backed_concentration_risk_evaluation_success_contract(endpoint)
...
def _load_contract_module() -> ModuleType:
    script_path = ROOT / "scripts" / "endpoint_concentration_risk_signal_contracts.py"
    spec = importlib.util.spec_from_file_location(
        "endpoint_concentration_risk_signal_contracts",
        script_path,
    )
