...
from app.api.high_volatility_signals import EvaluateHighVolatilitySignalResponse
...
def test_high_volatility_examples_cover_every_domain_outcome_and_authority_boundary() -> None:
    caller_examples = build_high_volatility_evaluation_response_examples()
    source_examples = build_source_backed_high_volatility_evaluation_response_examples()
    ...
    assert all(
        EvaluateHighVolatilitySignalResponse.model_validate(example)
...
def test_high_volatility_examples_match_ledger_and_generated_openapi() -> None:
    expected_by_path = {
        HIGH_VOLATILITY_EVALUATE_OPERATION_PATH: (
            build_high_volatility_evaluation_response_examples()
        ),
...
def test_high_volatility_contract_blocks_missing_openapi_success_mode() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(HIGH_VOLATILITY_EVALUATE_OPERATION_PATH)
    ...
    errors = module.validate_high_volatility_evaluation_success_contract(
        endpoint,
        openapi_spec,
    )

    assert errors == [
        "('POST', '/api/v1/idea-signals/high-volatility/evaluate'): OpenAPI 200 "
        "examples must exactly match every named code-owned high-volatility-evaluation "
        "success mode"
    ]


def test_high_volatility_contract_blocks_missing_behavior_evidence() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(HIGH_VOLATILITY_EVALUATE_FROM_SOURCE_OPERATION_PATH)
    ...
    errors = module.validate_source_backed_high_volatility_evaluation_success_contract(endpoint)
...
def _load_contract_module() -> ModuleType:
    script_path = ROOT / "scripts" / "endpoint_high_volatility_signal_contracts.py"
    spec = importlib.util.spec_from_file_location(
        "endpoint_high_volatility_signal_contracts",
        script_path,
    )
