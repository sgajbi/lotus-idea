...
from app.api.missing_benchmark_signals import EvaluateMissingBenchmarkSignalResponse
...
def test_missing_benchmark_examples_cover_every_domain_outcome_and_authority() -> None:
    caller_examples = build_missing_benchmark_evaluation_response_examples()
    source_examples = build_source_backed_missing_benchmark_evaluation_response_examples()
    ...
    assert all(
        EvaluateMissingBenchmarkSignalResponse.model_validate(example)
...
def test_missing_benchmark_examples_match_ledger_and_generated_openapi() -> None:
    expected_by_path = {
        MISSING_BENCHMARK_EVALUATE_OPERATION_PATH: (
            build_missing_benchmark_evaluation_response_examples()
        ),
...
def test_missing_benchmark_contract_blocks_missing_openapi_success_mode() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(MISSING_BENCHMARK_EVALUATE_OPERATION_PATH)
    ...
    errors = module.validate_missing_benchmark_evaluation_success_contract(
        endpoint,
        openapi_spec,
    )

    assert errors == [
        "('POST', '/api/v1/idea-signals/missing-benchmark/evaluate'): OpenAPI 200 "
        "examples must exactly match every named code-owned missing-benchmark-evaluation "
        "success mode"
    ]


def test_missing_benchmark_contract_blocks_missing_behavior_evidence() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(MISSING_BENCHMARK_EVALUATE_FROM_SOURCE_OPERATION_PATH)
    ...
    errors = module.validate_source_backed_missing_benchmark_evaluation_success_contract(endpoint)
...
def _load_contract_module() -> ModuleType:
    script_path = ROOT / "scripts" / "endpoint_missing_benchmark_signal_contracts.py"
    spec = importlib.util.spec_from_file_location(
        "endpoint_missing_benchmark_signal_contracts",
        script_path,
    )
