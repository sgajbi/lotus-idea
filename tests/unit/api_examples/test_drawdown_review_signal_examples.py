...
from app.api.drawdown_review_signals import EvaluateDrawdownReviewSignalResponse
...
def test_drawdown_review_examples_cover_every_domain_outcome_and_authority_boundary() -> None:
    caller_examples = build_drawdown_review_evaluation_response_examples()
    source_examples = build_source_backed_drawdown_review_evaluation_response_examples()
    ...
    assert all(
        EvaluateDrawdownReviewSignalResponse.model_validate(example)
...
def test_drawdown_review_examples_match_ledger_and_generated_openapi() -> None:
    expected_by_path = {
        DRAWDOWN_REVIEW_EVALUATE_OPERATION_PATH: (
            build_drawdown_review_evaluation_response_examples()
        ),
...
def test_drawdown_review_contract_blocks_missing_openapi_success_mode() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(DRAWDOWN_REVIEW_EVALUATE_OPERATION_PATH)
    ...
    assert errors == [
        "('POST', '/api/v1/idea-signals/drawdown-review/evaluate'): OpenAPI 200 "
        "examples must exactly match every named code-owned drawdown-review-evaluation "
        "success mode"
    ]


def test_drawdown_review_contract_blocks_missing_behavior_evidence() -> None:
    module = _load_contract_module()
...
def _load_contract_module() -> ModuleType:
    script_path = ROOT / "scripts" / "endpoint_drawdown_review_signal_contracts.py"
    spec = importlib.util.spec_from_file_location(
        "endpoint_drawdown_review_signal_contracts",
        script_path,
    )
