...
from app.api.idea_signal_models import EvaluateMandateRestrictionSignalResponse
...
def test_mandate_restriction_examples_cover_every_domain_outcome_and_authority() -> None:
    caller_examples = build_mandate_restriction_evaluation_response_examples()
    source_examples = build_source_backed_mandate_restriction_evaluation_response_examples()
    ...
    assert all(
        EvaluateMandateRestrictionSignalResponse.model_validate(example)
...
def test_mandate_restriction_examples_match_ledger_and_generated_openapi() -> None:
    expected_by_path = {
        MANDATE_RESTRICTION_EVALUATE_OPERATION_PATH: (
            build_mandate_restriction_evaluation_response_examples()
        ),
...
def test_mandate_restriction_contract_blocks_missing_openapi_success_mode() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(MANDATE_RESTRICTION_EVALUATE_OPERATION_PATH)
    ...
    errors = module.validate_mandate_restriction_evaluation_success_contract(
        endpoint,
        openapi_spec,
    )

    assert errors == [
        "('POST', '/api/v1/idea-signals/mandate-restriction/evaluate'): OpenAPI 200 "
        "examples must exactly match every named code-owned mandate-restriction-evaluation "
        "success mode"
    ]


def test_mandate_restriction_contract_blocks_missing_behavior_evidence() -> None:
    module = _load_contract_module()
    endpoint = _ledger_endpoint(MANDATE_RESTRICTION_EVALUATE_FROM_SOURCE_OPERATION_PATH)
    ...
    errors = module.validate_source_backed_mandate_restriction_evaluation_success_contract(endpoint)
...
def _load_contract_module() -> ModuleType:
    script_path = ROOT / "scripts" / "endpoint_mandate_restriction_signal_contracts.py"
    spec = importlib.util.spec_from_file_location(
        "endpoint_mandate_restriction_signal_contracts",
        script_path,
    )
