...
def test_gateway_workbench_discovery_contract_proof_application_is_noop_for_other_capability() -> (
    None
):
    ...
    result = _apply_gateway_workbench_discovery_contract_proof(
        capability,
...
def test_readiness_uses_gateway_workbench_discovery_contract_proof_without_support_promotion(
    tmp_path: Path,
) -> None:
    proof_ref = "output/workbench/gateway-workbench-discovery-contract-proof.json"
    proof = _bound_aggregate_proof(
        _valid_gateway_workbench_discovery_contract_proof(tmp_path),
        proof_ref,
    )
    ...
    snapshot = build_implementation_proof_readiness_snapshot(
        ...
        gateway_workbench_discovery_contract_proof=proof,
        gateway_workbench_discovery_contract_proof_ref=proof_ref,
    )
    ...
    assert "gateway_workbench_discovery_proof_missing" in data_mesh.blockers
    assert "gateway_workbench_discovery_proof_missing" in runtime_telemetry.blockers
    assert (
        "output/workbench/gateway-workbench-discovery-contract-proof.json"
        in data_mesh.evidence_refs
    )
    assert (
        "output/workbench/gateway-workbench-discovery-contract-proof.json"
...
def _valid_gateway_workbench_discovery_contract_proof(tmp_path: Path) -> dict[str, object]:
