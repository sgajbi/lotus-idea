...
def _load_slice2_structure_gate() -> ModuleType:
    scripts_dir = ROOT / "scripts"
    if str(scripts_dir) not in sys.path:
        sys.path.append(str(scripts_dir))
    script_path = scripts_dir / "slice2_structure_gate.py"
    spec = importlib.util.spec_from_file_location("slice2_structure_gate", script_path)
...
def _write_minimum_slice2_truth(root: Path) -> None:
    ...
    documents = {
        ...
        "wiki/Validation-and-CI.md": ("make slice2-structure-gate\nRFC-0002 Slice 2\n"),
    }
...
def test_slice2_structure_gate_accepts_foundation_only_truth(tmp_path: Path) -> None:
    module = _load_slice2_structure_gate()
    _write_minimum_slice2_truth(tmp_path)

    assert module.validate_slice2_structure(tmp_path, architecture_violations=[]) == []


def test_slice2_structure_gate_blocks_supported_feature_promotion(tmp_path: Path) -> None:
    module = _load_slice2_structure_gate()
    _write_minimum_slice2_truth(tmp_path)
    ...
    errors = module.validate_slice2_structure(tmp_path, architecture_violations=[])

    assert "supported-features features[] must remain empty for Slice 2" in errors


def test_slice2_structure_gate_blocks_missing_documentation_truth(tmp_path: Path) -> None:
    module = _load_slice2_structure_gate()
    _write_minimum_slice2_truth(tmp_path)
...
def test_slice2_structure_gate_reports_architecture_boundary_violations(tmp_path: Path) -> None:
    module = _load_slice2_structure_gate()
    _write_minimum_slice2_truth(tmp_path)

    errors = module.validate_slice2_structure(
        tmp_path,
