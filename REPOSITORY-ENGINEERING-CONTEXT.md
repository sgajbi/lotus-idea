...
posture has 133 tracked RFC-0002 issues, 107 closed and 26 open; #681 is the
in-progress issue and #1094 is fixed locally for PR handoff. #1094 owns the
...
#1094 as the Idea fixed-local issue.
...
`sgajbi/lotus-idea#1094` is the fixed-local Slice 09/17/19 AI-governance
test-support maintainability refactor awaiting PR handoff. `sgajbi/lotus-idea#1091` is the latest
...
(`sgajbi/lotus-idea#681`), 1 fixed-local RFC-0002 issue
(`sgajbi/lotus-idea#1094`), with
...
repositories: 164 closed and 38 open. The open split is 25 `status/blocked`, 1 `status/fixed-local`, 1 `status/in-progress`, 1 `status/merged-main`, 2 `status/merged-to-main`, 0 `status/pr-open`, 8 `status/tracker`.
