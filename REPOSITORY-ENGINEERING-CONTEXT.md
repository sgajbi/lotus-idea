...
clones. Workflow and operator route error metadata and `app.main` exception
handlers now use `src/app/api/problem_details.py` for product-safe
400/403/404/409/503 OpenAPI examples and runtime response helpers, and
...
6. `src/app/infrastructure/`: HTTP/database/message adapters behind ports. The
   ...
   succeeded, but must not be inferred as current freshness. Manage source refs
   accept only explicit `current` or `same_day` freshness values as current;
   `ready` remains supportability/data-quality vocabulary and fails closed as
   unavailable freshness. `make source-observability-contract-gate` enforces
   that future `lotus_*_sources.py` adapters cannot return
   `EvidenceFreshness.CURRENT` directly from readiness, supportability,
...
report-only CI timing/signal evidence,
...
CI timing/signal evidence is report-only and source-safe. Feature Lane, PR
Merge Gate, and Main Releasability now upload lane-specific
`ci-signal-evidence.json` artifacts generated by `scripts/ci_signal_evidence.py`
from GitHub run-job metadata. The artifact records job/step duration,
conclusion, critical path, failure category, and the explicit
`thresholdEnforced: false` posture; it does not contain secrets, client data,
portfolio identifiers, or live source payloads. Main Releasability
`release-evidence.json` references `main-releasability-ci-signal-evidence` and
`ci-signal-evidence.json`. `make ci-signal-evidence-contract-gate` validates
the schema without live GitHub access, and `make ci-contract-gate` prevents the
workflow wiring from being removed.
...
caller-context header binding, and scope-unaware signal permission checks.
Caller-supplied signal routes must parse identity, capability, and entitlement
scope through `CallerContextHeaders`, then pass each request's `accessScope`
into `signal_permission_problem_or_none(...)`. Scoped requests fail closed with
product-safe 403 behavior when caller entitlement-scope headers are absent or
do not cover the requested tenant/book/portfolio/client. The gate also requires
shared signal API support for source-authority, operation-event, and
problem-detail behavior, and every signal evaluation route metadata block must
...
response metadata and runtime permission/request-failure helpers. Keep route-
or handler-specific error codes and descriptions close to the caller, but do
not hand-roll duplicate `ProblemDetails` shapes for lifecycle, review,
feedback, conversion, report-evidence, readiness/operator APIs, or entrypoint
