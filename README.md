...
| Downstream intent | Conversion intent, source-versioned append-only Advise/Manage outcome history, policy-valid current posture, report evidence-pack requests, outbox records, and digest-bound downstream contracts are bounded foundations. Advise and Manage submission persist owner identities and support entitlement-gated reconciliation of exact append-only owner history. Report submission validates and persists the exact materialization receipt, including report-job identity, render/archive creation posture, and explicit remaining blockers. Complete trusted tenant/book/portfolio/client scope is checked before dispatch and bound into idempotency. | Owner receipts prove only what the owning service states. They do not infer suitability, execution, report completion, client publication, or supported-feature posture. Gateway/Workbench realization, production identity, lifecycle-safe publication, and remaining Report/Render/Archive certification stay open; malformed or uncertain receipts require reconciliation. |
