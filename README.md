...
Current implemented foundations include source-authority-preserving high-cash, concentration-risk, underperformance, allocation-drift mandate-review, high-volatility / drawdown review, missing suitability context, and low-income / liquidity-shortfall policies with Core, Risk, Performance, Manage, and Advise source adapters; the low-income foundation consumes Core cashflow evidence only for advisor-review candidates, and a source-safe Core cashflow live-proof artifact can clear only the low-income live Core cashflow source blocker. It does not certify client income needs, funding advice, treasury instruction, suitability, planning claims, data mesh, Workbench behavior, client publication, or supported-feature promotion.
