...
| Lifecycle and review | Candidate persistence, replay, review actions, feedback, queues, scoring, audit-oriented state transitions, and tenant-scoped retention/hold/erasure/purge enforcement are implemented as internal foundations. Data lifecycle includes versioned classification/residency policy, transactional PostgreSQL controls, pseudonymized tombstones, bounded trust telemetry, aggregate-only scheduled expiry review, production-profile verification plus single-use signed bank-authority decisions, independently verified Lotus AI provider-retention receipts, and independently verified Archive posture for linked report evidence. Archive decisions bind tenant, candidate, evidence pack, document, policy, action, digest, TTL, and signing key; applied receipts are restart-safe and single-use. | Live bank authority producer/key-discovery proof, jurisdiction approval, managed Archive store/key/trust-distribution proof, provider-native AI confirmation and approval, production-authorized Report/Archive policy sources, production purge proof, client publication, and supported-feature promotion remain blocked. |
