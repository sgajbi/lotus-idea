...
| Downstream intent | Conversion intent, source-versioned append-only outcome history, policy-valid current posture, report evidence-pack requests, outbox records, and digest-bound Advise/Manage/Report source-contract provenance are bounded foundations. Advise submission now persists the owner intake/realization receipt for both accepted-for-review and rejected-before-work outcomes; an entitlement-gated reconciliation use case reads the exact Advise-owned append-only history and persists it by opaque support reference without inferring suitability, execution, or publication authority. Downstream submission fails closed unless complete trusted tenant/book/portfolio/client scope covers the persisted candidate and binds that scope into idempotency. | The Advise consumer is implementation-backed but remains unpromoted pending cross-repository exact-main/runtime acceptance and Gateway/Workbench proof. Manage and Report/Render/Archive realization, production identity, supportable route acceptance, execution, rendering, archive authority, and materialization proof remain open. Quarantined outcome history does not count as ready. |
