...
Performance, and Manage source adapters. High-volatility consumes Risk-owned volatility only, and
its proof contract can clear only the live Risk volatility blocker; drawdown proof remains blocked. The Manage adapter blocks store-wide supportability evidence from
...
| `make risk-concentration-live-proof-contract-gate`, `make high-volatility-live-proof-contract-gate` | Validate source-safe Lotus Risk concentration and high-volatility live-proof artifact contracts that can clear only their named opportunity-archetype live Risk blockers when valid artifacts are supplied. |
