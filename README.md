...
Source-safe Manage mandate live proof can now clear only the portfolio-scoped Manage action-register source blocker when live `lotus-manage:PortfolioActionRegister:v1` evidence proves current workflow decisions, lineage, source freshness, and portfolio-scope confirmation; source-safe Core portfolio-state live proof can clear only the allocation-drift Core portfolio-state source-ref blocker when live `lotus-core:PortfolioStateSnapshot:v1` evidence proves current source freshness. Mandate performance-health, mandate risk-health, data-mesh, Workbench, client-publication, supported-feature, rebalance, action, and order-execution blockers remain.
