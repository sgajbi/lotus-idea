...
The missing risk-profile foundation consumes only explicit Advise-owned risk-profile diagnostic posture to create an advisor-review evidence-gap candidate; it does not approve risk profiling, suitability, policy, proposal, client publication, or external communication. Source-safe missing-suitability live Advise proof can clear only the corresponding opportunity-archetype live-source blocker. The mandate/restriction review foundation adds a source-safe `POST /api/v1/idea-signals/mandate-restriction/evaluate` API over caller-supplied Core, Manage, or Advise restriction posture evidence; it creates only compliance-review candidates and keeps live upstream source proof, typed restriction source products, Workbench, data-mesh, client-publication, and supported-feature blockers intact.
