...
| --- | --- | --- |
| Opportunity policies | Bounded foundations exist for high cash, concentration risk, underperformance, allocation drift, bond maturity, volatility, drawdown, suitability gaps, risk-profile gaps, mandate/restriction review, income gaps, and missing benchmark review. Missing benchmark now has a closed v2 Core runtime contract that binds one source fetch to pseudonymous request, assignment-state, and deterministic candidate or no-opportunity receipts. Advise-backed suitability, risk-profile, and mandate/restriction policies use the same receipt-bound standard while retaining separate domain outcomes. | Performance benchmark readiness, typed source products, data-mesh, Workbench, client-publication, deployment, production, and supported-feature certification remain blocked. Core retains benchmark-assignment authority; Idea creates review posture only. |
